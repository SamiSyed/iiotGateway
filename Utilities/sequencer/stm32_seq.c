...
/**
 * @brief macro used to enter the critical section before calling the IDLE
 function
 * @note  in a basic configuration shall be identical to the macro
 *        UTIL_SEQ_ENTER_CRITICAL_SECTION. The redefinition of this macro will
 allow
...
/**
 * @brief default number of task is default 32 (maximum), can be reduced by
 * redefining in utilities_conf.h
 */
 ...
 * That is the reason why many variables that are used only in that function are
 * declared static. Note: These variables could have been declared static in the
 * function.
 *
 */
void UTIL_SEQ_Run(UTIL_SEQ_bm_t Mask_bm) {
  ...
  /*
   * When this function is nested, the mask to be applied cannot be larger than
   * the first call The mask is always getting smaller and smaller A copy is
   * made of the mask set by UTIL_SEQ_Run() in case it is called again in the
   * task
   */
  ...
  while (((local_taskset & local_taskmask & SuperMask) != 0U) &&
    ...
    counter = 0U;
    /*
     * When a flag is set, the associated bit is set in
     * TaskPrio[counter].priority mask depending on the priority parameter given
     * from UTIL_SEQ_SetTask() The while loop is looking for a flag set from the
     * highest priority maskr to the lower
     */
     ...
     * The concept is to make sure that on each round on UTIL_SEQ_Run(), if two
     * same flags are always set, the sequencer does not run always only the
     * first one. When a task has been executed, The flag is removed from the
     * round_robin mask. If on the next UTIL_SEQ_RUN(), the two same flags are
     * set again, the round_robin mask will mask out the first flag so that the
     * second one can be executed. Note that the first flag is not removed from
     * the list of pending task but just masked by the round_robin mask
     *
     * In the check below, the round_robin mask is reinitialize in case all
     * pending tasks haven been executed at least once
     */
     ...
     * Once the index is read, the associated task will be executed even though
     * a higher priority stack is requested before task execution.
     */
    CurrentTaskIdx =
        (SEQ_BitPosition(current_task_set & TaskPrio[counter].round_robin));

    /*
     * remove from the roun_robin mask the task that has been selected to be
     * executed
     */
    TaskPrio[counter].round_robin &= ~(1U << CurrentTaskIdx);

    UTIL_SEQ_ENTER_CRITICAL_SECTION();
    /* remove from the list or pending task the one that has been selected to be
     * executed */
    TaskSet &= ~(1U << CurrentTaskIdx);
    /* remove from all priority mask the task that has been selected to be
     * executed */
  ...
  /* the set of CurrentTaskIdx to no task running allows to call WaitEvt in the
   * Pre/Post ilde context */
...
uint32_t UTIL_SEQ_IsSchedulableTask(UTIL_SEQ_bm_t TaskId_bm) {
  ...
  local_taskset = TaskSet;
  _status = ((local_taskset & TaskMask & SuperMask & TaskId_bm) == TaskId_bm)
                ? 1U
                : 0U;
...
void UTIL_SEQ_WaitEvt(UTIL_SEQ_bm_t EvtId_bm) {
   ...
   * It case it occurs while waiting for the second one, the while loop will
   * exit immediately
   */
  ...
  /*
   * Restore the CurrentTaskIdx that may have been modified by call of
   * UTIL_SEQ_Run() from UTIL_SEQ_EvtIdle() This is required so that a second
   * call of UTIL_SEQ_WaitEvt() in the same process pass the correct
   * current_task_id_bm in the call of UTIL_SEQ_EvtIdle()
   */
...
__WEAK void UTIL_SEQ_Idle(void) { return; }
 ...
 * @retval bit position
 */
uint8_t SEQ_BitPosition(uint32_t Value) { return (uint8_t)(31 - __CLZ(Value)); }
#endif
