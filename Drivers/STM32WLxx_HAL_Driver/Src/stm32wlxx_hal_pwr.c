 ...
 * @note   RTCSEL that sets the RTC clock source selection is in the RTC backup
...
regulators off.
      ...
      (+) Note: system power mode depends on each sub-system (CPU1, CPU2, radio)
power modes. Each CPU request to enter in a low-power mode will make system
enter in the equivalent low-power mode if all other sub-systems are aligned.
...
API's:
          (++) HAL_PWREx_EnterSTOP0Mode() for mode 0, HAL_PWREx_EnterSTOP1Mode()
for mode 1, HAL_PWREx_EnterSTOP2Mode() for mode 2 or for porting reasons
HAL_PWR_EnterSTOPMode().

      (+) Regulator setting (applicable to HAL_PWR_EnterSTOPMode() only):
          (++) PWR_MAINREGULATOR_ON: Regulator in main mode (STOP0 mode)
          (++) PWR_LOWPOWERREGULATOR_ON: Regulator in low-power mode (STOP1
mode)
...
HAL_PWREx_EnableSRAMRetention() API to set RRS bit.
      (++) Exit:
...
time-stamp event, external reset in NRST pin.
    [..] After waking up from Shutdown mode, program execution restarts in the
same way as after a Reset.
...
void HAL_PWR_EnableWakeUpPin(uint32_t WakeUpPinPolarity) {
  ...
  MODIFY_REG(PWR->CR4,
             ((PWR_CR4_WP1 | PWR_CR4_WP2 | PWR_CR4_WP3) & WakeUpPinPolarity),
 ...
 * in low-power mode)
 * @note  Low-power Sleep mode is entered from Low-power Run mode (low-power
 * regulator used and clock frequency below 2 MHz) and by calling
 * HAL_PWR_EnterSLEEPMode() with Regulator set to PWR_LOWPOWERREGULATOR_ON.
 ...
 * legacy STM32 series where only "Stop mode" is mentioned with main or low
 * power regulator ON. It is recommended to use functions:
 ...
 * regulator ON)
 * @param STOPEntry Specifies Stop 0, Stop 1 mode is entered with WFI or WFE
 * instruction. This parameter can be one of the following values:
 ...
 * preserved in which case the regulator is in low-power mode. SRAM and register
 * contents are lost except for registers in the Backup domain and Standby
 * circuitry. SRAM2 content can be preserved if the bit RRS is set in PWR_CR3
 * register. To enable this feature, the user can resort to
 * HAL_PWREx_EnableSRAMRetention() API to set RRS bit. The BOR is available.
