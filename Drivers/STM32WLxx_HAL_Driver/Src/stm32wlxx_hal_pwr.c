...
regulators off.
      ...
      (+) Note: system power mode depends on each sub-system (CPU1, CPU2, radio) power modes.
                Each CPU request to enter in a low-power mode will make system enter in the
equivalent low-power mode if all other sub-systems are aligned.
...
HAL_PWREx_EnterSTOP2Mode() for mode 2 or for porting reasons HAL_PWR_EnterSTOPMode().
...
register. To enable this feature, the user can resort to HAL_PWREx_EnableSRAMRetention() API to set
RRS bit.
      (++) Exit:
 ...
 *        and clock frequency below 2 MHz) and by calling HAL_PWR_EnterSLEEPMode() with Regulator
 * set to PWR_LOWPOWERREGULATOR_ON. Additionally, the clock frequency must be reduced below 2 MHz.
 ...
 *        Standby circuitry. SRAM2 content can be preserved if the bit RRS is set in PWR_CR3
 * register. To enable this feature, the user can resort to HAL_PWREx_EnableSRAMRetention() API to
 * set RRS bit. The BOR is available.
