...
#define PWR_PORTC_AVAILABLE_PINS                                               \
  (PWR_GPIO_BIT_15 | PWR_GPIO_BIT_14 | PWR_GPIO_BIT_13 | PWR_GPIO_BIT_6 |      \
   PWR_GPIO_BIT_5 | PWR_GPIO_BIT_4 | PWR_GPIO_BIT_3 | PWR_GPIO_BIT_2 |         \
   PWR_GPIO_BIT_1 | PWR_GPIO_BIT_0)
 ...
 * PWPWR_REGULATOR_VOLTAGE_SCALE2)
 */
 ...
 * range 1 mode, typical output voltage at 1.2 V, system frequency up to 48 MHz.
 ...
 * the system frequency can be increased to a value up to 48 MHz after calling
...
HAL_StatusTypeDef HAL_PWREx_ControlVoltageScaling(uint32_t VoltageScaling) {
  ...
  if (VoltageScaling == PWR_REGULATOR_VOLTAGE_SCALE1) {
    if (READ_BIT(PWR->CR1, PWR_CR1_VOS) != PWR_REGULATOR_VOLTAGE_SCALE1) {
      ...
      wait_loop_index =
          ((PWR_FLAG_SETTING_DELAY_US * SystemCoreClock) / 1000000UL);
...
#if defined(DUAL_CORE)
/**
 * @brief  Hold CPU and allocated peripherals after reset or wakeup from Stop or
 * Standby.
...
/**
 * @brief  Release CPU and allocated peripherals after reset or wakeup from Stop
 * or Standby.
 ...
 * @retval None
 */
void HAL_PWREx_EnableWakeUp_ILAC(void) { LL_PWR_C2_EnableWakeUp_ILAC(); }
 ...
 * @retval None
 */
void HAL_PWREx_DisableWakeUp_ILAC(void) { LL_PWR_C2_DisableWakeUp_ILAC(); }
 ...
 * @retval None
 */
void HAL_PWREx_EnableBORPVD_ULP(void) { SET_BIT(PWR->CR3, PWR_CR3_ULPEN); }
 ...
 * @retval None
 */
void HAL_PWREx_DisableBORPVD_ULP(void) { CLEAR_BIT(PWR->CR3, PWR_CR3_ULPEN); }
