...
ErrorStatus LL_RCC_DeInit(void) {
  ...
  /* Reset CFGR register */
  LL_RCC_WriteReg(CFGR, 0x00070000U); /* MSI selected as System Clock and all
                                         prescaler to not divided */

  /* Read CR register */
  vl_mask = LL_RCC_ReadReg(CR);

  /* Reset HSION, HSIKERON, HSIASFS, HSEON, HSEBYP, HSEPRE, HSEBYPPWR and
   * PLLSYSON bits */
  CLEAR_BIT(vl_mask,
            (RCC_CR_HSION | RCC_CR_HSIKERON | RCC_CR_HSIASFS | RCC_CR_HSEON |
  ...
  vl_mask = RCC_CICR_LSIRDYC | RCC_CICR_LSERDYC | RCC_CICR_MSIRDYC |
            RCC_CICR_HSIRDYC | RCC_CICR_HSERDYC | RCC_CICR_PLLRDYC |
 ...
 * HSE_VALUE(***) or HSI_VALUE(**) or MSI values(*) multiplied/divided by the
 * PLL factors.
...
void LL_RCC_GetSystemClocksFreq(LL_RCC_ClocksTypeDef *RCC_Clocks) {
  ...
  RCC_Clocks->HCLK1_Frequency =
      RCC_GetHCLK1ClockFreq(RCC_Clocks->SYSCLK_Frequency);

#if defined(DUAL_CORE)
  /* HCLK2 clock frequency */
  RCC_Clocks->HCLK2_Frequency =
      RCC_GetHCLK2ClockFreq(RCC_Clocks->SYSCLK_Frequency);
#endif /* DUAL_CORE */

  /* HCLK3 clock frequency */
  RCC_Clocks->HCLK3_Frequency =
      RCC_GetHCLK3ClockFreq(RCC_Clocks->SYSCLK_Frequency);

  /* PCLK1 clock frequency */
  RCC_Clocks->PCLK1_Frequency =
      RCC_GetPCLK1ClockFreq(RCC_Clocks->HCLK1_Frequency);

  /* PCLK2 clock frequency */
  RCC_Clocks->PCLK2_Frequency =
      RCC_GetPCLK2ClockFreq(RCC_Clocks->HCLK1_Frequency);
}
  ...
  if (USARTxSource == LL_RCC_USART1_CLKSOURCE) {
    ...
    default:
      usart_frequency = RCC_GetPCLK2ClockFreq(
          RCC_GetHCLK1ClockFreq(RCC_GetSystemClockFreq()));
      break;
    }
  } else {
 ...
 *         - @ref  LL_RCC_PERIPH_FREQUENCY_NO indicates that oscillator (MSI) or
 * PLLs (PLL) is not ready
 */
uint32_t LL_RCC_GetRNGClockFreq(uint32_t RNGxSource) {
  ...
  case LL_RCC_RNG_CLKSOURCE_MSI: /* MSI used as RNG clock source */
    if (LL_RCC_MSI_IsReady() == 1U) {
      rng_frequency =
          __LL_RCC_CALC_MSI_FREQ(LL_RCC_MSI_IsEnabledRangeSelect(),
...
static uint32_t RCC_GetHCLK2ClockFreq(uint32_t SYSCLK_Frequency) {
  /* HCLK clock frequency */
  return __LL_RCC_CALC_HCLK2_FREQ(SYSCLK_Frequency,
