/**
  ...
  ******************************************************************************
  ...
  default weak (surcharged) function. HAL_SMARTCARD_UnRegisterCallback() takes
  as parameters the HAL peripheral handle, and the Callback ID. This function
  ...
  (surcharged) functions: examples HAL_SMARTCARD_TxCpltCallback(),
  ...
  functions in the HAL_SMARTCARD_Init() and HAL_SMARTCARD_DeInit() only when
  these callbacks are null (not registered beforehand). If not, MspInit or
  MspDeInit are not null, the HAL_SMARTCARD_Init() and HAL_SMARTCARD_DeInit()
...
HAL_StatusTypeDef HAL_SMARTCARD_Transmit(SMARTCARD_HandleTypeDef *hsmartcard,
  ...
  if (hsmartcard->gState == HAL_SMARTCARD_STATE_READY) {
    ...
    /* In case of TX only mode, if NACK is enabled, the USART must be able to
       ...
       error.
       Therefore, the receiver block must be enabled as well (RE bit must be
       set). */
...
SMARTCARD_SetConfig(SMARTCARD_HandleTypeDef *hsmartcard) {
  ...
  MODIFY_REG(hsmartcard->Instance->RTOR, (USART_RTOR_RTO | USART_RTOR_BLEN),
             tmpreg);
