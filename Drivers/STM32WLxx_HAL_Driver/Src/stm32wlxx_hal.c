...
#define __STM32WLxx_HAL_VERSION                                                \
  ((__STM32WLxx_HAL_VERSION_MAIN << 24U) |                                     \
   (__STM32WLxx_HAL_VERSION_SUB1 << 16U) |                                     \
   (__STM32WLxx_HAL_VERSION_SUB2 << 8U) | (__STM32WLxx_HAL_VERSION_RC))
 ...
 * defined in user file "stm32wlxx_hal_msp.c" to do the global low level
...
#ifdef CORE_CM0PLUS
    if (HAL_SYSTICK_Config(HAL_RCC_GetHCLK2Freq() /
 ...
 */
uint32_t HAL_GetHalVersion(void) { return __STM32WLxx_HAL_VERSION; }
 ...
 * @note  This functionality does not influence CPU2 operation, CPU2 cannot be
 * debugged in Stop mode even when this bit is enabled
 * @retval None
 */
 ...
 * @note  This functionality does not influence CPU2 operation, CPU2 cannot be
 * debugged in Standby mode even when this bit is enabled
 * @retval None
 */
 ...
 * @param  Interrupt Pointer to a SYSCFG_InterruptTypeDef structure that
 * contains the Interrupt Mask configuration
 * @retval None
 */
void HAL_SYSCFG_EnableIT(SYSCFG_InterruptTypeDef *Interrupt) {
  uint32_t InterruptMask1 =
      (Interrupt->InterruptMask1 & ~HAL_SYSCFG_GRP1_RESERVED);
  uint32_t InterruptMask2 =
      (Interrupt->InterruptMask2 & ~HAL_SYSCFG_GRP2_RESERVED);
...
HAL_StatusTypeDef HAL_RADIO_Init(void) {
  ...
  gpio_init_structure.Pin = (RADIO_SWITCH_CTRL1_PIN | RADIO_SWITCH_CTRL2_PIN |
                             RADIO_SWITCH_CTRL3_PIN);
  ...
  status = HAL_GPIO_LockPin(RADIO_SWITCH_CTRL_GPIO_PORT,
                            (RADIO_SWITCH_CTRL1_PIN | RADIO_SWITCH_CTRL2_PIN |
                             RADIO_SWITCH_CTRL3_PIN));
  if (status == HAL_OK) {
...
HAL_RADIO_SetSwitchConfig(HAL_RADIO_SwitchConfig_TypeDef Config) {
  ...
  case RADIO_SWITCH_RFO_LP: {
    ...
    HAL_GPIO_WriteMultipleStatePin(
        RADIO_SWITCH_CTRL_GPIO_PORT, RADIO_SWITCH_CTRL2_PIN,
  ...
  case RADIO_SWITCH_RFO_HP: {
    ...
    HAL_GPIO_WriteMultipleStatePin(
        RADIO_SWITCH_CTRL_GPIO_PORT, RADIO_SWITCH_CTRL1_PIN,
 ...
 */
uint8_t HAL_RADIO_IsTCXO(void) { return RADIO_CONF_TCXO_SUPPORTED; }
 ...
 */
uint8_t HAL_RADIO_IsDCDC(void) { return RADIO_CONF_DCDC_SUPPORTED; }
