/**
  ...
  ******************************************************************************
  ...
  authentication.
         (##) The HeaderSize providing the size of the header buffer in words or
  ...
  *** Callback registration ***
  ...
  [..]
  By default, after the HAL_CRYP_Init() and when the state is
  HAL_CRYP_STATE_RESET all callbacks are set to the corresponding weak functions
  : examples HAL_CRYP_InCpltCallback() , HAL_CRYP_OutCpltCallback(). Exception
  done for MspInit and MspDeInit functions that are reset to the legacy weak
  function in the HAL_CRYP_Init()/ HAL_CRYP_DeInit() only when these callbacks
  are null (not registered beforehand). if not, MspInit or MspDeInit are not
  null, the HAL_CRYP_Init() / HAL_CRYP_DeInit() keep and use the user
...
static void CRYP_Write_SuspendRegisters(CRYP_HandleTypeDef *hcryp,
                                        uint32_t *Input);
static void CRYP_Read_KeyRegisters(CRYP_HandleTypeDef *hcryp, uint32_t *Output,
                                   uint32_t KeySize);
static void CRYP_Write_KeyRegisters(CRYP_HandleTypeDef *hcryp, uint32_t *Input,
                                    uint32_t KeySize);
...
the CRYP_ConfigTypeDef Parameters which are configured in This section are :
          ...
          (++) AlgoMode :
              ...
              (+++) for TinyAES2 peripheral, only ECB,CBC,CTR,GCM/GMAC and CCM
in AES Standard are supported.
...
HAL_StatusTypeDef HAL_CRYP_Init(CRYP_HandleTypeDef *hcryp) {
  ...
  MODIFY_REG(
      hcryp->Instance->CR, AES_CR_DATATYPE | AES_CR_KEYSIZE | AES_CR_CHMOD,
 ...
 * @param  Size Length of the plaintext buffer in bytes or words (depending upon
 * DataWidthUnit field)
...
static void CRYP_SetDMAConfig(CRYP_HandleTypeDef *hcryp, uint32_t inputaddr,
  ...
  if (HAL_DMA_Start_IT(hcryp->hdmain, inputaddr,
                       (uint32_t)&hcryp->Instance->DINR, Size) != HAL_OK) {
    /* DMA error code field */
    hcryp->ErrorCode |= HAL_CRYP_ERROR_DMA;
  ...
  if (HAL_DMA_Start_IT(hcryp->hdmaout, (uint32_t)&hcryp->Instance->DOUTR,
                       outputaddr, Size) != HAL_OK) {
    /* DMA error code field */
    hcryp->ErrorCode |= HAL_CRYP_ERROR_DMA;
...
CRYP_GCMCCM_SetPayloadPhase_DMA(CRYP_HandleTypeDef *hcryp) {
  ...
  if (hcryp->Size == 0U) {
    ...
    /*DMA transfer */
    CRYP_SetDMAConfig(hcryp, (uint32_t)(hcryp->pCrypInBuffPtr), wordsize,
