/**
  ...
  ******************************************************************************
  ...
  __HAL_RCC_AES_CLK_ENABLE for TinyAES peripheral
      ...
      (#)Initialize the CRYP according to the specified parameters :
         ...
         (##) The DataWidthUnit field. It specifies whether the data length (or the payload length
  for authentication algorithms) is in words or bytes.
         (##) The Header used only in AES GCM and CCM Algorithm for authentication.
         (##) The HeaderSize providing the size of the header buffer in words or bytes, depending
  upon HeaderWidthUnit field.
         (##) The HeaderWidthUnit field. It specifies whether the header length (for authentication
  algorithms) is in words or bytes.
         (##) The B0 block is the first authentication block used only in AES CCM mode.
         (##) The KeyIVConfigSkip used to process several messages in a row (please see more
  information below).
  ...
  HAL_CRYP_Decrypt_IT(), HAL_CRYP_Encrypt_DMA() or HAL_CRYP_Decrypt_DMA().
    ...
    [..]
      ...
      (#) The data encryption standard (DES) and Triple-DES (TDES) supported only by CRYP1
  peripheral:
  ...
  {B15..B0}. The 4 32-bit words that make the key must be stored as follows in MCU memory:
    ...
    [..]  This section describes the AES Galois/counter mode (GCM) supported by both CRYP1 and
  TinyAES peripherals:
      (#)  Algorithm supported :
  ...
  the cleartext message only cleartext payload (not the ciphertext payload) is used and no outpout.
         (##) Final phase: peripheral generates the authenticated tag (T) using the last block of
  data.
...
CRYP_ConfigTypeDef Parameters which are configured in This section are :
          ...
          (++) AlgoMode :
              ...
              (+++) for TinyAES2 peripheral, only ECB,CBC,CTR,GCM/GMAC and CCM in AES Standard are
supported.
...
HAL_StatusTypeDef HAL_CRYP_Init(CRYP_HandleTypeDef *hcryp)
{
    ...
    /* Set the key size (This bit field is do not care in the DES or TDES modes), data type and
     * Algorithm */
...
HAL_StatusTypeDef HAL_CRYP_Suspend(CRYP_HandleTypeDef *hcryp)
{
    ...
    else
    {
        ...
        hcryp->Size_saved
            = ((hcryp->Init.DataWidthUnit == CRYP_DATAWIDTHUNIT_WORD) ? (hcryp->Size / 4U)
                                                                      : hcryp->Size);
...
HAL_StatusTypeDef HAL_CRYP_Resume(CRYP_HandleTypeDef *hcryp)
{
    ...
    else
    {
        ...
        else /* Authentication algorithms case */
        {
            ...
            /* At the same time, set handle state back to READY to be able to resume the AES
            calculations without the processing APIs returning HAL_BUSY when called. */
            hcryp->State = HAL_CRYP_STATE_READY;
        }
 ...
 * @param  Size Length of the plaintext buffer in bytes or words (depending upon DataWidthUnit
 * field)
...
static HAL_StatusTypeDef CRYP_AESGCM_Process_IT(CRYP_HandleTypeDef *hcryp)
{
    ...
    if (DoKeyIVConfig == 1U)
    {
        ...
        if (hcryp->Init.HeaderSize == 0U) /*header phase is  skipped*/
        {
            ...
            else /* Size < 16Bytes  : first block is the last block*/
            {
                ...
                <128bit do not use CRYP_Encrypt_IT otherwise TAG is incorrectly generated for GCM
                Encryption. */
           ...
           only process here message payload */
    {
        ...
        else /* Size < 16Bytes  : first block is the last block*/
        {
            ...
            payload <128bit do not use CRYP_Encrypt_IT otherwise TAG is incorrectly generated for
            GCM Encryption. */
...
static HAL_StatusTypeDef CRYP_GCMCCM_SetPayloadPhase_DMA(CRYP_HandleTypeDef *hcryp)
{
    ...
    else /* length of input data is < 16 */
    {
        ...
        /* Set Npblb in case of AES GCM payload encryption or AES CCM payload decryption to get
         * right tag*/
...
static HAL_StatusTypeDef CRYP_GCMCCM_SetHeaderPhase_DMA(CRYP_HandleTypeDef *hcryp)
{
    ...
    if (headersize_in_bytes >= 16U)
    {
        ...
        if (CRYP_SetHeaderDMAConfig(hcryp,
                                    (uint32_t)(hcryp->Init.Header),
                                    (uint16_t)((headersize_in_bytes / 16U) * 4U))
            != HAL_OK)
        {
            return HAL_ERROR;
        }
...
static void CRYP_Read_SuspendRegisters(CRYP_HandleTypeDef *hcryp, uint32_t *Output)
{
    ...
    if (READ_BIT(hcryp->Instance->CR, (AES_CR_CHMOD | AES_CR_GCMPH | AES_CR_MODE))
        == (CRYP_AES_GCM_GMAC | AES_CR_GCMPH_1 | 0x0U))
    {
