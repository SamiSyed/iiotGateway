/**
  ...
  *** Internal TimeStamp configuration ***
  ...
  initialization by setting ClockEnable inti field to RTC_CLOCK_DISABLE.
  ...
  HAL_RTCEx_BKUPWrite() function.
     (+) To read the RTC Backup Data registers, use the HAL_RTCEx_BKUPRead()
         function.
     (+) Before calling these functions you have to call HAL_RTC_Init() in order
  to perform TAMP base address offset calculation.
 ...
 * to avoid any RTC software execution after Wake Up. That is why when
 * WakeUpAutoClr is set, EXTI is configured as EVENT instead of Interrupt to
 * avoid useless IRQ handler execution.
 * @retval HAL status
 */
  ...
  if (sIntTamper->NoErase != RTC_TAMPER_ERASE_BACKUP_ENABLE) {
    ...
    SET_BIT(TAMP->CR3,
            (sIntTamper->IntTamper >>
             16U)); /* Shift of 16 bit to manage ITAMP on MSB part of CR3 */
  }
