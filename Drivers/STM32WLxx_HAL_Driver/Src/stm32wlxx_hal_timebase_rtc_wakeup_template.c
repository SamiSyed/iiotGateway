/**
  ...
  *           + Initializes the RTC peripheral and configures the wakeup timer
  to be
  *             incremented each 1ms when uwTickFreq is set to default value,
  else
...
#error Please select the RTC Clock source
    ...
    if (status == HAL_OK) {
      ...
      if (status == HAL_OK) {
        ...
        status = HAL_RTCEx_SetWakeUpTimer_IT(&hRTC_Handle, wucounter,
                                             RTC_WAKEUPCLOCK_RTCCLK_DIV2, 0);
 ...
 *         RTC_WKUP_IRQHandler(). It makes a direct call to HAL_IncTick() to
