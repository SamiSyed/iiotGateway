/**
  ...
  ******************************************************************************
  ...
  and HAL_GTZC_TZIC_ClearFlag() functions
    ...
    (#) Illegal access interrupt service routing is served by
  HAL_GTZC_IRQHandler() and user can add his own code using
  HAL_GTZC_TZIC_ILA_Callback()
...
/** @defgroup GTZC_Exported_Functions_Group1 TZSC Initialization and
 ...
 * @param  PeriphAttributes Periph attribute  (see structure in
 * stm32wlxx_hal_gtzc.h).
 * @retval HAL status
 */
HAL_StatusTypeDef
...
#if defined(CORE_CM0PLUS)
    /* secure configuration. Make sure not to configure SUBGHZ SPI which is
     * securable by option byte. */
    if ((PeriphId != GTZC_PERIPH_SUBGHZSPI) &&
        ((PeriphAttributes & GTZC_TZSC_ATTRIBUTE_SEC) != 0x00U)) {
 ...
 * @param  PeriphId Peripheral identifier.
 *         This parameter can be a value of @ref GTZC_Peripheral_Identification
 * @param  PeriphAttributes Pointer to the periph attributes (see structure in
 * stm32wlxx_hal_gtzc.h). If PeriphId target a single peripheral, pointer on a
 * single element. If all peripherals selected, pointer to an array of
 * GTZC_PERIPH_TZSC_MAX elements
 * @retval HAL status
 */
HAL_StatusTypeDef
HAL_GTZC_TZSC_GetConfigPeriphAttributes(uint32_t PeriphId,
  ...
  if ((PeriphId & GTZC_PERIPH_ALL) != 0x00U) {
    ...
    /* special case where the configuration getting is requested for all
     * peripherals */
...
/** @defgroup GTZC_Exported_Functions_Group2 MPCWM Initialization and
  Configuration functions
...
HAL_GTZC_TZSC_MPCWM_ConfigMemAttributes(uint32_t MemBaseAddress,
  ...
  if (status != HAL_ERROR) {
    ...
    *pregister =
        (reg_value | ((pMPCWM_Desc->Length >> gran_offset) << length_pos));
  } else {
    /* Do nothing */
  }
...
/** @defgroup GTZC_Exported_Functions_Group3 TZSC and TZSC-MPCWM Lock functions
  ...
  *
  ...
  TZSC-MPCWM lock. It includes lock enable, and current value read. TZSC:
  Security Controller MPCWM: Memory Protection Controller WaterMark
  @endverbatim
  * @{
  */
...
/** @defgroup GTZC_Exported_Functions_Group4 TZIC Initialization and
  Configuration functions
...
HAL_StatusTypeDef HAL_GTZC_TZIC_DisableIT(uint32_t PeriphId) {
  ...
  if ((PeriphId & GTZC_PERIPH_ALL) != 0x00U) {
    ...
    /* common case where only one peripheral is configured */
    register_address =
        (uint32_t) & (GTZC_TZIC->IER1) + (4U * GTZC_GET_REG_INDEX(PeriphId));
    CLEAR_BIT(*(uint32_t *)register_address,
...
/**
 * @brief  Enable an interrupt on a single TZIC peripheral or on all
 * peripherals.
 * @param  PeriphId Peripheral identifier.
 *         This parameter can be a value of @ref GTZC_Peripheral_Identification
 * @retval HAL status
...
HAL_StatusTypeDef HAL_GTZC_TZIC_EnableIT(uint32_t PeriphId) {
  ...
  if ((PeriphId & GTZC_PERIPH_ALL) != 0x00U) {
    ...
    register_address =
        (uint32_t) & (GTZC_TZIC->IER1) + (4U * GTZC_GET_REG_INDEX(PeriphId));
    SET_BIT(*(uint32_t *)register_address,
            1UL << GTZC_GET_PERIPH_POS(PeriphId));
  }
 ...
 *         If all peripherals selected, pointer to an array of
 * GTZC_PERIPH_TZIC_MAX elements Element content is either
 * GTZC_TZIC_NO_ILA_EVENT or GTZC_TZIC_ILA_EVENT_PENDING
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_GTZC_TZIC_GetFlag(uint32_t PeriphId, uint32_t *pFlag) {
  ...
  if ((PeriphId & GTZC_PERIPH_ALL) != 0x00U) {
    ...
    register_address =
        (uint32_t) & (GTZC_TZIC->MISR1) + (4U * GTZC_GET_REG_INDEX(PeriphId));
    *pFlag = READ_BIT(*(uint32_t *)register_address,
                      1UL << GTZC_GET_PERIPH_POS(PeriphId)) >>
...
/**
 * @brief  Clear interrupt flag on a single TZIC peripheral or on all
 * peripherals.
 * @param  PeriphId Peripheral identifier.
 *         This parameter can be a value of @ref GTZC_Peripheral_Identification
 * @retval HAL status
...
HAL_StatusTypeDef HAL_GTZC_TZIC_ClearFlag(uint32_t PeriphId) {
  ...
  if ((PeriphId & GTZC_PERIPH_ALL) != 0x00U) {
    ...
    /* common case where only one peripheral is configured */
    register_address =
        (uint32_t) & (GTZC_TZIC->ICR1) + (4U * GTZC_GET_REG_INDEX(PeriphId));
    SET_BIT(*(uint32_t *)register_address,
            1UL << GTZC_GET_PERIPH_POS(PeriphId));
  }
