...
#if defined(CORE_CM0PLUS)
        /* secure configuration. Make sure not to configure SUBGHZ SPI which is securable by option
         * byte. */
 ...
 * stm32wlxx_hal_gtzc.h). If PeriphId target a single peripheral, pointer on a single element. If
 * all peripherals selected, pointer to an array of GTZC_PERIPH_TZSC_MAX elements
 * @retval HAL status
 */
...
HAL_StatusTypeDef HAL_GTZC_TZIC_GetFlag(uint32_t PeriphId, uint32_t *pFlag)
{
    ...
    else
    {
        ...
        *pFlag = READ_BIT(*(uint32_t *)register_address, 1UL << GTZC_GET_PERIPH_POS(PeriphId))
