...
#define IS_LL_UTILS_SYSCLK_DIV(__VALUE__)                                                          \
    (((__VALUE__) == LL_RCC_SYSCLK_DIV_1) || ((__VALUE__) == LL_RCC_SYSCLK_DIV_2)                  \
     || ((__VALUE__) == LL_RCC_SYSCLK_DIV_3) || ((__VALUE__) == LL_RCC_SYSCLK_DIV_4)               \
     || ((__VALUE__) == LL_RCC_SYSCLK_DIV_5) || ((__VALUE__) == LL_RCC_SYSCLK_DIV_6)               \
     || ((__VALUE__) == LL_RCC_SYSCLK_DIV_8) || ((__VALUE__) == LL_RCC_SYSCLK_DIV_10)              \
     || ((__VALUE__) == LL_RCC_SYSCLK_DIV_16) || ((__VALUE__) == LL_RCC_SYSCLK_DIV_32)             \
     || ((__VALUE__) == LL_RCC_SYSCLK_DIV_64) || ((__VALUE__) == LL_RCC_SYSCLK_DIV_128)            \
     || ((__VALUE__) == LL_RCC_SYSCLK_DIV_256) || ((__VALUE__) == LL_RCC_SYSCLK_DIV_512))
 ...
 * LL_RCC_GetSystemClocksFreq (HCLK2_Frequency field)
 * @retval None
 */
#else
/**
 ...
 * @param  HCLKFrequency HCLK frequency in Hz
 * @note   HCLK frequency can be calculated thanks to RCC helper macro or function @ref
 * LL_RCC_GetSystemClocksFreq (HCLK1_Frequency field)
 * @retval None
 */
#endif /* CORE_CM0PLUS */
...
ErrorStatus LL_SetFlashLatency(uint32_t HCLK3_Frequency)
{
    ...
    const uint32_t UTILS_CLK_SRC_RANGE_VOS1[]
        = {18000000UL, 36000000UL, UTILS_MAX_FREQUENCY_SCALE1};
 ...
 *         - PLLM: ensure that the VCO input frequency ranges from 2.66 to 16 MHz (PLLVCO_input =
 * MSI frequency / PLLM)
 *         - PLLN: ensure that the VCO output frequency is between 96 and 344 MHz (PLLVCO_output =
 * PLLVCO_input * PLLN)
                                         ...
                                         LL_UTILS_ClkInitTypeDef *UTILS_ClkInitStruct)
{
    ...
    if (UTILS_PLL_IsBusy() == SUCCESS)
    {
        ...
        if (status != ERROR)
        {
            /* Calculate the new PLL output frequency & verify all PLL stages are correct (VCO input
               ranges, VCO output ranges & SYSCLK max) when assert activated */
                                                  ...
                                                  LL_UTILS_ClkInitTypeDef *UTILS_ClkInitStruct)
{
    ...
    hclks_frequency_target
        = __LL_RCC_CALC_HCLK3_FREQ(SYSCLK_Frequency, UTILS_ClkInitStruct->AHB3CLKDivider);

    /* Calculate HCLK3 frequency current */
    sysclk_current
        = (SystemCoreClock
           * AHBPrescTable[(LL_RCC_GetAHBPrescaler() & RCC_CFGR_HPRE) >> RCC_CFGR_HPRE_Pos]);
    ...
    if (status == SUCCESS)
    {
        ...
        LL_SetSystemCoreClock(
            __LL_RCC_CALC_HCLK2_FREQ(SYSCLK_Frequency, UTILS_ClkInitStruct->CPU2CLKDivider));
#else
        LL_SetSystemCoreClock(
            __LL_RCC_CALC_HCLK1_FREQ(SYSCLK_Frequency, UTILS_ClkInitStruct->CPU1CLKDivider));
