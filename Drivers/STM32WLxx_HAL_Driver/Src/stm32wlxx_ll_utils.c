...
#define UTILS_MAX_FREQUENCY_SCALE1                                             \
  48000000U /*!< Maximum frequency for system clock at power scale1, in Hz */
#define UTILS_MAX_FREQUENCY_SCALE2                                             \
  16000000U /*!< Maximum frequency for system clock at power scale2, in Hz */
...
#define UTILS_PLLVCO_OUTPUT_MIN                                                \
  96000000U /*!< Frequency min for PLLVCO output, in Hz  */
...
#define IS_LL_UTILS_PLLM_VALUE(__VALUE__)                                      \
  (((__VALUE__) == LL_RCC_PLLM_DIV_1) || ((__VALUE__) == LL_RCC_PLLM_DIV_2) || \
   ((__VALUE__) == LL_RCC_PLLM_DIV_3) || ((__VALUE__) == LL_RCC_PLLM_DIV_4) || \
   ((__VALUE__) == LL_RCC_PLLM_DIV_5) || ((__VALUE__) == LL_RCC_PLLM_DIV_6) || \
...
#define IS_LL_UTILS_PLLR_VALUE(__VALUE__)                                      \
  (((__VALUE__) == LL_RCC_PLLR_DIV_2) || ((__VALUE__) == LL_RCC_PLLR_DIV_3) || \
   ((__VALUE__) == LL_RCC_PLLR_DIV_4) || ((__VALUE__) == LL_RCC_PLLR_DIV_5) || \
   ((__VALUE__) == LL_RCC_PLLR_DIV_6) || ((__VALUE__) == LL_RCC_PLLR_DIV_7) || \
 ...
 * function @ref LL_RCC_GetSystemClocksFreq (HCLK2_Frequency field)
 * @retval None
 */
#else
/**
...
ErrorStatus LL_SetFlashLatency(uint32_t HCLK3_Frequency) {
  ...
  const uint32_t UTILS_CLK_SRC_RANGE_VOS1[] = {18000000UL, 36000000UL,
  ...
  /* Flash Latency range */
  const uint32_t UTILS_LATENCY_RANGE[] = {
...
/**
 * @brief  This function configures system clock with MSI as clock source of the
 * PLL
 ...
 *         - PLLM: ensure that the VCO input frequency ranges from 2.66 to 16
 * MHz (PLLVCO_input = MSI frequency / PLLM)
 *         - PLLN: ensure that the VCO output frequency is between 96 and 344
...
LL_PLL_ConfigSystemClock_MSI(LL_UTILS_PLLInitTypeDef *UTILS_PLLInitStruct,
  ...
  if (UTILS_PLL_IsBusy() == SUCCESS) {
    ...
    if (status != ERROR) {
      /* Calculate the new PLL output frequency & verify all PLL stages are
         correct (VCO input ranges,
         VCO output ranges & SYSCLK max) when assert activated */
      pllrfreq = UTILS_GetPLLOutputFrequency(
          __LL_RCC_CALC_MSI_FREQ(range_sel, msi_range), UTILS_PLLInitStruct);
    ...
    if (status != ERROR) {
      ...
      LL_RCC_PLL_ConfigDomain_SYS(
          LL_RCC_PLLSOURCE_MSI, UTILS_PLLInitStruct->PLLM,
          UTILS_PLLInitStruct->PLLN, UTILS_PLLInitStruct->PLLR);

      /* Enable PLL and switch system clock to PLL - latency check done
       * internally */
 ...
 *         - PLLM: ensure that the VCO input frequency ranges from 2.66 to 16
 * MHz (PLLVCO_input = HSI frequency / PLLM)
 *         - PLLN: ensure that the VCO output frequency is between 96 and 344
 * MHz (PLLVCO_output = PLLVCO_input * PLLN)
 *         - PLLR: ensure that max frequency at 48000000 Hz is reach
 * (PLLVCO_output / PLLR)
 * @param  UTILS_PLLInitStruct pointer to a @ref LL_UTILS_PLLInitTypeDef
 * structure that contains the configuration information for the PLL.
 * @param  UTILS_ClkInitStruct pointer to a @ref LL_UTILS_ClkInitTypeDef
...
#if defined(DUAL_CORE)
    hclk2freq =
        __LL_RCC_CALC_HCLK2_FREQ(pllrfreq, UTILS_ClkInitStruct->CPU2CLKDivider);
 ...
 */
ErrorStatus
LL_PLL_ConfigSystemClock_HSE(LL_UTILS_PLLInitTypeDef *UTILS_PLLInitStruct,
                             LL_UTILS_ClkInitTypeDef *UTILS_ClkInitStruct) {
  ErrorStatus status = SUCCESS;
  uint32_t pllrfreq;
#if defined(DUAL_CORE)
  ...
  if (UTILS_PLL_IsBusy() == SUCCESS) {
    ...
    if (LL_RCC_HSE_IsEnabledDiv2() != 1UL) {
      ...
      /* HSE Pre is set */
      pllrfreq =
          UTILS_GetPLLOutputFrequency(HSE_VALUE / 2UL, UTILS_PLLInitStruct);
    }
...
UTILS_GetPLLOutputFrequency(uint32_t PLL_InputFrequency,
  ...
  /*  - PLLR: ensure that max frequency at 48000000 Hz is reached */
  pllfreq =
      pllfreq / ((UTILS_PLLInitStruct->PLLR >> RCC_PLLCFGR_PLLR_Pos) + 1U);
...
UTILS_EnablePLLAndSwitchSystem(uint32_t SYSCLK_Frequency,
  ...
  hclks_frequency_target = __LL_RCC_CALC_HCLK3_FREQ(
      SYSCLK_Frequency, UTILS_ClkInitStruct->AHB3CLKDivider);

  /* Calculate HCLK3 frequency current */
  sysclk_current = (SystemCoreClock *
                    AHBPrescTable[(LL_RCC_GetAHBPrescaler() & RCC_CFGR_HPRE) >>
                                  RCC_CFGR_HPRE_Pos]);
  hclks_frequency_current =
      __LL_RCC_CALC_HCLK3_FREQ(sysclk_current, LL_RCC_GetAHB3Prescaler());
  ...
  if (status == SUCCESS) {
    ...
    LL_SetSystemCoreClock(__LL_RCC_CALC_HCLK2_FREQ(
        SYSCLK_Frequency, UTILS_ClkInitStruct->CPU2CLKDivider));
#else
    LL_SetSystemCoreClock(__LL_RCC_CALC_HCLK1_FREQ(
        SYSCLK_Frequency, UTILS_ClkInitStruct->CPU1CLKDivider));
