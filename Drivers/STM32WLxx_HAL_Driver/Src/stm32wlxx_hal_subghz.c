/*******************************************************************************
 ...
 [..]
    ...
    (#) Initialize the SUBGHZ low level resources by implementing the @ref
HAL_SUBGHZ_MspInit() API:
        (##) PWR configuration
    ...
    (#) Initialize the SUBGHZ handle and SUBGHZSPI SPI registers by calling the
@ref HAL_SUBGHZ_Init(&hUserSubghz), configures also the low level Hardware
(GPIO, CLOCK, NVIC...etc) by calling the customized @ref HAL_SUBGHZ_MspInit()
API.

    (#) For SUBGHZ IO operations, polling operation modes is available within
this driver :

    *** Polling mode IO operation      ***
    =====================================
    [..]
      (+) Set and execute a command in blocking mode using @ref
HAL_SUBGHZ_ExecSetCmd()
      (+) Get a status blocking mode using @ref HAL_SUBGHZ_ExecGetCmd()
      (+) Write a Data Buffer in blocking mode using @ref
HAL_SUBGHZ_WriteBuffer()
      (+) Read a Data Buffer  in blocking mode using @ref
HAL_SUBGHZ_ReadBuffer()
      (+) Write Registers (more than 1 byte) in blocking mode using @ref
HAL_SUBGHZ_WriteRegisters()
      (+) Read Registers (more than 1 byte) in blocking mode using @ref
HAL_SUBGHZ_ReadRegisters()
      (+) Write Register (1 byte) in blocking mode using @ref
HAL_SUBGHZ_WriteRegister()
      (+) Read Register (1 byte) in blocking mode using @ref
HAL_SUBGHZ_ReadRegister()
    ...
    *** SUBGHZ HAL driver macros list ***
    ...
    [..]
     ...
     Use Functions @ref HAL_SUBGHZ_RegisterCallback() or @ref
HAL_SUBGHZ_RegisterAddrCallback() to register an interrupt callback.

    [..]
     Function @ref HAL_SUBGHZ_RegisterCallback() allows to register following
callbacks:
       ...
       (+) LrFhssHopCallback        : callback for LoRa Frequency Hopping Spread
Spectrum Hopping. This function takes as parameters the HAL peripheral handle,
the Callback ID and a pointer to the user callback function.
    ...
    [..]
     Use function @ref HAL_SUBGHZ_UnRegisterCallback to reset a callback to the
default weak function.
     @ref HAL_SUBGHZ_UnRegisterCallback takes as parameters the HAL peripheral
...
Spectrum Hopping.
    ...
    [..]
     MspInit and MspDeInit functions are reset to the legacy weak functions in
the
     @ref HAL_SUBGHZ_Init()/ @ref HAL_SUBGHZ_DeInit() only when these callbacks
are null (not registered beforehand). If MspInit or MspDeInit are not null, the
@ref HAL_SUBGHZ_Init()/ @ref HAL_SUBGHZ_DeInit() keep and use the user
...
Init/DeInit. Then, the user first registers the MspInit/MspDeInit user callbacks
     using @ref HAL_SUBGHZ_RegisterCallback() before calling @ref
HAL_SUBGHZ_DeInit() or @ref HAL_SUBGHZ_Init() function.
...
callbacks are set to the corresponding weak functions. #endif
  */
...
/** @defgroup SUBGHZ_Exported_Functions_Group1 Initialization and
...
@verbatim
 ...
 ===============================================================================
    [..]  This subsection provides a set of functions allowing to initialize and
      ...
      (+) User must implement HAL_SUBGHZ_MspInit() function in which he
configures all related peripherals resources (CLOCK, GPIO, IT and NVIC ).
      ...
      (+) Call the function HAL_SUBGHZ_DeInit() to restore the default
configuration of the SUBGHZ peripheral.
 ...
 *         set the state to HAL_SUBGHZ_STATE_RESET_RF_READY with
 * __HAL_SUBGHZ_RESET_HANDLE_STATE_RF_READY to avoid the reset of Radio
 * peripheral.
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SUBGHZ_Init(SUBGHZ_HandleTypeDef *hsubghz) {
  ...
  assert_param(
      IS_SUBGHZSPI_BAUDRATE_PRESCALER(hsubghz->Init.BaudratePrescaler));
...
HAL_StatusTypeDef HAL_SUBGHZ_DeInit(SUBGHZ_HandleTypeDef *hsubghz) {
  ...
  if (hsubghz->MspDeInitCallback == NULL) {
    hsubghz->MspDeInitCallback =
        HAL_SUBGHZ_MspDeInit; /* Legacy weak MspDeInit  */
  }
 ...
 *         To be used instead of the weak HAL_SUBGHZ_AddrCallback() predefined
 * callback
 * @param  hsubghz Pointer to a SUBGHZ_HandleTypeDef structure that contains
 *                the configuration information for the specified SUBGHZ.
...
HAL_SUBGHZ_UnRegisterCadStatusCallback(SUBGHZ_HandleTypeDef *hsubghz) {
  ...
  if (HAL_SUBGHZ_STATE_READY == hsubghz->State) {
    hsubghz->CADStatusCallback =
        HAL_SUBGHZ_CADStatusCallback; /* Legacy weak AddrCallback */
  } else {
    /* Update the error code */
    hsubghz->ErrorCode = HAL_SUBGHZ_ERROR_INVALID_CALLBACK;
...
@verbatim
 ...
 [..]
    ...
    (#) There are four modes of transfer:
       (++) Set operation: The Set Command operation is performed in polling
mode. The HAL status of command processing is returned by the same function
            after finishing transfer.
       (++) Get operation: The Get Status operation is performed using polling
mode These API update buffer in parameter to retrieve status of command. These
API return the HAL status
...
void HAL_SUBGHZ_IRQHandler(SUBGHZ_HandleTypeDef *hsubghz) {
  ...
  if (SUBGHZ_CHECK_IT_SOURCE(itsource, SUBGHZ_IT_CAD_DONE) != RESET) {
    ...
    if (SUBGHZ_CHECK_IT_SOURCE(itsource, SUBGHZ_IT_CAD_ACTIVITY_DETECTED) !=
        RESET) {
...
__weak void HAL_SUBGHZ_PreambleDetectedCallback(SUBGHZ_HandleTypeDef *hsubghz) {
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_SUBGHZ_PreambleDetectedCallback should be implemented in the
     user file
   */
}
...
__weak void HAL_SUBGHZ_SyncWordValidCallback(SUBGHZ_HandleTypeDef *hsubghz) {
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_SUBGHZ_SyncWordValidCallback should be implemented in the
     user file
   */
}
...
__weak void HAL_SUBGHZ_HeaderValidCallback(SUBGHZ_HandleTypeDef *hsubghz) {
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_SUBGHZ_HeaderValidCallback should be implemented in the user
     file
   */
}
...
__weak void HAL_SUBGHZ_HeaderErrorCallback(SUBGHZ_HandleTypeDef *hsubghz) {
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_SUBGHZ_HeaderErrorCallback should be implemented in the user
     file
   */
}
...
__weak void HAL_SUBGHZ_CRCErrorCallback(SUBGHZ_HandleTypeDef *hsubghz) {
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_SUBGHZ_CRCErrorCallback should be implemented in the user
     file
   */
}
 ...
 */
__weak void
HAL_SUBGHZ_CADStatusCallback(SUBGHZ_HandleTypeDef *hsubghz,
  ...
  UNUSED(cadstatus);
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_SUBGHZ_CADStatusCallback should be implemented in the user
     file
   */
}
...
__weak void HAL_SUBGHZ_RxTxTimeoutCallback(SUBGHZ_HandleTypeDef *hsubghz) {
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_SUBGHZ_RxTxTimeoutCallback should be implemented in the user
     file
   */
}
...
__weak void HAL_SUBGHZ_LrFhssHopCallback(SUBGHZ_HandleTypeDef *hsubghz) {
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_SUBGHZ_LrFhssHopCallback should be implemented in the user
     file
   */
}
/**
...
/** @defgroup SUBGHZ_Exported_Functions_Group3 Peripheral State and Errors
functions
...
@verbatim
 ...
 ===============================================================================
    ...
    This subsection provides a set of functions allowing to control the SUBGHZ.
     (+) HAL_SUBGHZ_GetState() API can be helpful to check in run-time the state
of the SUBGHZ peripheral
     (+) HAL_SUBGHZ_GetError() check in run-time Errors occurring during
communication
@endverbatim
  * @{
  */
...
void SUBGHZSPI_Init(uint32_t BaudratePrescaler) {
  ...
  WRITE_REG(SUBGHZSPI->CR1,
            (SPI_CR1_MSTR | SPI_CR1_SSI | BaudratePrescaler | SPI_CR1_SSM));
