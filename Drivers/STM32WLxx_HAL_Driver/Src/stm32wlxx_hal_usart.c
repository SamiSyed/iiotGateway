...
#define USART_CR2_FIELDS                                                                           \
    ((uint32_t)(USART_CR2_CPHA | USART_CR2_CPOL | USART_CR2_CLKEN | USART_CR2_LBCL                 \
                | USART_CR2_STOP | USART_CR2_SLVEN | USART_CR2_DIS_NSS)) /*!< USART CR2 fields of  \
                                                                            parameters set by      \
                                                                            USART_SetConfig API */

#define USART_CR3_FIELDS                                                                           \
    ((uint32_t)(USART_CR3_TXFTCFG | USART_CR3_RXFTCFG)) /*!< USART or USART CR3 fields of          \
                                                           parameters set by USART_SetConfig API   \
                                                         */
                                               ...
                                               HAL_USART_CallbackIDTypeDef CallbackID)
{
    ...
    if (HAL_USART_STATE_READY == husart->State)
    {
        ...
        {
        case HAL_USART_TX_HALFCOMPLETE_CB_ID:
            husart->TxHalfCpltCallback = HAL_USART_TxHalfCpltCallback; /* Legacy weak
                                                                          TxHalfCpltCallback  */
            break;
        ...
        case HAL_USART_RX_HALFCOMPLETE_CB_ID:
            husart->RxHalfCpltCallback = HAL_USART_RxHalfCpltCallback; /* Legacy weak
                                                                          RxHalfCpltCallback   */
            break;
        ...
        case HAL_USART_RX_FIFO_FULL_CB_ID:
            husart->RxFifoFullCallback = HAL_USARTEx_RxFifoFullCallback; /* Legacy weak
                                                                            RxFifoFullCallback   */
            break;
 ...
 * @note   Dual core specific: there is no support for unaligned accesses on the Cortex-M0+
 * processor. When USART parity is not enabled (PCE = 0), and Word Length is configured to 9 bits
 * (M1-M0 = 01), address of user data buffer containing data to be sent, should be aligned on a half
 * word frontier (16 bits) (as sent data will be handled using u16 pointer cast). Depending on
 * compilation chain, use of specific alignment compilation directives or pragmas might be required
 ...
 * @note   Dual core specific: there is no support for unaligned accesses on the Cortex-M0+
 * processor. When USART parity is not enabled (PCE = 0), and Word Length is configured to 9 bits
 * (M1-M0 = 01), address of user data buffer for storing data to be received, should be aligned on a
 * half word frontier (16 bits) (as received data will be handled using u16 pointer cast). Depending
 * on compilation chain, use of specific alignment compilation directives or pragmas might be
 * required to ensure proper alignment for pRxData.
 * @param husart USART handle.
 ...
 * @note   Dual core specific: there is no support for unaligned accesses on the Cortex-M0+
 * processor. When USART parity is not enabled (PCE = 0), and Word Length is configured to 9 bits
 * (M1-M0 = 01), address of user data buffers containing data to be sent/received, should be aligned
 * on a half word frontier (16 bits) (as sent/received data will be handled using u16 pointer cast).
 * Depending on compilation chain, use of specific alignment compilation directives or pragmas might
 * be required to ensure proper alignment for pTxData and pRxData.
 * @param  husart USART handle.
 ...
 * processor. When USART parity is not enabled (PCE = 0), and Word Length is configured to 9 bits
 * (M1-M0 = 01), address of user data buffer containing data to be sent, should be aligned on a half
 * word frontier (16 bits) (as sent data will be handled by DMA from halfword frontier). Depending
 * on compilation chain, use of specific alignment compilation directives or pragmas might be
 ...
 * processor. When USART parity is not enabled (PCE = 0), and Word Length is configured to 9 bits
 * (M1-M0 = 01), address of user data buffer for storing data to be received, should be aligned on a
 * half word frontier (16 bits)(as received data will be handled by DMA from halfword frontier).
 * Depending on compilation chain, use of specific alignment compilation directives or pragmas might
 * be required to ensure proper alignment for pRxData.
 * @param  husart USART handle.
 * @param  pRxData pointer to data buffer (u8 or u16 data elements).
 * @param  Size amount of data elements (u8 or u16) to be received.
 * @retval HAL status
 ...
 * processor. When USART parity is not enabled (PCE = 0), and Word Length is configured to 9 bits
 * (M1-M0 = 01), address of user data buffers containing data to be sent/received, should be aligned
 * on a half word frontier (16 bits)(as sent/received data will be handled by DMA from halfword
 * frontier). Depending on compilation chain, use of specific alignment compilation directives or
 * pragmas might be required to ensure proper alignment for pTxData and pRxData.
 * @param  husart USART handle.
 * @param  pTxData pointer to TX data buffer (u8 or u16 data elements).
 * @param  pRxData pointer to RX data buffer (u8 or u16 data elements).
...
#if defined(CORE_CM0PLUS)
        /* In case of 9bits/No Parity transfer, pTxData and pRxData buffers provided as input
           parameter should be aligned on a u16 frontier, as data copy to/from TDR/RDR will be
           handled by DMA from a u16 frontier. */
        if ((husart->Init.WordLength == USART_WORDLENGTH_9B)
            && (husart->Init.Parity == USART_PARITY_NONE))
        {
...
HAL_StatusTypeDef HAL_USART_Abort_IT(USART_HandleTypeDef *husart)
{
    ...
    if (abortcplt == 1U)
    {
        ...
        /* Clear the Error flags in the ICR register */
        __HAL_USART_CLEAR_FLAG(husart,
                               USART_CLEAR_OREF | USART_CLEAR_NEF | USART_CLEAR_PEF
                                   | USART_CLEAR_FEF);
...
void USART_InitCallbacksToDefault(USART_HandleTypeDef *husart)
{
    ...
    husart->RxFifoFullCallback = HAL_USARTEx_RxFifoFullCallback; /* Legacy weak RxFifoFullCallback
                                                                  */
    husart->TxFifoEmptyCallback = HAL_USARTEx_TxFifoEmptyCallback; /* Legacy weak
                                                                      TxFifoEmptyCallback       */
}
#endif /* USE_HAL_USART_REGISTER_CALLBACKS */
...
static HAL_StatusTypeDef USART_SetConfig(USART_HandleTypeDef *husart)
{
    ...
    case USART_CLOCKSOURCE_PCLK1:
        ...
        usartdiv = (uint32_t)(USART_DIV_SAMPLING8(pclk,
    ...
    case USART_CLOCKSOURCE_HSI:
        usartdiv = (uint32_t)(USART_DIV_SAMPLING8(HSI_VALUE,
                                                  husart->Init.BaudRate,
                                                  husart->Init.ClockPrescaler));
        break;
    ...
    case USART_CLOCKSOURCE_LSE:
        usartdiv = (uint32_t)(USART_DIV_SAMPLING8(LSE_VALUE,
                                                  husart->Init.BaudRate,
                                                  husart->Init.ClockPrescaler));
        break;
    default:
