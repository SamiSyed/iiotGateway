...
/** @defgroup RCC_Exported_Functions_Group1 Initialization and de-initialization functions
 ...
 ===============================================================================
    [..]
      ...
      (HSE, HSI, LSE, MSI, LSI, PLL, CSS and MCO) and the System buses clocks (SYSCLK, HCLK1, HCLK2,
 HCLK3, PCLK1 and PCLK2).
 ...
 48MHZ. The number of flash wait states is automatically adjusted when MSI range is updated with
         ...
         (+) PLL (clocked by HSI, HSE or MSI) providing up to three independent output clocks:
           ...
           (++) The second output is used to generate the clock for I2S2 and the random analog
 generator (<=48 MHz)
 ...
 modes an interrupt can be sent to wakeup the software
         ...
         (+) MCO (microcontroller clock output): used to output MSI, LSI, HSI, LSE, HSE (before and
             after stabilization), SYSCLK, or main PLL clocks (through a configurable prescaler) on
 PA8 pin.
    ...
    [..] System, AHB and APB buses clocks configuration
         ...
         -@- All the peripheral clocks are derived from the System clock (SYSCLK) except:
           ...
           (+@) RTC: the RTC clock can be derived either from the LSI, LSE or HSE clock divided
 by 32. You have to use __HAL_RCC_RTC_ENABLE() and HAL_RCCEx_PeriphCLKConfig() function to configure
 this clock.
...
HAL_StatusTypeDef HAL_RCC_DeInit(void)
{
    ...
    CLEAR_BIT(RCC->CR,
              RCC_CR_HSION | RCC_CR_HSIKERON | RCC_CR_HSIASFS | RCC_CR_HSEON | RCC_CR_HSEPRE
                  | RCC_CR_PLLON);
        ...
        if (sysclk_source != RCC_SYSCLKSOURCE_STATUS_PLLCLK)
        {
            ...
            else
            {
                ...
                CLEAR_BIT(RCC->PLLCFGR,
                          (RCC_PLLCFGR_PLLSRC | RCC_PLLCFGR_PLLPEN | RCC_PLLCFGR_PLLQEN
                           | RCC_PLLCFGR_PLLREN));
            }
        }
        else
...
uint32_t HAL_RCC_GetHCLK3Freq(void)
{
    ...
    return (
        (uint32_t)(__LL_RCC_CALC_HCLK3_FREQ(HAL_RCC_GetSysClockFreq(), LL_RCC_GetAHB3Prescaler())));
}
