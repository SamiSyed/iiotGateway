  ...
  FLASH.
    ...
    [..]
      ...
      (+) Configure the clock source(s) for peripherals which clocks are not
          derived from the System clock (RTC, ADC, RNG, I2S2, USARTx, LPUART1,
  LPTIMx, I2Cx)
...
#define RCC_PLLCFR_RESET_VALUE                                                 \
  (RCC_PLLCFGR_PLLR_0 | RCC_PLLCFGR_PLLQ_0 | RCC_PLLCFGR_PLLP_1 |              \
   RCC_PLLCFGR_PLLN_0)
...
/** @defgroup RCC_Exported_Functions_Group1 Initialization and de-initialization
 ...
 buses clocks (SYSCLK, HCLK1, HCLK2, HCLK3, PCLK1 and PCLK2).
 ...
 from 100KHZ to 48MHZ. The number of flash wait states is automatically adjusted
 when MSI range is updated with HAL_RCC_OscConfig() and the MSI is used as
 System clock source.
 ...
 or through the PLL as System clock source. It is used also as RF clock source
             Can be used also optionally as RTC clock source.

         (+) LSE (low-speed external): 32.768 KHz oscillator used optionally to
 drive RTC used for Auto-wakeup from Stop and Standby modes, or the real-time
 clock (RTCCLK).
 ...
 (up to 48MHz).
           (++) The second output is used to generate the clock for I2S2 and the
 random analog generator (<=48 MHz)
 ...
 LSE, HSE (before and after stabilization), SYSCLK, or main PLL clocks (through
 ...
 System clock through configurable prescaler and used to clock the CPU1, memory
 ...
 these clocks. The AHB3 clock (HCLK3) is derived from System clock through
 configurable prescaler and used to clock the FLASH. APB3 (PCLK3) is derived
 from AHB3 clock.
 ...
 HAL_RCCEx_PeriphCLKConfig() function to configure this clock.
         ...
         (+) The maximum frequency of the SYSCLK, HCLK1, HCLK2, HCLK3, PCLK1 and
 PCLK2 is 48 MHz. The clock source frequency should be adapted depending on the
...
HAL_StatusTypeDef HAL_RCC_OscConfig(RCC_OscInitTypeDef *RCC_OscInitStruct) {
  ...
  if (((RCC_OscInitStruct->OscillatorType) & RCC_OSCILLATORTYPE_MSI) ==
    ...
    assert_param(
        IS_RCC_MSI_CALIBRATION_VALUE(RCC_OscInitStruct->MSICalibrationValue));
    ...
    if ((sysclk_source == RCC_SYSCLKSOURCE_STATUS_MSI) ||
      ...
      /* Otherwise, just the calibration and MSI range change are allowed */
        ...
        /* To correctly read data from FLASH memory, the number of wait states
           ...
           the AHB3 clock and the supply voltage of the device. */
 ...
 *         wake-up from Reset, wake-up from STANDBY mode. After restart from
...
HAL_StatusTypeDef HAL_RCC_ClockConfig(RCC_ClkInitTypeDef *RCC_ClkInitStruct,
  ...
  /* To correctly read data from FLASH memory, the number of wait states
    ...
    FLASH clock (HCLK3) and the supply voltage of the device. */
  ...
  /*-------------------------- HCLK3 Configuration ---------------------------*/
  if (((RCC_ClkInitStruct->ClockType) & RCC_CLOCKTYPE_HCLK3) ==
      RCC_CLOCKTYPE_HCLK3) {
 ...
 * factors.
 * @note     (*) HSI_VALUE is a constant defined in stm32wlxx_hal_conf.h file
 * (default value 16 MHz) but the real value may vary depending on the
 * variations in voltage and temperature.
 * @note     (**) HSE_VALUE is a constant defined in stm32wlxx_hal_conf.h file
...
uint32_t HAL_RCC_GetHCLK3Freq(void) {
  ...
  return ((uint32_t)(__LL_RCC_CALC_HCLK3_FREQ(HAL_RCC_GetSysClockFreq(),
...
static HAL_StatusTypeDef RCC_SetFlashLatencyFromMSIRange(uint32_t MSI_Range) {
  ...
  flash_clksrcfreq =
      __LL_RCC_CALC_HCLK3_FREQ(msifreq, LL_RCC_GetAHB3Prescaler());
