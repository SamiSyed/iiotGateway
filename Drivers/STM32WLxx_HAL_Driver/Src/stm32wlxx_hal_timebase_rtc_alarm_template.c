/**
  ...
  *           + Initializes the RTC peripheral to increment the seconds
 registers
  *             each 1s
 ...
 *         Calendar time base is = ((RTC_ASYNCH_PREDIV + 1) * (RTC_SYNCH_PREDIV
 * + 1)) / RTC_CLOCK = 1s Alarm interrupt timebase is = (RTC_SYNCH_PREDIV /
 * (1000 / uwTickFreq)) = 1 ms when uwTickFreq is set to 1 kHz
...
HAL_StatusTypeDef HAL_InitTick(uint32_t TickPriority) {
  ...
  if ((uint32_t)uwTickFreq != 0U) {
    ...
    if ((PeriphClkInitStruct.RTCClockSelection == RCC_RTCCLKSOURCE_LSI) &&
        (__HAL_RCC_GET_FLAG(RCC_FLAG_LSIRDY) != 0x00u))
#elif defined(RTC_CLOCK_SOURCE_HSE)
...
#error Please select the RTC Clock source
    ...
    if (status == HAL_OK) {
      /* The time base is defined to have highest synchronous prescaler but
         keeping a 1Hz RTC frequency. */
    ...
    if ((status == HAL_OK) &&
        (__HAL_RTC_GET_FLAG(&hRTC_Handle, RTC_FLAG_INITS) == 0x00u)) {
      time.Hours = 0x00u;
      time.Minutes = 0x00u;
      time.Seconds = 0x00u;
    ...
    if (status == HAL_OK) {
      ...
      alarm.AlarmTime.SubSeconds =
          (RTC_SYNCH_PREDIV / (1000 / (uint32_t)uwTickFreq));
...
#else
        /* When RTCCLK is around 32 kHz, need to mask Subsecond register bit 12
          to 11 to have 10 Hhz interrupt */
 ...
 *         HAL_RTC_AlarmIRQHandler(). It makes a direct call to HAL_IncTick() to
 * increment a global variable "uwTick" used as application time base.
 * @param  hrtc : RTC handle
 * @retval None
 */
void HAL_RTC_AlarmAEventCallback(RTC_HandleTypeDef *hrtc) { HAL_IncTick(); }
