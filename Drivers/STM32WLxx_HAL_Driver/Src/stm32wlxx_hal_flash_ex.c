...
static void FLASH_OB_ConfigSecureMode(uint32_t SecureMode,
                                      uint32_t *Reg,
                                      uint32_t Bit,
                                      uint32_t ValueEnable);
static uint32_t FLASH_OB_GetSecureMode(uint32_t Reg,
                                       uint32_t Bit,
                                       uint32_t ValueEnable,
...
HAL_StatusTypeDef HAL_FLASHEx_OBProgram(FLASH_OBProgramInitTypeDef *pOBInit)
{
    ...
    if ((pOBInit->OptionType
         & (OPTIONBYTE_SECURE_MODE | OPTIONBYTE_C2_BOOT_VECT | OPTIONBYTE_C2_DEBUG_ACCESS
            | OPTIONBYTE_SUBGHZSPI_SECURE_ACCESS))
        != 0U)
    {
        /* Set the secure flash, SRAM memory start address, CPU2 boot reset vector or CPU2 debug
         * access */
...
void HAL_FLASHEx_OBGetConfig(FLASH_OBProgramInitTypeDef *pOBInit)
{
    ...
    /* Get the Secure Flash start address, Secure Hide Protection start address Secure Backup SRAM2
     * start address, Secure non-Backup SRAM1 start address and the Security Mode, */
    FLASH_OB_GetSecureMemoryConfig(&(pOBInit->SecureFlashStartAddr),
                                   &(pOBInit->HideProtectionStartAddr),
                                   &(pOBInit->SecureSRAM2StartAddr),
                                   &(pOBInit->SecureSRAM1StartAddr),
                                   &(pOBInit->SecureMode));
 ...
 *         When FLASH_SFR_HDPAD = 0 (Secure Hide Protection area enabled) the CPU2 software debug is
 * disabled after a system reset. When FLASH_SFR_HDPAD = 1 (Secure Hide Protection area disabled)
 * the CPU2 software debug is enabled, after a system reset.
 * @retval None
 */
 ...
 *            @arg @ref FLASH_PRIV_DENIED Access to Flash registers is denied to non-privilege
 * access
 * @retval None
 */
 ...
 *         @arg @ref OB_USER_nBOOT0 or @ref OB_USER_BOOT_LOCK or @ref OB_USER_BOOT_LOCK or @ref
 * OB_USER_ALL
                                   ...
                                   uint32_t PCROP1AEndAddr)
{
    ...
    if ((PCROPConfig & OB_PCROP_ZONE_A) != 0U)
    {
        ...
        startoffset = ((PCROP1AStartAddr - FLASH_BASE) >> FLASH_PCROP_GRANULARITY_OFFSET); /* 2K
                                                                                              pages
                                                                                            */
 ...
 * @note   To validate the PCROP options, the option bytes must be reloaded
 *         through the call of the @ref HAL_FLASH_OB_Launch() function.
 * @param  PCROP1BStartAddr Specifies the Zone 1B Start address of the Proprietary code readout
 * protection This parameter can be an address between begin and end of the flash
...
static void FLASH_OB_SecureConfig(FLASH_OBProgramInitTypeDef *pOBParam)
{
    ...
    if ((pOBParam->OptionType & OPTIONBYTE_SECURE_MODE) != 0U)
    {
        ...
        /* Configure SFR register content with start FLASH PAGE index to secure and start FLASH PAGE
         * index for hide protection area */
        MODIFY_REG(sfr_reg_val,
                   (FLASH_SFR_SFSA | FLASH_SFR_HDPSA),
        ...
        MODIFY_REG(srrvr_reg_val,
                   (FLASH_SRRVR_SBRSA | FLASH_SRRVR_SNBRSA),
        ...
        else /* Enable Secure Area bit per bit */
        {
            ...
            FLASH_OB_ConfigSecureMode(pOBParam->SecureMode,
                                      &sfr_reg_val,
                                      FLASH_SFR_FSD,
    ...
    if ((pOBParam->OptionType & OPTIONBYTE_C2_BOOT_VECT) != 0U)
    {
        ...
        if (pOBParam->C2BootRegion == OB_C2_BOOT_FROM_FLASH)
        {
            MODIFY_REG(
                srrvr_reg_val,
                (FLASH_SRRVR_SBRV | FLASH_SRRVR_C2OPT),
...
/**
 * @brief  Return the Secure Flash start address, Secure Hide Protection Area start address, Secure
 * Backup SRAM2 start address, Secure non-Backup SRAM1 start address and the SecureMode
 * @param  SecureFlashStartAddr  Specifies the address where to copied the Secure Flash start
 * address
 * @param  HideProtectionStartAddr Specifies the address where to copied the Secure Hide Protection
 * Area start address
 * @param  SecureSRAM2StartAddr  Specifies the address where to copied the Secure Backup SRAM2 start
 * address
 * @param  SecureSRAM1StartAddr  Specifies the address where to copied the Secure non-Backup SRAM1
 * start address
 * @param  SecureMode            Specifies the Secure area enabled or disabled.
 *                               This return value can be one of the following values:
 *                               @arg @ref OB_SECURE_SYSTEM_AND_ALL_AREAS_DISABLE : All System
 * Security disabled
 *                               @arg @ref OB_SECURE_SYSTEM_AND_FLASH_ENABLE : Flash Security
 * enabled
 *                               @arg @ref OB_SECURE_HIDE_PROTECTION_ENABLE : Hide Protection
 * Security enabled
 *                               @arg @ref OB_SECURE_SRAM1_ENABLE : SRAM1 Security enabled
 *                               @arg @ref OB_SECURE_SRAM2_ENABLE : SRAM2 Security enabled
 *                               @arg @ref OB_SECURE_SYSTEM_AND_FLASH_DISABLE : Flash Security
 * disabled
 *                               @arg @ref OB_SECURE_HIDE_PROTECTION_DISABLE : Hide Protection
 * Security disabled
 *                               @arg @ref OB_SECURE_SRAM1_DISABLE : SRAM1 Security disabled
 *                               @arg @ref OB_SECURE_SRAM2_DISABLE : SRAM2 Security disabled
 *                               @arg @ref OB_SECURE_SYSTEM_AND_ALL_AREAS_ENABLE : All System
 * Security enabled
 * @retval None
 */
static void FLASH_OB_GetSecureMemoryConfig(uint32_t *SecureFlashStartAddr,
                                           ...
                                           uint32_t *SecureMode)
{
    ...
    *SecureMode = (FLASH_OB_GetSecureMode(sfr_reg_val,
                                          FLASH_SFR_FSD,
                                          OB_SECURE_SYSTEM_AND_FLASH_ENABLE,
                                          OB_SECURE_SYSTEM_AND_FLASH_DISABLE)
                   | FLASH_OB_GetSecureMode(sfr_reg_val,
                                            FLASH_SFR_HDPAD,
                                            OB_SECURE_HIDE_PROTECTION_ENABLE,
                                            OB_SECURE_HIDE_PROTECTION_DISABLE)
                   | FLASH_OB_GetSecureMode(srrvr_reg_val,
                                            FLASH_SRRVR_NBRSD,
                                            OB_SECURE_SRAM1_ENABLE,
