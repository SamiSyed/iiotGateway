/**
 ...
 ==============================================================================
  ...
  [..] Comparing to other previous devices, the FLASH interface for STM32WLxx
       ...
       (+) Capacity up to 256kB with single bank architecture supporting
 ...
 HAL_FLASHEx_FlashEmptyCheck(). and modify this setting (for flash loader
 purpose e.g.) using HAL_FLASHEx_ForceFlashEmpty().

      (#) Enable, Disable CPU2 debug access using HAL_FLASHEx_EnableC2Debug() or
          HAL_FLASHEx_DisableC2Debug()

      (#) Enable Secure Hide Protection area access using
 HAL_FLASHEx_EnableSecHideProtection()
         (++) Enable Secure Hide Protection area access done by hardware on a
 system reset
 ...
 HAL_FLASHEx_GetPrivMode()
        (++) Returns if the FLASH registers are protected against non-privilege
 accesses
...
static void FLASH_OB_ConfigSecureMode(uint32_t SecureMode, uint32_t *Reg,
                                      uint32_t Bit, uint32_t ValueEnable);
static uint32_t FLASH_OB_GetSecureMode(uint32_t Reg, uint32_t Bit,
 ...
 * @note   Before any operation, it is possible to check there is no operation
 * suspended by call HAL_FLASHEx_IsOperationSuspended()
...
HAL_StatusTypeDef HAL_FLASHEx_Erase(FLASH_EraseInitTypeDef *pEraseInit,
  ...
  if (status == HAL_OK) {
    if (pEraseInit->TypeErase == FLASH_TYPEERASE_MASSERASE) {
      ...
      /* If operation is completed or interrupted, no need to clear the Mass
       * Erase Bit */
    } else {
...
HAL_StatusTypeDef HAL_FLASHEx_OBProgram(FLASH_OBProgramInitTypeDef *pOBInit) {
  ...
  if ((pOBInit->OptionType & (OPTIONBYTE_SECURE_MODE | OPTIONBYTE_C2_BOOT_VECT |
                              OPTIONBYTE_C2_DEBUG_ACCESS |
                              OPTIONBYTE_SUBGHZSPI_SECURE_ACCESS)) != 0U) {
    /* Set the secure flash, SRAM memory start address, CPU2 boot reset vector
     * or CPU2 debug access */
 ...
 * @param  pOBInit Pointer to an @ref FLASH_OBProgramInitTypeDef structure that
 * contains the configuration information. The fields pOBInit->WRPArea and
...
void HAL_FLASHEx_OBGetConfig(FLASH_OBProgramInitTypeDef *pOBInit) {
  ...
  /* Get the Secure Flash start address, Secure Hide Protection start address
   * Secure Backup SRAM2 start address, Secure non-Backup SRAM1 start address
   * and the Security Mode, */
  FLASH_OB_GetSecureMemoryConfig(
      &(pOBInit->SecureFlashStartAddr), &(pOBInit->HideProtectionStartAddr),
      &(pOBInit->SecureSRAM2StartAddr), &(pOBInit->SecureSRAM1StartAddr),
      &(pOBInit->SecureMode));

  /* Get the M0+ Secure Boot reset vector address and Secure Boot memory
   * selection */
 ...
 * allowed until one of this bit or the same bit in Flash CPU2 access control
 * register (FLASH_C2ACR) is set. The PESD bit in both the Flash status register
 * (FLASH_SR) and Flash CPU2 status register (FLASH_C2SR) register will be
 * cleared when both PES bit in FLASH_ACR and FLASH_C2ACR are cleared.
 * @retval Status
 ...
 *         When FLASH_SFR_HDPAD = 0 (Secure Hide Protection area enabled) the
 * CPU2 software debug is disabled after a system reset. When FLASH_SFR_HDPAD =
 * 1 (Secure Hide Protection area disabled) the CPU2 software debug is enabled,
 ...
 * @note   This feature can only be accessed by a secure privileged CPU2
 *         This bit is set by software and will only be reset by hardware on a
 * system reset.
 * @retval None
 */
 ...
 *            @arg @ref FLASH_PRIV_DENIED Access to Flash registers is denied to
...
#ifdef CORE_CM0PLUS
  MODIFY_REG(FLASH->C2CR, FLASH_CR_PNB,
 ...
 * number of pages in the Flash - 1)
 * @retval None
 */
static void FLASH_OB_WRPConfig(uint32_t WRPArea, uint32_t WRPStartOffset,
                               uint32_t WRDPEndOffset) {
  ...
  if (WRPArea == OB_WRPAREA_BANK1_AREAA) {
    MODIFY_REG(
        FLASH->WRP1AR, (FLASH_WRP1AR_WRP1A_STRT | FLASH_WRP1AR_WRP1A_END),
        (WRPStartOffset | (WRDPEndOffset << FLASH_WRP1AR_WRP1A_END_Pos)));
  } else /* OB_WRPAREA_BANK1_AREAB */
  {
    MODIFY_REG(
        FLASH->WRP1BR, (FLASH_WRP1BR_WRP1B_STRT | FLASH_WRP1BR_WRP1B_END),
        (WRPStartOffset | (WRDPEndOffset << FLASH_WRP1AR_WRP1A_END_Pos)));
  }
}
 ...
 *         @arg @ref OB_USER_BOR_LEV or @ref OB_USER_nRST_STOP or @ref
 * OB_USER_nRST_STDBY or
 *         @arg @ref OB_USER_nRST_SHDW or @ref OB_USER_IWDG_SW or @ref
 * OB_USER_IWDG_STOP or
 *         @arg @ref OB_USER_IWDG_STDBY or @ref OB_USER_WWDG_SW or @ref
 * OB_USER_nBOOT1 or
 *         @arg @ref OB_USER_SRAM2_PE or @ref OB_USER_SRAM_RST or @ref
 * OB_USER_nSWBOOT0 or
 *         @arg @ref OB_USER_nBOOT0 or @ref OB_USER_BOOT_LOCK or @ref
 * OB_USER_BOOT_LOCK or @ref OB_USER_ALL
 * @param  UserConfig The FLASH User Option Bytes values.
 *         This parameter can be a combination of all the following values:
 *         @arg @ref OB_BOR_LEVEL_0 or @ref OB_BOR_LEVEL_1 or ... or @ref
 * OB_BOR_LEVEL_4
...
/**
 * @brief  Configure the Zone 1A Proprietary code readout protection of the
 * desired addresses, and erase configuration on RDP regression.
...
/**
 * @brief  Configure the Zone 1B Proprietary code readout protection of the
 * desired addresses.
 * @note   To configure the PCROP options, the option lock bit OPTLOCK must be
 *         cleared with the call of the @ref HAL_FLASH_OB_Unlock() function.
 * @note   To validate the PCROP options, the option bytes must be reloaded
...
static void FLASH_OB_PCROP1BConfig(uint32_t PCROP1BStartAddr,
  ...
  startoffset = ((PCROP1BStartAddr - FLASH_BASE) >>
                 FLASH_PCROP_GRANULARITY_OFFSET); /* 2K pages */
  endoffset = ((PCROP1BEndAddr - FLASH_BASE) >>
               FLASH_PCROP_GRANULARITY_OFFSET); /* 2K pages */
 ...
 * @note   To configure the extra user option bytes, the option lock bit OPTLOCK
 * must be cleared with the call of the @ref HAL_FLASH_OB_Unlock() function.
 * @note   To validate the extra user option bytes, the option bytes must be
 * reloaded through the call of the @ref HAL_FLASH_OB_Launch() function.
 ...
 *         the secure mode, the CPU2 Secure Boot reset vector and the CPU2 debug
 * access
 * @note   To configure the PCROP options, the option lock bit OPTLOCK must be
 *         cleared with the call of the @ref HAL_FLASH_OB_Unlock() function.
...
static void FLASH_OB_SecureConfig(FLASH_OBProgramInitTypeDef *pOBParam) {
  ...
  if ((pOBParam->OptionType & OPTIONBYTE_SECURE_MODE) != 0U) {
    ...
    /* Configure SFR register content with start FLASH PAGE index to secure and
     * start FLASH PAGE index for hide protection area */
    MODIFY_REG(
        sfr_reg_val, (FLASH_SFR_SFSA | FLASH_SFR_HDPSA),
        ((((pOBParam->SecureFlashStartAddr - FLASH_BASE) / FLASH_PAGE_SIZE)
          << FLASH_SFR_SFSA_Pos) |
         (((pOBParam->HideProtectionStartAddr - FLASH_BASE) / FLASH_PAGE_SIZE)
          << FLASH_SFR_HDPSA_Pos)));

    /* Configure SRRVR register */
    MODIFY_REG(srrvr_reg_val, (FLASH_SRRVR_SBRSA | FLASH_SRRVR_SNBRSA),
               (((((pOBParam->SecureSRAM2StartAddr - SRAM2_BASE) >>
                   SRAM_SECURE_PAGE_GRANULARITY_OFFSET)
                  << FLASH_SRRVR_SBRSA_Pos)) |
                ((((pOBParam->SecureSRAM1StartAddr - SRAM1_BASE) >>
                   SRAM_SECURE_PAGE_GRANULARITY_OFFSET)
    ...
    } else /* Enable Secure Area bit per bit */
    {
      ...
      FLASH_OB_ConfigSecureMode(pOBParam->SecureMode, &sfr_reg_val,
                                FLASH_SFR_FSD,
      ...
      FLASH_OB_ConfigSecureMode(pOBParam->SecureMode, &sfr_reg_val,
                                FLASH_SFR_HDPAD,
                                OB_SECURE_HIDE_PROTECTION_ENABLE);

      /* Set SRAM1 Area in secure if requested */
      FLASH_OB_ConfigSecureMode(pOBParam->SecureMode, &srrvr_reg_val,
                                FLASH_SRRVR_NBRSD, OB_SECURE_SRAM1_ENABLE);

      /* Set SRAM2 Area in secure if requested */
      FLASH_OB_ConfigSecureMode(pOBParam->SecureMode, &srrvr_reg_val,
                                FLASH_SRRVR_BRSD, OB_SECURE_SRAM2_ENABLE);
    }
  }
  ...
  if ((pOBParam->OptionType & OPTIONBYTE_C2_BOOT_VECT) != 0U) {
    ...
    if (pOBParam->C2BootRegion == OB_C2_BOOT_FROM_FLASH) {
      MODIFY_REG(srrvr_reg_val, (FLASH_SRRVR_SBRV | FLASH_SRRVR_C2OPT),
                 (((pOBParam->C2SecureBootVectAddr - FLASH_BASE) >> 2) |
                  pOBParam->C2BootRegion));
    } else {
      MODIFY_REG(srrvr_reg_val, (FLASH_SRRVR_SBRV | FLASH_SRRVR_C2OPT),
                 (((pOBParam->C2SecureBootVectAddr - SRAM1_BASE) >> 2) |
                  pOBParam->C2BootRegion));
    }
  }
  ...
  if ((pOBParam->OptionType & OPTIONBYTE_C2_DEBUG_ACCESS) != 0U) {
    ...
    MODIFY_REG(sfr_reg_val, FLASH_SFR_DDS,
               (uint32_t)pOBParam->C2DebugAccessMode);
  }
  ...
  if ((pOBParam->OptionType & OPTIONBYTE_SUBGHZSPI_SECURE_ACCESS) != 0U) {
    /* Check the parameters */
    assert_param(
        IS_OB_SUBGHZSPI_SECURE_ACCESS(pOBParam->SUBGHZSPISecureAccess));

    /* Set the Sub-GHz radio SPI Secure Access */
    MODIFY_REG(sfr_reg_val, FLASH_SFR_SUBGHZSPISD,
               (uint32_t)pOBParam->SUBGHZSPISecureAccess);
  }
...
/**
 * @brief  Return the FLASH Write Protection Option Bytes value.
 * @param PCROPConfig [out] Specifies the address where to copied the
 * configuration of PCROP_RDP option
 * @param PCROP1AStartAddr [out] Specifies the address where to copied the start
 * address of the Zone 1A Proprietary code readout protection
 * @param PCROP1AEndAddr [out] Specifies the address where to copied the end
 * address of the Zone 1A Proprietary code readout protection
 * @param PCROP1BStartAddr [out] Specifies the address where to copied the start
 * address of the Zone 1B Proprietary code readout protection
 ...
 * @retval Returned value is the address where to copied the IPCC data buffer
 * address. Value return between Min_Data = 0x0 and Max_Data = 0x3FFF This value
 * correspond to the first double-word of the IPCC mailbox data buffer area in
 * SRAM starting from 0x20000000 (SRAM1 start address to SRAM2 end address).
 */
static uint32_t FLASH_OB_GetIPCCBufferAddr(void) {
...
/**
 * @brief  Return the Secure Flash start address, Secure Hide Protection Area
 * start address, Secure Backup SRAM2 start address, Secure non-Backup SRAM1
 * start address and the SecureMode
 * @param  SecureFlashStartAddr  Specifies the address where to copied the
 * Secure Flash start address
 * @param  HideProtectionStartAddr Specifies the address where to copied the
 * Secure Hide Protection Area start address
 * @param  SecureSRAM2StartAddr  Specifies the address where to copied the
 * Secure Backup SRAM2 start address
 * @param  SecureSRAM1StartAddr  Specifies the address where to copied the
 ...
 * values:
 *                               @arg @ref
 * OB_SECURE_SYSTEM_AND_ALL_AREAS_DISABLE : All System Security disabled
 *                               @arg @ref OB_SECURE_SYSTEM_AND_FLASH_ENABLE :
 * Flash Security enabled
 *                               @arg @ref OB_SECURE_HIDE_PROTECTION_ENABLE :
 * Hide Protection Security enabled
 *                               @arg @ref OB_SECURE_SRAM1_ENABLE : SRAM1
 * Security enabled
 *                               @arg @ref OB_SECURE_SRAM2_ENABLE : SRAM2
 * Security enabled
 *                               @arg @ref OB_SECURE_SYSTEM_AND_FLASH_DISABLE :
 * Flash Security disabled
 *                               @arg @ref OB_SECURE_HIDE_PROTECTION_DISABLE :
 * Hide Protection Security disabled
 *                               @arg @ref OB_SECURE_SRAM1_DISABLE : SRAM1
 * Security disabled
 *                               @arg @ref OB_SECURE_SRAM2_DISABLE : SRAM2
 * Security disabled
 *                               @arg @ref OB_SECURE_SYSTEM_AND_ALL_AREAS_ENABLE
 * : All System Security enabled
 * @retval None
 */
static void FLASH_OB_GetSecureMemoryConfig(uint32_t *SecureFlashStartAddr,
                                           uint32_t *HideProtectionStartAddr,
  ...
  uint32_t user_config =
      (READ_BIT(sfr_reg_val, FLASH_SFR_SFSA) >> FLASH_SFR_SFSA_Pos);
  ...
  *HideProtectionStartAddr = ((user_config * FLASH_PAGE_SIZE) + FLASH_BASE);
  ...
  user_config =
      (READ_BIT(srrvr_reg_val, FLASH_SRRVR_SBRSA) >> FLASH_SRRVR_SBRSA_Pos);

  *SecureSRAM2StartAddr =
      ((user_config << SRAM_SECURE_PAGE_GRANULARITY_OFFSET) + SRAM2_BASE);

  /* Get Secure SRAM1 start address */
  user_config =
      (READ_BIT(srrvr_reg_val, FLASH_SRRVR_SNBRSA) >> FLASH_SRRVR_SNBRSA_Pos);

  *SecureSRAM1StartAddr =
      ((user_config << SRAM_SECURE_PAGE_GRANULARITY_OFFSET) + SRAM1_BASE);

  /* Get Secure Area mode */
  *SecureMode =
      (FLASH_OB_GetSecureMode(sfr_reg_val, FLASH_SFR_FSD,
                              OB_SECURE_SYSTEM_AND_FLASH_ENABLE,
                              OB_SECURE_SYSTEM_AND_FLASH_DISABLE) |
       FLASH_OB_GetSecureMode(sfr_reg_val, FLASH_SFR_HDPAD,
                              OB_SECURE_HIDE_PROTECTION_ENABLE,
                              OB_SECURE_HIDE_PROTECTION_DISABLE) |
       FLASH_OB_GetSecureMode(srrvr_reg_val, FLASH_SRRVR_NBRSD,
                              OB_SECURE_SRAM1_ENABLE, OB_SECURE_SRAM1_DISABLE) |
       FLASH_OB_GetSecureMode(srrvr_reg_val, FLASH_SRRVR_BRSD,
                              OB_SECURE_SRAM2_ENABLE, OB_SECURE_SRAM2_DISABLE));
}

/**
 * @brief  Return the CPU2 Secure Boot reset vector address and the CPU2 Secure
 * Boot Region
 * @param  C2BootResetVectAddr Specifies the address where to copied the CPU2
 * Secure Boot reset vector address
 ...
 * @retval Returned value can be one of the following values:
 *           @arg @ref OB_SUBGHZSPI_SECURE_ACCESS_DISABLE : Sub-GHz radio SPI
 * Secure access disabled
 *           @arg @ref OB_SUBGHZSPI_SECURE_ACCESS_ENABLE : Sub-GHz radio SPI
 ...
 * @param ValueEnable Constant to check in comparison with SecureMode as enabled
 * @param ValueDisable Constant to check in comparison with SecureMode as
 * disabled
 * @retval SecureMode Specifies the Secure area enabled or disabled.
 *                               This return value can be one of the following
 * values: ValueEnable ValueDisable Null
 */
static uint32_t FLASH_OB_GetSecureMode(uint32_t Reg, uint32_t Bit,
                                       uint32_t ValueEnable,
                                       uint32_t ValueDisable) {
