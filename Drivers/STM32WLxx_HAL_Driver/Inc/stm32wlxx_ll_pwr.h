...
#define LL_PWR_SR1_WRFBUSY                                                                         \
    PWR_SR1_WRFBUSYF /* Radio busy flag for wake-up and interruption (triggered status: wake-up    \
                        event or interruption occurred at least once. Can be cleared by software)  \
                      */
...
#define LL_PWR_PVDLEVEL_7                                                                          \
    (PWR_CR2_PLS_2 | PWR_CR2_PLS_1 | PWR_CR2_PLS_0) /* External input analog voltage (Compare      \
                                                       internally to VREFINT) */
/**
 * @}
 */

/** @defgroup PWR_LL_EC_EOL_OPERATING_MODES Monitoring of supply voltage for radio operating level
 * (radio End Of Life)
 * @{
 */
...
#define LL_PWR_SMPS_STEP_DOWN                                                                      \
    (PWR_CR5_SMPSEN) /*!< SMPS step down in step down mode if system low power mode is run, LP run \
                        or Stop 0. If system low power mode is Stop 1, Stop 2, Standby, Shutdown,  \
...
#define LL_PWR_SUBGHZSPI_NSS_SRC_SW                                                                \
    (0x00000000UL) /*!< Sub-GHz radio SPI NSS controlled by software. Refer to function @ref       \
 ...
 * @note   Return value "0" means the main regulator is ready and used. Return value "1" means the
 * low-power regulator is used.
 ...
 * @note   Return value "0" for main regulator supplied directly from VDD, return value "1" for main
 * regulator supplied directly from LDO or SMPS.
