...
/* Radio flags */
#define LL_PWR_SR1_WRFBUSY                                                     \
  PWR_SR1_WRFBUSYF /* Radio busy flag for wake-up and interruption (triggered  \
                      status: wake-up event or interruption occurred at least  \
                      once. Can be cleared by software) */
#define LL_PWR_SR2_RFBUSYS                                                     \
  PWR_SR2_RFBUSYS /* Radio busy signal flag (current status) */
#define LL_PWR_SR2_RFBUSYMS                                                    \
  PWR_SR2_RFBUSYMS /* Radio busy masked signal flag (current status) */

/* Multicore flags */
#define LL_PWR_EXTSCR_C1SBF PWR_EXTSCR_C1SBF /* System Standby flag for CPU1   \
                                              */
#define LL_PWR_EXTSCR_C1STOP2F                                                 \
  PWR_EXTSCR_C1STOP2F /* System Stop 2 flag for CPU1 */
#define LL_PWR_EXTSCR_C1STOPF                                                  \
  PWR_EXTSCR_C1STOPF /* System Stop 0 or stop 1 flag for CPU1 */
...
#define LL_PWR_EXTSCR_C2SBF PWR_EXTSCR_C2SBF /* System Standby flag for CPU2   \
                                              */
#define LL_PWR_EXTSCR_C2STOP2F                                                 \
  PWR_EXTSCR_C2STOP2F /* System Stop 2 flag for CPU2 */
#define LL_PWR_EXTSCR_C2STOPF                                                  \
  PWR_EXTSCR_C2STOPF /* System Stop 0 or stop 1 flag for CPU2 */
#define LL_PWR_EXTSCR_C2DS PWR_EXTSCR_C2DS /* CPU2 deepsleep mode */
#define LL_PWR_SR2_C2BOOTS                                                     \
  PWR_SR2_C2BOOTS /* CPU2 boot request source information flag */
...
/** @defgroup PWR_LL_EC_FLASH_LPRUN_POWER_DOWN_MODE Flash power-down mode during
 * low-power run mode
 * @{
 */
...
/** @defgroup PWR_LL_EC_FLASH_SLEEP_POWER_DOWN_MODE Flash power-down mode during
 * sleep mode
 * @{
 */
...
#define LL_PWR_PVDLEVEL_3                                                      \
  (PWR_CR2_PLS_1 | PWR_CR2_PLS_0)         /* VPVD3 around 2.5 V */
#define LL_PWR_PVDLEVEL_4 (PWR_CR2_PLS_2) /* VPVD4 around 2.6 V */
#define LL_PWR_PVDLEVEL_5                                                      \
  (PWR_CR2_PLS_2 | PWR_CR2_PLS_0) /* VPVD5 around 2.8 V */
#define LL_PWR_PVDLEVEL_6                                                      \
  (PWR_CR2_PLS_2 | PWR_CR2_PLS_1) /* VPVD6 around 2.9 V */
#define LL_PWR_PVDLEVEL_7                                                      \
  (PWR_CR2_PLS_2 | PWR_CR2_PLS_1 |                                             \
   PWR_CR2_PLS_0) /* External input analog voltage (Compare internally to      \
                     VREFINT) */
/**
 * @}
 */

/** @defgroup PWR_LL_EC_EOL_OPERATING_MODES Monitoring of supply voltage for
 * radio operating level (radio End Of Life)
 * @{
 */
#define LL_PWR_RADIO_EOL_DISABLE                                               \
  (0x00000000UL) /*!< Monitoring of supply voltage for radio operating level   \
                    (radio End Of Life) disable */
#define LL_PWR_RADIO_EOL_ENABLE                                                \
...
/** @defgroup PWR_LL_EC_SMPS_OPERATING_MODES SMPS step down converter operating
 * modes
 * @{
 */
#define LL_PWR_SMPS_BYPASS                                                     \
  (0x00000000UL) /*!< SMPS step down in bypass mode (disable). */
#define LL_PWR_SMPS_STEP_DOWN                                                  \
  (PWR_CR5_SMPSEN) /*!< SMPS step down in step down mode if system low power   \
                      mode is run, LP run or Stop 0. If system low power mode  \
                      is Stop 1, Stop 2, Standby, Shutdown, then SMPS is       \
                      forced in mode open to preserve energy stored in         \
                      decoupling capacitor as long as possible. Note: In case  \
                      of a board without SMPS coil mounted, SMPS should not be \
                      activated. */
/**
 * @}
 */
 ...
 * @{
 */
#define LL_PWR_RADIO_BUSY_POLARITY_RISING                                      \
  (0x00000000UL) /*!< Radio busy signal polarity to rising edge (detection on  \
                    high level). */
#define LL_PWR_RADIO_BUSY_POLARITY_FALLING                                     \
 ...
 * @{
 */
#define LL_PWR_SUBGHZSPI_NSS_SRC_SW                                            \
  (0x00000000UL) /*!< Sub-GHz radio SPI NSS controlled by software. Refer to   \
                    function @ref LL_PWR_UnselectSUBGHZSPI_NSS(). */
#define LL_PWR_SUBGHZSPI_NSS_SRC_LPTIM3                                        \
  (PWR_CR1_SUBGHZSPINSSSEL) /*!< Sub-GHz radio SPI NSS controlled by LPTIM3.   \
                             */
/**
 * @}
 */
...
/**
 * @brief  Get monitoring of supply voltage for radio operating level (radio End
 * Of Life)
...
/**
 * @brief  Check if wake-up power voltage detection is enabled with trigger for
 * CPU1
 ...
 * @note   In case of a board without SMPS coil mounted, SMPS should not be
 * activated.
 ...
 * @note   In case of a board without SMPS coil mounted, SMPS should not be
 * activated and this function is not relevant.
...
__STATIC_INLINE uint32_t LL_PWR_SMPS_GetEffectiveMode(void) {
  ...
  return (uint32_t)(READ_BIT(PWR->SR2, PWR_SR2_SMPSRDY)
                    << (PWR_CR5_SMPSEN_Pos - PWR_SR2_SMPSRDY_Pos));
}
...
LL_PWR_SetSUBGHZSPI_NSSSource(uint32_t RadioSPI_NSSSource) {
 ...
 * @retval State of bit (1 for SUBGHZSPI_NSS selected or 0 for SUBGHZSPI_NSS
 * unselected).
 */
__STATIC_INLINE uint32_t LL_PWR_IsSUBGHZSPI_NSS_Selected(void) {
  return (
      (READ_BIT(PWR->SUBGHZSPICR, PWR_SUBGHZSPICR_NSS) != (PWR_SUBGHZSPICR_NSS))
          ? 1UL
          : 0UL);
}
...
/** @defgroup PWR_LL_EF_Configuration_Multicore Configuration of multicore,
 * intended to be executed by CPU1
 * @{
 */
...
/** @defgroup PWR_LL_EF_Configuration_CPU2 Configuration of CPU2, intended to be
 * executed by CPU2
 * @{
 */
 ...
 */
__STATIC_INLINE void
LL_C2_PWR_SetFlashPowerModeLPRun(uint32_t FlashLowPowerMode) {
  /* Unlock bit FPDR */
 ...
 */
__STATIC_INLINE void
LL_C2_PWR_SetFlashPowerModeSleep(uint32_t FlashLowPowerMode) {
...
__STATIC_INLINE uint32_t LL_C2_PWR_IsEnabledInternWU(void) {
  return ((READ_BIT(PWR->C2CR3, PWR_C2CR3_EIWUL) == (PWR_C2CR3_EIWUL)) ? 1UL
                                                                       : 0UL);
}
...
__STATIC_INLINE uint32_t LL_C2_PWR_IsEnabledWPVD(void) {
  return ((READ_BIT(PWR->C2CR3, PWR_C2CR3_EWPVD) == (PWR_C2CR3_EWPVD)) ? 1UL
                                                                       : 0UL);
}
...
__STATIC_INLINE uint32_t LL_PWR_C2_IsEnabledWakeUp_ILAC(void) {
  return ((READ_BIT(PWR->SECCFGR, PWR_SECCFGR_C2EWILA) == (PWR_SECCFGR_C2EWILA))
              ? 1UL
              : 0UL);
}
#endif
 ...
 * low-power mode
 * @note   Return value "0" means the main regulator is ready and used. Return
 * value "1" means the low-power regulator is used.
 ...
 * @note   Return value "0" for main regulator supplied directly from VDD,
 * return value "1" for main regulator supplied directly from LDO or SMPS.
...
__STATIC_INLINE uint32_t LL_PWR_IsActiveFlag_FLASHRDY(void) {
  return ((READ_BIT(PWR->SR2, PWR_SR2_FLASHRDY) == (PWR_SR2_FLASHRDY)) ? 1UL
                                                                       : 0UL);
}
...
__STATIC_INLINE uint32_t LL_PWR_IsActiveFlag_SMPSRDY(void) {
  return ((READ_BIT(PWR->SR2, PWR_SR2_SMPSRDY) == (PWR_SR2_SMPSRDY)) ? 1UL
                                                                     : 0UL);
}
...
__STATIC_INLINE uint32_t LL_PWR_IsActiveFlag_RFBUSY(void) {
  return ((READ_BIT(PWR->SR1, PWR_SR1_WRFBUSYF) == (PWR_SR1_WRFBUSYF)) ? 1UL
                                                                       : 0UL);
}
...
__STATIC_INLINE uint32_t LL_PWR_IsActiveFlag_RFBUSYS(void) {
  return ((READ_BIT(PWR->SR2, PWR_SR2_RFBUSYS) == (PWR_SR2_RFBUSYS)) ? 1UL
                                                                     : 0UL);
}
...
__STATIC_INLINE uint32_t LL_PWR_IsActiveFlag_RFBUSYMS(void) {
  return ((READ_BIT(PWR->SR2, PWR_SR2_RFBUSYMS) == (PWR_SR2_RFBUSYMS)) ? 1UL
                                                                       : 0UL);
}
 ...
 *         or 1 (boot from a SW request from function @ref
 * LL_PWR_EnableBootC2()).
 */
__STATIC_INLINE uint32_t LL_PWR_IsActiveFlag_C2BOOTS(void) {
  return ((READ_BIT(PWR->SR2, PWR_SR2_C2BOOTS) == (PWR_SR2_C2BOOTS)) ? 1UL
                                                                     : 0UL);
}
#endif
...
__STATIC_INLINE uint32_t LL_PWR_IsActiveFlag_C1STOP2(void) {
  return ((READ_BIT(PWR->EXTSCR, PWR_EXTSCR_C1STOP2F) == (PWR_EXTSCR_C1STOP2F))
              ? 1UL
              : 0UL);
}
...
__STATIC_INLINE uint32_t LL_PWR_IsActiveFlag_C1DS(void) {
  return ((READ_BIT(PWR->EXTSCR, PWR_EXTSCR_C1DS) == (PWR_EXTSCR_C1DS)) ? 1UL
                                                                        : 0UL);
}
...
__STATIC_INLINE uint32_t LL_PWR_IsActiveFlag_C2STOP2(void) {
  return ((READ_BIT(PWR->EXTSCR, PWR_EXTSCR_C2STOP2F) == (PWR_EXTSCR_C2STOP2F))
              ? 1UL
              : 0UL);
}
...
__STATIC_INLINE uint32_t LL_PWR_IsActiveFlag_C2DS(void) {
  return ((READ_BIT(PWR->EXTSCR, PWR_EXTSCR_C2DS) == (PWR_EXTSCR_C2DS)) ? 1UL
                                                                        : 0UL);
}
