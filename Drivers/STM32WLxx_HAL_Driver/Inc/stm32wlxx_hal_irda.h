 ...
 * @note  HAL IRDA State value is a combination of 2 different substates: gState and RxState (see
 * @ref IRDA_State_Definition).
...
#endif /* USE_HAL_IRDA_REGISTER_CALLBACKS */
{
...
#if (USE_HAL_IRDA_REGISTER_CALLBACKS == 1)
    void (*TxHalfCpltCallback)(struct __IRDA_HandleTypeDef *hirda); /*!< IRDA Tx Half Complete
                                                                       Callback        */

    void (*TxCpltCallback)(struct __IRDA_HandleTypeDef *hirda); /*!< IRDA Tx Complete Callback */

    void (*RxHalfCpltCallback)(struct __IRDA_HandleTypeDef *hirda); /*!< IRDA Rx Half Complete
                                                                       Callback        */
    ...
    void (*AbortTransmitCpltCallback)(struct __IRDA_HandleTypeDef *hirda); /*!< IRDA Abort Transmit
                                                                              Complete Callback */

    void (*AbortReceiveCpltCallback)(struct __IRDA_HandleTypeDef *hirda); /*!< IRDA Abort Receive
                                                                             Complete Callback  */
 ...
 */
typedef void (*pIRDA_CallbackTypeDef)(IRDA_HandleTypeDef *hirda); /*!< pointer to an IRDA callback
                                                                     function */
...
#define __HAL_IRDA_ENABLE_IT(__HANDLE__, __INTERRUPT__)                                            \
    (((((__INTERRUPT__)&IRDA_CR_MASK) >> IRDA_CR_POS) == 1U)                                       \
         ? ((__HANDLE__)->Instance->CR1 |= ((uint32_t)1U << ((__INTERRUPT__)&IRDA_IT_MASK)))       \
     : ((((__INTERRUPT__)&IRDA_CR_MASK) >> IRDA_CR_POS) == 2U)                                     \
         ? ((__HANDLE__)->Instance->CR2 |= ((uint32_t)1U << ((__INTERRUPT__)&IRDA_IT_MASK)))       \
...
#define __HAL_IRDA_GET_IT(__HANDLE__, __INTERRUPT__)                                               \
    ((((__HANDLE__)->Instance->ISR & (0x01U << (((__INTERRUPT__)&IRDA_ISR_MASK) >> IRDA_ISR_POS))) \
      != 0U)                                                                                       \
         ? SET                                                                                     \
         : RESET)
