...
typedef struct
{
    uint32_t BaudRate; /*!< Configures the SmartCard communication baud rate.
                          ...
                          ((hsmartcard->Init.BaudRate))) where usart_ker_ckpres is the USART input
                          clock divided by a prescaler */

    uint32_t WordLength; /*!< Specifies the number of data bits transmitted or received in a frame.
                              This parameter @ref SMARTCARD_Word_Length can only be set to 9 (8 data
                            + 1 parity bits). */
    ...
    uint16_t OneBitSampling; /*!< Specifies whether a single sample or three samples' majority vote
                                ...
                                value of @ref SMARTCARD_OneBit_Sampling. */
 ...
 * (see @ref SMARTCARD_State_Definition).
...
typedef struct __SMARTCARD_HandleTypeDef
{
    ...
    SMARTCARD_AdvFeatureInitTypeDef AdvancedInit; /*!< SmartCard advanced features initialization
                                                     parameters */
    ...
    void (*TxCpltCallback)(struct __SMARTCARD_HandleTypeDef *hsmartcard); /*!< SMARTCARD Tx Complete
                                                                             Callback             */

    void (*RxCpltCallback)(struct __SMARTCARD_HandleTypeDef *hsmartcard); /*!< SMARTCARD Rx Complete
                                                                             Callback             */

    void (*ErrorCallback)(struct __SMARTCARD_HandleTypeDef *hsmartcard); /*!< SMARTCARD Error
                                                                            Callback */

    void (*AbortCpltCallback)(
        struct __SMARTCARD_HandleTypeDef *hsmartcard); /*!< SMARTCARD Abort Complete Callback */

    void (*AbortTransmitCpltCallback)(struct __SMARTCARD_HandleTypeDef *hsmartcard); /*!< SMARTCARD
                                                                                        Abort
                                                                                        Transmit
                                                                                        Complete
                                                                                        Callback */

    void (*AbortReceiveCpltCallback)(struct __SMARTCARD_HandleTypeDef *hsmartcard); /*!< SMARTCARD
                                                                                       Abort Receive
                                                                                       Complete
                                                                                       Callback  */

    void (*RxFifoFullCallback)(struct __SMARTCARD_HandleTypeDef *hsmartcard); /*!< SMARTCARD Rx Fifo
                                                                                 Full Callback */

    void (*TxFifoEmptyCallback)(struct __SMARTCARD_HandleTypeDef *hsmartcard); /*!< SMARTCARD Tx
                                                                                  Fifo Empty
                                                                                  Callback */

    void (*MspInitCallback)(struct __SMARTCARD_HandleTypeDef *hsmartcard); /*!< SMARTCARD Msp Init
                                                                              callback */

    void (*MspDeInitCallback)(struct __SMARTCARD_HandleTypeDef *hsmartcard); /*!< SMARTCARD Msp
                                                                                DeInit callback */
...
typedef enum
{
    ...
    HAL_SMARTCARD_ABORT_TRANSMIT_COMPLETE_CB_ID = 0x04U, /*!< SMARTCARD Abort Transmit Complete
                                                            Callback ID */
    HAL_SMARTCARD_ABORT_RECEIVE_COMPLETE_CB_ID = 0x05U,  /*!< SMARTCARD Abort Receive Complete
                                                            Callback ID  */
 ...
 */
typedef void (*pSMARTCARD_CallbackTypeDef)(SMARTCARD_HandleTypeDef *hsmartcard); /*!< pointer to an
                                                                                    SMARTCARD
                                                                                    callback
                                                                                    function */
 ...
 *            @arg @ref SMARTCARD_IT_TCBGT  Transmission complete before guard time interrupt (when
...
#define __HAL_SMARTCARD_ENABLE_IT(__HANDLE__, __INTERRUPT__)                                       \
    (((((__INTERRUPT__)&SMARTCARD_CR_MASK) >> SMARTCARD_CR_POS) == 1U)                             \
         ? ((__HANDLE__)->Instance->CR1 |= ((uint32_t)1U << ((__INTERRUPT__)&SMARTCARD_IT_MASK)))  \
     : ((((__INTERRUPT__)&SMARTCARD_CR_MASK) >> SMARTCARD_CR_POS) == 2U)                           \
         ? ((__HANDLE__)->Instance->CR2 |= ((uint32_t)1U << ((__INTERRUPT__)&SMARTCARD_IT_MASK)))  \
...
#define __HAL_SMARTCARD_GET_IT(__HANDLE__, __INTERRUPT__)                                          \
    ((((__HANDLE__)->Instance->ISR                                                                 \
       & ((uint32_t)0x01U << (((__INTERRUPT__)&SMARTCARD_ISR_MASK) >> SMARTCARD_ISR_POS)))         \
      != 0U)                                                                                       \
         ? SET                                                                                     \
         : RESET)
...
#define __HAL_SMARTCARD_GET_IT_SOURCE(__HANDLE__, __INTERRUPT__)                                   \
    ((((((((__INTERRUPT__)&SMARTCARD_CR_MASK) >> SMARTCARD_CR_POS) == 0x01U)                       \
            ...
            : (((((__INTERRUPT__)&SMARTCARD_CR_MASK) >> SMARTCARD_CR_POS) == 0x02U)                \
                   ...
                   : (__HANDLE__)->Instance->CR3))                                                 \
       & ((uint32_t)0x01U << (((uint16_t)(__INTERRUPT__)) & SMARTCARD_IT_MASK)))                   \
      != 0U)                                                                                       \
         ? SET                                                                                     \
         : RESET)
