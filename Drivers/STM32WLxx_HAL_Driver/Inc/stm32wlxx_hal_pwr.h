...
#define PWR_PVDLEVEL_3                                                         \
  (PWR_CR2_PLS_1 | PWR_CR2_PLS_0)      /*!< PVD threshold around 2.5 V */
#define PWR_PVDLEVEL_4 (PWR_CR2_PLS_2) /*!< PVD threshold around 2.6 V */
#define PWR_PVDLEVEL_5                                                         \
  (PWR_CR2_PLS_2 | PWR_CR2_PLS_0) /*!< PVD threshold around 2.8 V */
#define PWR_PVDLEVEL_6                                                         \
  (PWR_CR2_PLS_2 | PWR_CR2_PLS_1) /*!< PVD threshold around 2.9 V */
#define PWR_PVDLEVEL_7                                                         \
  (PWR_CR2_PLS_2 | PWR_CR2_PLS_1 |                                             \
   PWR_CR2_PLS_0) /*!< External input analog voltage (compared internally to   \
                     VREFINT) */
/**
 * @}
 */
...
#define PWR_PVD_MODE_NORMAL                                                    \
  (0x00000000UL) /*!< PVD in polling mode (PVD flag update without             \
                    interruption) */

#define PWR_PVD_MODE_IT_RISING                                                 \
  (PVD_MODE_IT | PVD_RISING_EDGE) /*!< PVD in interrupt mode with rising edge  \
                                     trigger detection */
#define PWR_PVD_MODE_IT_FALLING                                                \
  (PVD_MODE_IT | PVD_FALLING_EDGE) /*!< PVD in interrupt mode with falling     \
                                      edge trigger detection */
#define PWR_PVD_MODE_IT_RISING_FALLING                                         \
  (PVD_MODE_IT |                                                               \
   PVD_RISING_FALLING_EDGE) /*!< PVD in interrupt mode with rising/falling     \
                               edge trigger detection */
/**
 * @}
 */
...
#define PWR_LOWPOWERMODE_STOP0                                                 \
  (0x00000000UL) /*!< Stop 0: Stop mode with main regulator */
#define PWR_LOWPOWERMODE_STOP1                                                 \
  (PWR_C2CR1_LPMS_0) /*!< Stop 1: Stop mode with low power regulator */
#define PWR_LOWPOWERMODE_STOP2                                                 \
  (PWR_C2CR1_LPMS_1) /*!< Stop 2: Stop mode with low power regulator and       \
                        VDD12I interruptible digital core domain supply OFF    \
                        (less peripherals activated than low power mode stop 1 \
                        to reduce power consumption)*/
#define PWR_LOWPOWERMODE_STANDBY                                               \
  (PWR_C2CR1_LPMS_0 | PWR_C2CR1_LPMS_1) /*!< Standby mode */
#define PWR_LOWPOWERMODE_SHUTDOWN                                              \
  (PWR_C2CR1_LPMS_2 | PWR_C2CR1_LPMS_1 | PWR_C2CR1_LPMS_0) /*!< Shutdown mode  \
                                                            */
#else
#define PWR_LOWPOWERMODE_STOP0                                                 \
  (0x00000000UL) /*!< Stop 0: Stop mode with main regulator */
...
#define PWR_LOWPOWERMODE_STOP2                                                 \
  (PWR_CR1_LPMS_1) /*!< Stop 2: Stop mode with low power regulator and VDD12I  \
                      interruptible digital core domain supply OFF (less       \
                      peripherals activated than low power mode stop 1 to      \
                      reduce power consumption)*/
#define PWR_LOWPOWERMODE_STANDBY                                               \
  (PWR_CR1_LPMS_0 | PWR_CR1_LPMS_1) /*!< Standby mode */
#define PWR_LOWPOWERMODE_SHUTDOWN                                              \
  (PWR_CR1_LPMS_2 | PWR_CR1_LPMS_1 | PWR_CR1_LPMS_0) /*!< Shutdown mode */
#endif
/**
 * @}
 */
...
/*       (only interruption is available through EXTI line 16).               */
#define PVD_MODE_IT                                                            \
  (0x00010000UL) /*!< Mask for interruption yielded by PVD threshold crossing  \
                  */
#define PVD_RISING_EDGE                                                        \
  (0x00000001UL) /*!< Mask for rising edge set as PVD trigger */
#define PVD_FALLING_EDGE                                                       \
  (0x00000002UL) /*!< Mask for falling edge set as PVD trigger */
#define PVD_RISING_FALLING_EDGE                                                \
  (0x00000003UL) /*!< Mask for rising and falling edges set as PVD trigger */
/**
 * @}
 */
 ...
 *
 *            @arg @ref PWR_FLAG_WRFBUSY   Wake-up radio busy flag (triggered
 * status: wake-up event or interruption occurred at least once. Can be cleared
 * by software)
 *
 *            /--------------------------------SR2-------------------------------/
 ...
 *
 *            @arg @ref PWR_FLAG_REGLPS Low-power Regulator 1 started: Indicates
 * whether the regulator is ready after a power-on reset or a Standby/Shutdown.
 *            @arg @ref PWR_FLAG_REGLPF Low-power Regulator 1 flag: Indicates
 * whether the regulator 1 is in main mode or is in low-power mode.
 *
 *            @arg @ref PWR_FLAG_REGMRS Low-power regulator (main regulator or
 * low-power regulator used) flag.
 *
 *            @arg @ref PWR_FLAG_FLASHRDY Flash ready flag
 *
 *            @arg @ref PWR_FLAG_VOSF   Voltage Scaling Flag. Indicates whether
 * the regulator is ready in the selected voltage range or is still changing to
 ...
 * Indicates whether VDDA voltage is is below or above PVM3 threshold.
 *
 *            @arg @ref PWR_FLAG_RFEOL Indicate whether supply voltage is below
 * radio operating level (radio "end of life").
 *
 *            @arg @ref PWR_FLAG_RFBUSYS  Radio busy signal flag (current
 * status).
 *            @arg @ref PWR_FLAG_RFBUSYMS Radio busy masked signal flag (current
 * status).
 *
 *            @arg @ref PWR_FLAG_C2BOOTS  CPU2 boot request source information
 * flag.
 *
 *            /----------------------------EXTSCR--------------------------/
...
#define __HAL_PWR_CLEAR_FLAG(__FLAG__)                                         \
  ((((__FLAG__)&PWR_FLAG_REG_MASK) == PWR_FLAG_REG_EXTSCR)                     \
       ? (PWR->EXTSCR = (1UL << (((__FLAG__)&PWR_FLAG_EXTSCR_CLR_MASK) >>      \
...
#define __HAL_PWR_PVD_EXTI_ENABLE_IT()                                         \
  LL_C2_EXTI_EnableIT_0_31(PWR_EXTI_LINE_PVD)
#else
...
#define __HAL_PWR_PVD_EXTI_DISABLE_IT()                                        \
  LL_C2_EXTI_DisableIT_0_31(PWR_EXTI_LINE_PVD)
#else
#define __HAL_PWR_PVD_EXTI_DISABLE_IT()                                        \
  LL_EXTI_DisableIT_0_31(PWR_EXTI_LINE_PVD)
#endif
 ...
 *        EXTI rising and falling logic edges are inverted versus PVD voltage
 * edges.
 * @retval None
 */
#define __HAL_PWR_PVD_EXTI_ENABLE_RISING_EDGE()                                \
  LL_EXTI_EnableFallingTrig_0_31(PWR_EXTI_LINE_PVD)
...
#define __HAL_PWR_PVD_EXTI_DISABLE_RISING_EDGE()                               \
  LL_EXTI_DisableFallingTrig_0_31(PWR_EXTI_LINE_PVD)
...
#define __HAL_PWR_PVD_EXTI_ENABLE_FALLING_EDGE()                               \
  LL_EXTI_EnableRisingTrig_0_31(PWR_EXTI_LINE_PVD)
...
#define __HAL_PWR_PVD_EXTI_DISABLE_FALLING_EDGE()                              \
  LL_EXTI_DisableRisingTrig_0_31(PWR_EXTI_LINE_PVD)
...
#define __HAL_PWR_PVD_EXTI_GENERATE_SWIT()                                     \
  LL_EXTI_GenerateSWI_0_31(PWR_EXTI_LINE_PVD)
...
#define __HAL_PWR_PVD_EXTI_CLEAR_FLAG()                                        \
  LL_EXTI_ClearFlag_0_31(PWR_EXTI_LINE_PVD)
...
#define IS_PWR_REGULATOR(__REGULATOR__)                                        \
  (((__REGULATOR__) == PWR_MAINREGULATOR_ON) ||                                \
