 ...
 *          - Scope entire ADC (differentiation done for compatibility with some other STM32 series
 * featuring ADC groups regular and injected): ClockPrescaler, Resolution, DataAlign, ScanConvMode,
 ...
 *          - For all parameters except 'ClockPrescaler' and 'Resolution': ADC enabled without
 * conversion on going on group regular. If ADC is not in the appropriate state to modify some
 * parameters, these parameters setting is bypassed without error reporting (as it can be the
 * expected behavior in case of intended action to update another parameter (which fulfills the ADC
 * state condition) on the fly).
 */
typedef struct
{
    uint32_t ClockPrescaler; /*!< Select ADC clock source (synchronous clock derived from APB clock
                                or asynchronous clock derived from system clock or PLL (Refer to
                                reference manual for list of clocks available)) and clock prescaler.
                                  This parameter can be a value of @ref
                                ADC_HAL_EC_COMMON_CLOCK_SOURCE. Note: The ADC clock configuration is
                                common to all ADC instances. Note: In case of synchronous clock mode
                                based on HCLK/1, the configuration must be enabled only if the
                                system clock has a 50% duty clock cycle (APB prescaler configured
                                inside RCC  must be bypassed and PCLK clock must have 50% duty
                                cycle). Refer to reference manual for details. Note: In case of
                                usage of asynchronous clock, the selected clock must be
                                        preliminarily enabled at RCC top level.
                                  Note: This parameter can be modified only if all ADC instances are
                                disabled. */
    ...
    uint32_t ScanConvMode; /*!< Configure the sequencer of ADC group regular.
                                On this STM32 series, ADC group regular sequencer both modes "fully
                              configurable" or "not fully configurable" are available:
                              ...
                              'DiscontinuousConvMode' to have main sequence subdivided in successive
                              parts. Sequencer is automatically enabled if several channels are set
                              (sequencer cannot be disabled, as it can be the case on other STM32
                              devices): If only 1 channel is set: Conversion is performed in single
                              mode. If several channels are set:  Conversions are performed in
                              sequence mode. This parameter can be a value of @ref ADC_Scan_mode */

    uint32_t EOCSelection; /*!< Specify which EOC (End Of Conversion) flag is used for conversion by
    ...
    FunctionalState LowPowerAutoWait; /*!< Select the dynamic low power Auto Delay: new conversion
                                         start only when the previous conversion (for ADC group
                                         regular) has been retrieved by user software, using
                                         function HAL_ADC_GetValue(). This feature automatically
                                         adapts the frequency of ADC conversions triggers to the
                                         speed of the system that reads the data. Moreover, this
                                         avoids risk of overrun for low frequency applications. This
                                         parameter can be set to ENABLE or DISABLE. Note: It is not
                                         recommended to use with interruption or DMA
                                         (HAL_ADC_Start_IT(), HAL_ADC_Start_DMA()) since these modes
                                         have to clear immediately the EOC flag (by CPU to free the
                                         IRQ pending event or by DMA). Auto wait will work but fort
                                         a very short time, discarding its intended benefit (except
                                         specific case of high load of CPU or DMA transfers which
    ...
    FunctionalState LowPowerAutoPowerOff; /*!< Select the auto-off mode: the ADC automatically
                                             powers-off after a conversion and automatically
                                             wakes-up when a new conversion is triggered (with
                                             startup time between trigger and start of sampling).
                                                This feature can be combined with automatic wait
                                             mode (parameter 'LowPowerAutoWait').
                                                This parameter can be set to ENABLE or DISABLE. */

    FunctionalState ContinuousConvMode; /*!< Specify whether the conversion is performed in single
                                           mode (one conversion) or continuous mode for ADC group
                                           regular, after the first ADC conversion start trigger
                                           occurred (software start or external trigger). This
    ...
    uint32_t NbrOfConversion; /*!< Specify the number of ranks that will be converted within the
                                 ...
                                 each rank corresponding to parameter "NbrOfConversion" must be set
                                 using 'HAL_ADC_ConfigChannel()'. Afterwards, when all needed
                                 sequencer ranks are set, parameter 'NbrOfConversion' can be updated
                                 without modifying configuration of sequencer ranks (sequencer ranks
                                 above 'NbrOfConversion' are discarded).
                                    - sequencer configured to not fully configurable:
                                      Number of ranks in the scan sequence is defined by number of
                                 channels set in the sequence. This parameter is discarded. This
                                 parameter must be a number between Min_Data = 1 and Max_Data = 8.
                                   Note: This parameter must be modified when no conversion is on
                                 going on regular group (ADC disabled, or ADC enabled without
                                 continuous mode or external trigger that could launch a
                                 conversion). */
    ...
    FunctionalState DiscontinuousConvMode; /*!< Specify whether the conversions sequence of ADC
                                              ...
                                              set to ENABLE or DISABLE. Note: On this STM32 series,
                                              ADC group regular number of discontinuous ranks
                                              increment is fixed to one-by-one. */

    uint32_t ExternalTrigConv; /*!< Select the external event source used to trigger ADC group
                                  regular conversion start. If set to ADC_SOFTWARE_START, external
                                  triggers are disabled and software trigger is used instead. This
    ...
    uint32_t Overrun; /*!< Select the behavior in case of overrun: data overwritten or preserved
                         (default). This parameter can be a value of @ref
                         ADC_HAL_EC_REG_OVR_DATA_BEHAVIOR. Note: In case of overrun set to data
                         preserved and usage with programming model with interruption
                         (HAL_Start_IT()): ADC IRQ handler has to clear end of conversion flags,
                         this induces the release of the preserved data. If needed, this data can be
                         saved in function HAL_ADC_ConvCpltCallback(), placed in user program code
                         (called before end of conversion flags clear) Note: Error reporting with
                         respect to the conversion mode:
                                 - Usage with ADC conversion by polling for event or interruption:
                         Error is reported only if overrun is set to data preserved. If overrun is
                         set to data overwritten, user can willingly not read all the converted
                         data, this is not considered as an erroneous case.
                                 - Usage with ADC conversion by DMA: Error is reported whatever
                         overrun setting (DMA is expected to process all data from data register).
                       */
    ...
    uint32_t SamplingTimeCommon1; /*!< Set sampling time common to a group of channels.
                                     ...
                                     bits, 10.5 cycles at 10 bits, 8.5 cycles at 8 bits, 6.5 cycles
                                     at 6 bits). Note: On this STM32 family, two different sampling
                                     time settings are available, each channel can use one of these
                                     two settings. On some other STM32 devices this parameter in
                                     channel wise and is located into ADC channel initialization
                                     structure. This parameter can be a value of @ref
                                     ADC_HAL_EC_CHANNEL_SAMPLINGTIME Note: In case of usage of
                                     internal measurement channels (VrefInt/Vbat/TempSensor)
                                             sampling time constraints must be respected (sampling
                                     time can be adjusted in function of ADC clock frequency and
                                     sampling time setting) Refer to device datasheet for timings
                                     values, parameters TS_vrefint, TS_vbat, TS_temp (values rough
                                     order: few tens of microseconds). */

    uint32_t SamplingTimeCommon2; /*!< Set sampling time common to a group of channels, second
                                     common setting possible. Unit: ADC clock cycles Conversion time
                                     is the addition of sampling time and processing time (12.5 ADC
                                     clock cycles at ADC resolution 12 bits, 10.5 cycles at 10 bits,
                                         8.5 cycles at 8 bits,
                                         6.5 cycles at 6 bits).
                                       Note: On this STM32 family, two different sampling time
                                     settings are available, each channel can use one of these two
                                     settings. On some other STM32 devices this parameter in channel
                                     wise and is located into ADC channel initialization structure.
                                       This parameter can be a value of @ref
                                     ADC_HAL_EC_CHANNEL_SAMPLINGTIME Note: In case of usage of
                                     internal measurement channels (VrefInt/Vbat/TempSensor)
                                             sampling time constraints must be respected (sampling
    ...
    uint32_t TriggerFrequencyMode; /*!< Set ADC trigger frequency mode.
                                        This parameter can be a value of @ref
                                      ADC_HAL_EC_REG_TRIGGER_FREQ. Note: ADC trigger frequency mode
                                      must be set to low frequency when a duration is exceeded
                                      before ADC conversion start trigger event (between ADC enable
                                      and ADC conversion start trigger event or between two ADC
                                      conversion start trigger event). Duration value: Refer to
                                      device datasheet, parameter "tIdle". Note: When ADC trigger
                                      frequency mode is set to low frequency, some rearm cycles are
                                      inserted before performing ADC conversion start, inducing a
                                      delay of 2 ADC clock cycles. */
 ...
 *          - For all parameters: ADC disabled or enabled without conversion on going on regular
 * group. If ADC is not in the appropriate state to modify some parameters, these parameters setting
 * is bypassed without error reporting (as it can be the expected behavior in case of intended
 * action to update another parameter (which fulfills the ADC state condition) on the fly).
 */
typedef struct
{
    ...
    uint32_t Rank; /*!< Add or remove the channel from ADC regular group sequencer and specify its
                        ...
                        - sequencer configured to not fully configurable:
                          ...
                          possibility: to remove the selected rank (selected channel) from
                      sequencer. This parameter can be a value of @ref ADC_HAL_EC_REG_SEQ_RANKS */
    ...
    uint32_t SamplingTime; /*!< Sampling time value to be set for the selected channel.
                              ...
                              "SamplingTimeCommon2"), each channel can use one of these two
                              settings.
...
/**
  ...
  *          - For all parameters except 'HighThreshold', 'LowThreshold': ADC disabled or ADC
  enabled without conversion on going on ADC groups regular.
...
typedef struct
{
    ...
    uint32_t WatchdogMode; /*!< Configure the ADC analog watchdog mode: single/all/none channels.
                                For Analog Watchdog 1: Configure the ADC analog watchdog mode:
                              single channel or all channels, ADC group regular. For Analog Watchdog
                              2 and 3: Several channels can be monitored by applying successively
                              the AWD init structure. This parameter can be a value of @ref
    ...
    uint32_t Channel; /*!< Select which ADC channel to monitor by analog watchdog.
                         ...
                         'WatchdogMode' is configured on single channel (only 1 channel can be
                                                  monitored).
                           For Analog Watchdog 2 and 3: Several channels can be monitored. To use
                         this feature, call successively the function HAL_ADC_AnalogWDGConfig() for
    ...
    uint32_t HighThreshold; /*!< Configure the ADC analog watchdog High threshold value.
                               ...
                               0xFFF, 0x3FF, 0xFF or 0x3F respectively. Note: Analog watchdog 2 and
                               3 are limited to a resolution of 8 bits: if ADC resolution is 12 bits
                               the 4 LSB are ignored, if ADC resolution is 10 bits the 2 LSB are
                               ignored. */
    ...
    uint32_t LowThreshold; /*!< Configures the ADC analog watchdog Low threshold value.
                                Depending of ADC resolution selected (12, 10, 8 or 6 bits), this
                              parameter must be a number between Min_Data = 0x000 and Max_Data =
                              0xFFF, 0x3FF, 0xFF or 0x3F respectively. Note: Analog watchdog 2 and 3
                              are limited to a resolution of 8 bits: if ADC resolution is 12 bits
                              the 4 LSB are ignored, if ADC resolution is 10 bits the 2 LSB are
                              ignored.*/
    /*!< Note: If ADC oversampling is enabled, ADC analog watchdog thresholds are
               impacted: the comparison of analog watchdog thresholds is done on
...
#define HAL_ADC_STATE_BUSY_INTERNAL                                                                \
    (0x00000002UL) /*!< ADC is busy due to internal process (ex : calibration) */
...
#endif /* USE_HAL_ADC_REGISTER_CALLBACKS */
{
    ...
    uint32_t ADCGroupRegularSequencerRanks; /*!< ADC group regular sequencer memorization of ranks
                                                 setting, used in mode "fully configurable" (refer
    ...
    void (*LevelOutOfWindowCallback)(struct __ADC_HandleTypeDef *hadc); /*!< ADC analog watchdog 1
                                                                           callback */
    void (*ErrorCallback)(struct __ADC_HandleTypeDef *hadc);            /*!< ADC error callback */
    void (*LevelOutOfWindow2Callback)(struct __ADC_HandleTypeDef *hadc); /*!< ADC analog watchdog 2
                                                                            callback */
    void (*LevelOutOfWindow3Callback)(struct __ADC_HandleTypeDef *hadc); /*!< ADC analog watchdog 3
                                                                            callback */
    void (*EndOfSamplingCallback)(struct __ADC_HandleTypeDef *hadc);     /*!< ADC end of sampling
                                                                            callback */
...
without prescaler. This configuration must be enabled only if PCLK has a 50% duty clock cycle (APB \
prescaler                                                                                          \
...
sequencer length and each rank affectation to a channel are fixed by channel HW number (channel 0  \
fixed on rank 0,                                                                                   \
...
sequencer length and each rank affectation to a channel are fixed by channel HW number (channel 0  \
fixed on rank 0, channel 1 fixed on rank1, ...). Scan direction backward: from channel 18 to                       \
channel 0 */
...
#define ADC_EXTERNALTRIG_T1_TRGO2                                                                  \
    (LL_ADC_REG_TRIG_EXT_TIM1_TRGO2) /*!< TIM1 TRGO. Trigger edge set to                           \
                                          rising edge (default setting). */
#define ADC_EXTERNALTRIG_T1_CC4                                                                    \
    (LL_ADC_REG_TRIG_EXT_TIM1_CH4) /*!< TIM1 channel 4 event (capture                              \
                                        compare: input capture or output                           \
                                        capture). Trigger edge set to                              \
                                        rising edge (default setting). */
#define ADC_EXTERNALTRIG_T2_TRGO                                                                   \
    (LL_ADC_REG_TRIG_EXT_TIM2_TRGO) /*!< TIM2 TRGO. Trigger edge set to                            \
                                         rising edge (default setting). */
#define ADC_EXTERNALTRIG_T2_CC4                                                                    \
    (LL_ADC_REG_TRIG_EXT_TIM2_CH4) /*!< TIM2 channel 4 event (capture                              \
                                        compare: input capture or output                           \
                                        capture). Trigger edge set to                              \
                                        rising edge (default setting). */
#define ADC_EXTERNALTRIG_T2_CC3                                                                    \
    (LL_ADC_REG_TRIG_EXT_TIM2_CH3) /*!< TIM2 channel 3 event (capture                              \
                                        compare: input capture or output                           \
                                        capture). Trigger edge set to                              \
                                        rising edge (default setting). */
#define ADC_EXTERNALTRIG_EXT_IT11                                                                  \
    (LL_ADC_REG_TRIG_EXT_EXTI_LINE11) /*!< External interrupt line 11. Trigger                     \
...
#define ADC_EXTERNALTRIGCONVEDGE_RISING                                                            \
    (LL_ADC_REG_TRIG_EXT_RISING) /*!< ADC group regular trigger                                    \
                                      polarity set to rising edge */
#define ADC_EXTERNALTRIGCONVEDGE_FALLING                                                           \
    (LL_ADC_REG_TRIG_EXT_FALLING) /*!< ADC group regular trigger                                   \
                                       polarity set to falling edge */
#define ADC_EXTERNALTRIGCONVEDGE_RISINGFALLING                                                     \
    (LL_ADC_REG_TRIG_EXT_RISINGFALLING) /*!< ADC group regular trigger                             \
...
#define ADC_RANK_CHANNEL_NUMBER                                                                    \
    (0x00000001U) /*!< Enable the rank of the selected channels. Number of ranks in                \
...
#define ADC_RANK_NONE                                                                              \
    (0x00000002U) /*!< Disable the selected rank (selected channel) from sequencer.                \
                       Setting relevant if parameter "ScanConvMode" is set to                      \
                       sequencer not fully configurable. */
...
/** @defgroup ADC_HAL_EC_SAMPLINGTIME_COMMON  ADC instance - Sampling time common to a group of
 * channels
 * @{
 */
...
#define ADC_ANALOGWATCHDOG_SINGLE_REG                                                              \
    (ADC_CFGR1_AWD1SGL | ADC_CFGR1_AWD1EN) /*!< Analog watchdog applied to a                       \
                                                regular group, single channel */
#define ADC_ANALOGWATCHDOG_ALL_REG                                                                 \
    (ADC_CFGR1_AWD1EN) /*!< Analog watchdog applied to                                             \
...
/**
 * @note ADC trigger frequency mode must be set to low frequency when a duration is exceeded before
 * ADC conversion start trigger event (between ADC enable and ADC conversion start trigger event or
 * between two ADC conversion start trigger event). Duration value: Refer to device datasheet,
 * parameter "tIdle".
 */
#define ADC_TRIGGER_FREQ_HIGH                                                                      \
    (LL_ADC_TRIGGER_FREQ_HIGH) /*!< Trigger frequency mode set to high frequency. */
...
#define ADC_IT_AWD1 ADC_IER_AWD1IE /*!< Analog watchdog 1 interrupt source (main analog watchdog)  \
                                    */
#define ADC_IT_AWD2                                                                                \
    ADC_IER_AWD2IE /*!< Analog watchdog 2 interrupt source (additional analog watchdog) */
#define ADC_IT_AWD3                                                                                \
    ADC_IER_AWD3IE /*!< Analog watchdog 3 interrupt source (additional analog watchdog) */
/**
 * @}
 */
...
#define IS_ADC_EXTTRIG(REGTRIG)                                                                    \
    (((REGTRIG) == ADC_EXTERNALTRIG_T1_TRGO2) || ((REGTRIG) == ADC_EXTERNALTRIG_T1_CC4)            \
     || ((REGTRIG) == ADC_EXTERNALTRIG_T2_TRGO) || ((REGTRIG) == ADC_EXTERNALTRIG_T2_CC4)          \
     || ((REGTRIG) == ADC_EXTERNALTRIG_T2_CC3) || ((REGTRIG) == ADC_EXTERNALTRIG_EXT_IT11)         \
...
#define IS_ADC_CHANNEL(CHANNEL)                                                                    \
    (((CHANNEL) == ADC_CHANNEL_0) || ((CHANNEL) == ADC_CHANNEL_1) || ((CHANNEL) == ADC_CHANNEL_2)  \
     ...
     || ((CHANNEL) == ADC_CHANNEL_17) || ((CHANNEL) == ADC_CHANNEL_TEMPSENSOR)                     \
