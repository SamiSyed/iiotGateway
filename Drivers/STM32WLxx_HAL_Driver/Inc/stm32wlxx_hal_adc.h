...
typedef struct {
  ...
  uint32_t
      RightBitShift; /*!< Configures the division coefficient for the
...
typedef struct {
  uint32_t ClockPrescaler; /*!< Select ADC clock source (synchronous clock
                              ...
                              and clock prescaler. This parameter can be a value
                              of @ref ADC_HAL_EC_COMMON_CLOCK_SOURCE. Note: The
                              ADC clock configuration is common to all ADC
                              instances. Note: In case of synchronous clock mode
                              based on HCLK/1, the configuration must be enabled
                              only if the system clock has a 50% duty clock
                              cycle (APB prescaler configured inside RCC  must
                              be bypassed and PCLK clock must have 50% duty
                              cycle). Refer to reference manual for details.
                                Note: In case of usage of asynchronous clock,
                              the selected clock must be preliminarily enabled
                              at RCC top level. Note: This parameter can be
                              modified only if all ADC instances are disabled.
                            */
  ...
  uint32_t EOCSelection; /*!< Specify which EOC (End Of Conversion) flag is used
  ...
  uint32_t
      ExternalTrigConvEdge; /*!< Select the external event edge used to trigger
                               ADC group regular conversion start If trigger
                               source is set to ADC_SOFTWARE_START, this
                               parameter is discarded. This parameter can be a
                               value of @ref ADC_regular_external_trigger_edge
                             */
  ...
  uint32_t Overrun; /*!< Select the behavior in case of overrun: data
                       overwritten or preserved (default). This parameter can be
                       a value of @ref ADC_HAL_EC_REG_OVR_DATA_BEHAVIOR. Note:
                       In case of overrun set to data preserved and usage with
                       programming model with interruption (HAL_Start_IT()): ADC
                       IRQ handler has to clear end of conversion flags, this
                       induces the release of the preserved data. If needed,
                       this data can be saved in function
                       ...
                       or interruption: Error is reported only if overrun is set
                       to data preserved. If overrun is set to data overwritten,
                       user can willingly not read all the converted data, this
  ...
  uint32_t TriggerFrequencyMode; /*!< Set ADC trigger frequency mode.
                                      This parameter can be a value of @ref
                                    ADC_HAL_EC_REG_TRIGGER_FREQ. Note: ADC
                                    trigger frequency mode must be set to low
                                    frequency when a duration is exceeded before
                                    ADC conversion start trigger event (between
                                    ADC enable and ADC conversion start trigger
                                    event or between two ADC conversion start
                                    trigger event). Duration value: Refer to
                                    device datasheet, parameter "tIdle". Note:
                                    When ADC trigger frequency mode is set to
                                    low frequency, some rearm cycles are
                                    inserted before performing ADC conversion
                                            start, inducing a delay of 2 ADC
                                    clock cycles. */
...
without prescaler. This configuration must be enabled only if PCLK has a 50%   \
duty clock cycle (APB prescaler                                                \
configured inside the RCC must be bypassed and the system clock must by 50%    \
duty cycle) */
...
HW number (channel 0 fixed on rank 0, channel 1 fixed on rank1, ...). Scan                                          \
direction forward: from channel 0 to channel 18 */
#define ADC_SCAN_SEQ_FIXED_BACKWARD                                            \
  (ADC_SCAN_SEQ_FIXED_INT | ADC_CFGR1_SCANDIR) /*!< Sequencer set to not fully \
configurable: sequencer length and each rank affectation to a channel are                   \
fixed by channel HW number (channel 0 fixed on rank 0, channel 1 fixed on                                                            \
rank1, ...). Scan direction backward: from channel 18 to channel 0 */
...
#define ADC_RANK_CHANNEL_NUMBER                                                \
  (0x00000001U) /*!< Enable the rank of the selected channels. Number of ranks \
                   in the sequence is defined by number of channels enabled,                        \
                   rank of each channel is defined by channel number (channel                         \
                   0 fixed on rank 0, channel 1 fixed on rank1, ...). Setting                                                                       \
                   relevant if parameter "ScanConvMode" is set to sequencer                                                                     \
                   not fully configurable. */
#define ADC_RANK_NONE                                                          \
  (0x00000002U) /*!< Disable the selected rank (selected channel) from         \
                   sequencer. Setting relevant if parameter "ScanConvMode" is                               \
                   set to sequencer not fully configurable. */
...
#define ADC_CHANNEL_DACCH1                                                     \
  (LL_ADC_CHANNEL_DACCH1) /*!< Internal channel DAC channel 1. */
/**
 * @}
 */
...
#define ADC_IT_EOC                                                             \
  ADC_IER_EOCIE /*!< End of regular conversion interrupt source */
#define ADC_IT_EOS                                                             \
  ADC_IER_EOSIE /*!< End of regular sequence of conversions interrupt source   \
                 */
#define ADC_IT_OVR ADC_IER_OVRIE /*!< overrun interrupt source */
#define ADC_IT_AWD1                                                            \
  ADC_IER_AWD1IE /*!< Analog watchdog 1 interrupt source (main analog          \
                    watchdog) */
#define ADC_IT_AWD2                                                            \
  ADC_IER_AWD2IE /*!< Analog watchdog 2 interrupt source (additional analog    \
                    watchdog) */
#define ADC_IT_AWD3                                                            \
  ADC_IER_AWD3IE /*!< Analog watchdog 3 interrupt source (additional analog    \
                    watchdog) */
/**
 * @}
 */
...
/**
  ...
  *                                       On STM32WL, refer to device datasheet
  parameter "Avg_Slope".
