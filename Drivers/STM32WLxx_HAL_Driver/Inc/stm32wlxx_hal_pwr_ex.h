...
#define PWR_FLAG_REG_MASK_POS                                                                      \
    (5UL) /* Bitfield mask position to indicate PWR flag location in PWR register */
#define PWR_FLAG_REG_MASK                                                                          \
    (PWR_FLAG_REG_SR1 | PWR_FLAG_REG_SR2 | PWR_FLAG_REG_EXTSCR) /* Bitfield mask to indicate PWR   \
                                                                   flag location in PWR register   \
                                                                 */
...
#define PWR_FLAG_EXTSCR_CLR_MASK                                                                   \
    ((PWR_EXTSCR_C1CSSF_Pos | PWR_EXTSCR_C2CSSF_Pos) << PWR_FLAG_EXTSCR_CLR_POS) /* Bitfield for   \
                                                                                    register       \
                                                                                    PWR_EXTSCR     \
                                                                                    clearable bits \
                                                                                    positions:     \
...
#else
#define PWR_FLAG_EXTSCR_CLR_MASK                                                                   \
    ((PWR_EXTSCR_C1CSSF_Pos) << PWR_FLAG_EXTSCR_CLR_POS) /* Bitfield for register PWR_EXTSCR       \
...
#define PWR_WAKEUP_PIN1_LOW                                                                        \
    ((PWR_CR4_WP1 << PWR_WUP_POLARITY_SHIFT) | PWR_CR3_EWUP1) /*!< Wakeup pin 1 (with low level    \
                                                                 polarity) */
#define PWR_WAKEUP_PIN2_LOW                                                                        \
    ((PWR_CR4_WP2 << PWR_WUP_POLARITY_SHIFT) | PWR_CR3_EWUP2) /*!< Wakeup pin 2 (with low level    \
                                                                 polarity) */
#define PWR_WAKEUP_PIN3_LOW                                                                        \
    ((PWR_CR4_WP3 << PWR_WUP_POLARITY_SHIFT) | PWR_CR3_EWUP3) /*!< Wakeup pin 3 (with low level    \
                                                                 polarity) */
/**
 * @}
 */
...
#define PWR_PVM_MODE_IT_RISING                                                                     \
    (PVM_MODE_IT | PVM_RISING_EDGE) /*!< External Interrupt Mode with Rising edge trigger          \
                                       detection */
#define PWR_PVM_MODE_IT_FALLING                                                                    \
    (PVM_MODE_IT | PVM_FALLING_EDGE) /*!< External Interrupt Mode with Falling edge trigger        \
                                        detection */
...
/** @defgroup PWR_EC_EOL_OPERATING_MODES Monitoring of supply voltage for radio operating level
 * (radio End Of Life)
 * @{
 */
#define PWR_RADIO_EOL_DISABLE                                                                      \
    (0x00000000UL) /*!< Monitoring of supply voltage for radio operating level (radio End Of Life) \
                      disable */
#define PWR_RADIO_EOL_ENABLE                                                                       \
    (PWR_CR5_RFEOLEN) /*!< Monitoring of supply voltage for radio operating level (radio End Of    \
                         Life) enable */
/**
 * @}
 */
...
#define PWR_SMPS_STEP_DOWN                                                                         \
    (PWR_CR5_SMPSEN) /*!< SMPS step down in step down mode if system low power mode is run, LP run \
                        or stop0. If system low power mode is stop1, stop2, standby, shutdown,     \
                        then SMPS is forced in mode open to preserve energy stored in decoupling   \
                        capacitor as long as possible. Note: In case of a board without SMPS coil  \
                        mounted, SMPS should not be activated. */
/**
 * @}
 */
...
#define PWR_RADIO_BUSY_POLARITY_RISING                                                             \
    (0x00000000UL) /*!< Radio busy signal polarity to rising edge (detection on high level). */
#define PWR_RADIO_BUSY_POLARITY_FALLING                                                            \
    (PWR_CR4_WRFBUSYP) /*!< Radio busy signal polarity to falling edge (detection on low level).   \
                        */
/**
 * @}
 */
...
#define PWR_RADIO_BUSY_TRIGGER_NONE                                                                \
    (0x00000000UL) /*!< Radio busy trigger action: no wake-up from low-power mode and no           \
                      interruption sent to the selected CPU. */
#define PWR_RADIO_BUSY_TRIGGER_WU_IT                                                               \
    (PWR_CR3_EWRFBUSY) /*!< Radio busy trigger action: wake-up from low-power mode Standby and     \
                          interruption sent to the selected CPU. */
/**
 * @}
 */
...
#define PWR_RADIO_IRQ_TRIGGER_NONE                                                                 \
    (0x00000000UL) /*!< Radio IRQ trigger action: no wake-up from low-power mode and no            \
                      interruption sent to the selected CPU. */
#define PWR_RADIO_IRQ_TRIGGER_WU_IT                                                                \
    (PWR_CR3_EWRFIRQ) /*!< Radio IRQ trigger action: wake-up from low-power mode Standby and       \
                         interruption sent to the selected CPU. */
/**
 * @}
 */
...
#define PWR_FLAG_WRFBUSY                                                                           \
    (PWR_FLAG_REG_SR1 | PWR_SR1_WRFBUSYF_Pos) /*!< Wakeup radio busy flag (triggered status:       \
...
#define PWR_FLAG_REGLPS                                                                            \
    (PWR_FLAG_REG_SR2 | PWR_SR2_REGLPS_Pos) /*!< Low-power regulator started and ready flag */
#define PWR_FLAG_REGLPF                                                                            \
    (PWR_FLAG_REG_SR2 | PWR_SR2_REGLPF_Pos) /*!< Low-power regulator (main regulator or low-power  \
                                               regulator used) flag */
#define PWR_FLAG_REGMRS                                                                            \
    (PWR_FLAG_REG_SR2 | PWR_SR2_REGMRS_Pos) /*!< Main regulator supply from LDO or SMPS or         \
                                               directly from VDD */
...
#define PWR_FLAG_RFEOL                                                                             \
    (PWR_FLAG_REG_SR2 | PWR_SR2_RFEOLF_Pos) /*!< Power Voltage Monitoring Radio end of life flag   \
                                             */
#define PWR_FLAG_RFBUSYS                                                                           \
    (PWR_FLAG_REG_SR2 | PWR_SR2_RFBUSYS_Pos) /*!< Radio busy signal flag (current status) */
#define PWR_FLAG_RFBUSYMS                                                                          \
    (PWR_FLAG_REG_SR2 | PWR_SR2_RFBUSYMS_Pos) /*!< Radio busy masked signal flag (current status)  \
                                               */
#define PWR_FLAG_C2BOOTS                                                                           \
    (PWR_FLAG_REG_SR2 | PWR_SR2_C2BOOTS_Pos) /*!< CPU2 boot request source information flag */
/*------------------------------EXTSCR------------------------------*/
#define PWR_FLAG_SB                                                                                \
...
#define PWR_FLAG_STOP2                                                                             \
    (PWR_FLAG_REG_EXTSCR | PWR_EXTSCR_C1STOP2F_Pos                                                 \
     | (PWR_EXTSCR_C1CSSF_Pos << PWR_FLAG_EXTSCR_CLR_POS)) /*!< System Stop 2 flag for CPU1 */
#define PWR_FLAG_STOP                                                                              \
    (PWR_FLAG_REG_EXTSCR | PWR_EXTSCR_C1STOPF_Pos                                                  \
     | (PWR_EXTSCR_C1CSSF_Pos << PWR_FLAG_EXTSCR_CLR_POS)) /*!< System Stop 0 or Stop 1 flag for   \
                                                              CPU1 */
#if defined(DUAL_CORE)
...
#define PWR_FLAG_C2STOP2                                                                           \
    (PWR_FLAG_REG_EXTSCR | PWR_EXTSCR_C2STOP2F_Pos                                                 \
     | (PWR_EXTSCR_C2CSSF_Pos << PWR_FLAG_EXTSCR_CLR_POS)) /*!< System Stop 2 flag for CPU2 */
#define PWR_FLAG_C2STOP                                                                            \
    (PWR_FLAG_REG_EXTSCR | PWR_EXTSCR_C2STOPF_Pos                                                  \
     | (PWR_EXTSCR_C2CSSF_Pos << PWR_FLAG_EXTSCR_CLR_POS)) /*!< System Stop 0 or Stop 1 flag for   \
                                                              CPU2 */
#endif

#define PWR_FLAG_LPMODES                                                                           \
    (PWR_FLAG_SB) /*!< System flag encompassing all low-powers flags (Stop0, 1, 2 and Standby) for \
                     CPU1, used when clearing flags */
#if defined(DUAL_CORE)
#define PWR_FLAG_C2LPMODES                                                                         \
    (PWR_FLAG_C2SB) /*!< System flag encompassing all low-powers flags (Stop0, 1, 2 and Standby)   \
                       for CPU2, used when clearing flags */
#endif
...
#define IS_PWR_PVM_MODE(__MODE__)                                                                  \
    (((__MODE__) == PWR_PVM_MODE_NORMAL) || ((__MODE__) == PWR_PVM_MODE_IT_RISING)                 \
     || ((__MODE__) == PWR_PVM_MODE_IT_FALLING) || ((__MODE__) == PWR_PVM_MODE_IT_RISING_FALLING))
...
#define IS_PWR_VOLTAGE_SCALING_RANGE(__RANGE__)                                                    \
    (((__RANGE__) == PWR_REGULATOR_VOLTAGE_SCALE1) || ((__RANGE__) == PWR_REGULATOR_VOLTAGE_SCALE2))
...
#define IS_PWR_SMPS_MODE(__SMPS_MODE__)                                                            \
    (((__SMPS_MODE__) == PWR_SMPS_BYPASS) || ((__SMPS_MODE__) == PWR_SMPS_STEP_DOWN))
