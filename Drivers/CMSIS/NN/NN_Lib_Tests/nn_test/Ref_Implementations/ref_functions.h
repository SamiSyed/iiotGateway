...
void arm_convolve_HWC_q7_ref(const q7_t *Im_in,         // input image
                             ...
                             const uint16_t ch_im_out,  // number of filters, i.e., output image
                                                        // channels
...
void arm_convolve_HWC_q7_ref_nonsquare(const q7_t *Im_in,          // input image
                                       ...
                                       const uint16_t ch_im_in,    // number of input image channels
                                       const q7_t *wt,             // kernel weights
                                       const uint16_t ch_im_out, // number of filters, i.e., output
                                                                 // image channels
...
void arm_depthwise_separable_conv_HWC_q7_ref(const q7_t *Im_in,         // input image
                                             const uint16_t dim_im_in,  // input image dimention
                                             const uint16_t ch_im_in,   // number of input image
                                                                        // channels
                                             const q7_t *wt,            // kernel weights
                                             const uint16_t ch_im_out,  // number of filters, i.e.,
                                                                        // output image channels
                                             const uint16_t dim_kernel, // filter kernel size
                                             const uint16_t padding,    // padding sizes
                                             const uint16_t stride,     // stride
                                             const q7_t *bias,          // bias
                                             const uint16_t bias_shift, // amount of left-shift for
                                                                        // bias
                                             const uint16_t out_shift,  // amount of right-shift for
                                                                        // output
                                             q7_t *Im_out,              // output image
                                             const uint16_t dim_im_out, // output image dimension
                                             q15_t *bufferA,            // buffer space for input
...
void arm_depthwise_separable_conv_HWC_q7_ref_nonsquare(const q7_t *Im_in,          // input image
                                                       const uint16_t dim_im_in_x, // input image
                                                                                   // dimention x
                                                       const uint16_t dim_im_in_y, // input image
                                                                                   // dimention y
                                                       const uint16_t ch_im_in,  // number of input
                                                                                 // image channels
                                                       const q7_t *wt,           // kernel weights
                                                       const uint16_t ch_im_out, // number of
                                                                                 // filters, i.e.,
                                                                                 // output image
                                                                                 // channels
                                                       const uint16_t dim_kernel_x, // filter kernel
                                                                                    // size x
                                                       const uint16_t dim_kernel_y, // filter kernel
                                                                                    // size y
                                                       const uint16_t padding_x,  // padding sizes x
                                                       const uint16_t padding_y,  // padding sizes y
                                                       const uint16_t stride_x,   // stride x
                                                       const uint16_t stride_y,   // stride y
                                                       const q7_t *bias,          // bias
                                                       const uint16_t bias_shift, // amount of
                                                                                  // left-shift for
                                                                                  // bias
                                                       const uint16_t out_shift,  // amount of
                                                                                  // right-shift for
                                                                                  // output
                                                       q7_t *Im_out,              // output image
                                                       const uint16_t dim_im_out_x, // output image
                                                                                    // dimension x
                                                       const uint16_t dim_im_out_y, // output image
                                                                                    // dimension y
                                                       q15_t *bufferA, // buffer space for input
                                                       q7_t *bufferB   // buffer space for output
);
...
void arm_fully_connected_mat_q7_vec_q15_opt_ref(const q15_t *pV,            // pointer to vector
                                                ...
                                                const uint16_t dim_vec,     // length of the vector
                                                const uint16_t num_of_rows, // numCol of A
                                                const uint16_t bias_shift,  // amount of left-shift
                                                                            // for bias
                                                const uint16_t out_shift,   // amount of right-shift
                                                                            // for output
                                                const q7_t *bias,
                                                q15_t *pOut, // output operand
                                                q15_t *vec_buffer);
