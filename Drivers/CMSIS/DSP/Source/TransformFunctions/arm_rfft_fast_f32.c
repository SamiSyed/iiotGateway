...
/**
  ...
  spectrum in complex format. Except the first complex number that contains the two real numbers
  ...
  arm_rfft_fast_init_f32(). The older functions \ref arm_rfft_f32() and \ref arm_rfft_init_f32()
  have been deprecated but are still documented.
  @par
  ...
  frequency. Looking at the data, we see that we can uniquely represent the FFT using only N/2
  complex numbers. These are packed into the output array in alternating real and imaginary
  components:
  @par
  ...
  twidCoefRModifier, pTwiddleAReal, pTwiddleBReal, pCfft}; arm_rfft_instance_q15 S = {fftLenReal,
