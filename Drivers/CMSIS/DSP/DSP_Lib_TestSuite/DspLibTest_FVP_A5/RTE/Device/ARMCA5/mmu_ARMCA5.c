...
// For cores without coherency logic (such as SCU) marking a region as shareable forces the
// processor to not cache that region regardless of the inner cache settings. Cortex-A versions of
// RTX use LDREX/STREX instructions relying on Local monitors. Local monitors will be used only when
// the region gets cached, regions that are not cached will use the Global Monitor. Some Cortex-A
// implementations do not include Global Monitors, so wrongly setting the attribute Shareable may
// cause STREX to fail.

// Recall: When the Shareable attribute is applied to a memory region that is not Write-Back, Normal
// memory, data held in this region is treated as Non-cacheable. When SMP bit = 0, Inner WB/WA
// Cacheable Shareable attributes are treated as Non-cacheable. When SMP bit = 1, Inner WB/WA
// Cacheable Shareable attributes are treated as Cacheable.

// Following MMU configuration is expected
// SCTLR.AFE == 1 (Simplified access permissions model - AP[2:1] define access permissions, AP[0] is
// an access flag) SCTLR.TRE == 0 (TEX remap disabled, so memory type and attributes are described
// directly by bits in the descriptor) Domain 0 is always the Client domain Descriptors should place
// all memory in domain 0
...
#define PERIPHERAL_A_TABLE_L2_BASE_64k (0x80504400) // Map 64k Peripheral #1 0x1C000000 -
                                                    // 0x1C00FFFFF
#define PERIPHERAL_B_TABLE_L2_BASE_64k (0x80504800) // Map 64k Peripheral #2 0x1C100000 -
                                                    // 0x1C1FFFFFF
...
static uint32_t Sect_Normal; // outer & inner wb/wa, non-shareable, executable, rw, domain 0, base
                             // addr 0
static uint32_t Sect_Normal_Cod; // outer & inner wb/wa, non-shareable, executable, ro, domain 0,
                                 // base addr 0
...
void MMU_CreateTranslationTable(void)
{
    ...
    // all DRAM executable, rw, cacheable - applications may choose to divide memory into ro
    // executable
    ...
    MMU_TTPage64k(&Image$$TTB$$ZI$$Base,
                  ...
                  Page_64k_Device_RW);
    MMU_TTPage64k(&Image$$TTB$$ZI$$Base,
                  VE_A5_MP_SYSTEM_REG_BASE,
                  1,
                  Page_L1_64k,
                  (uint32_t *)PERIPHERAL_A_TABLE_L2_BASE_64k,
                  Page_64k_Device_RW);
    MMU_TTPage64k(&Image$$TTB$$ZI$$Base,
                  VE_A5_MP_SERIAL_BASE,
                  1,
                  Page_L1_64k,
                  (uint32_t *)PERIPHERAL_A_TABLE_L2_BASE_64k,
                  Page_64k_Device_RW);
    ...
    // Create (256 * 4k)=1MB faulting entries to cover private address space. Needs to be marked as
    // Device memory
    MMU_TTPage4k(&Image$$TTB$$ZI$$Base,
                 __get_CBAR(),
                 256,
                 Page_L1_4k,
                 (uint32_t *)PRIVATE_TABLE_L2_BASE_4k,
                 DESCRIPTOR_FAULT);
    ...
    // Create (256 * 4k)=1MB faulting entries to synchronization space (Useful if some non-cacheable
    // DMA agent is present in the SoC)
    MMU_TTPage4k(&Image$$TTB$$ZI$$Base,
                 F_SYNC_BASE,
                 256,
                 Page_L1_4k,
                 (uint32_t *)SYNC_FLAGS_TABLE_L2_BASE_4k,
                 DESCRIPTOR_FAULT);
