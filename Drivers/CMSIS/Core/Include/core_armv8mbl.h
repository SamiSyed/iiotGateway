...
#define __ARMv8MBL_CMSIS_VERSION                                                                   \
    ((__ARMv8MBL_CMSIS_VERSION_MAIN << 16U) | __ARMv8MBL_CMSIS_VERSION_SUB) /*!< \deprecated CMSIS \
                                                                               HAL version number  \
                                                                             */
...
typedef struct
{
    ...
    uint32_t RESERVED1;
    __IOM uint32_t SHPR[2U]; /*!< Offset: 0x01C (R/W)  System Handlers Priority Registers. [0] is
                                RESERVED */
...
#define SysTick_NS                                                                                 \
    ((SysTick_Type *)SysTick_BASE_NS) /*!< SysTick configuration struct      (non-secure address   \
                                         space) */
#define NVIC_NS                                                                                    \
 ...
 */
#if defined(__FPU_PRESENT) && (__FPU_PRESENT == 1U) /* Value for processors with floating-point    \
                                                       extension:                  */
...
__STATIC_INLINE uint32_t SysTick_Config(uint32_t ticks)
{
    ...
    NVIC_SetPriority(SysTick_IRQn, (1UL << __NVIC_PRIO_BITS) - 1UL); /* set Priority for Systick
                                                                        Interrupt */
    SysTick->VAL = 0UL; /* Load the SysTick Counter Value */
    SysTick->CTRL
        = SysTick_CTRL_CLKSOURCE_Msk | SysTick_CTRL_TICKINT_Msk | SysTick_CTRL_ENABLE_Msk; /* Enable
                                                                                              SysTick
                                                                                              IRQ
                                                                                              and
                                                                                              SysTick
...
__STATIC_INLINE uint32_t TZ_SysTick_Config_NS(uint32_t ticks)
{
    ...
    TZ_NVIC_SetPriority_NS(SysTick_IRQn, (1UL << __NVIC_PRIO_BITS) - 1UL); /* set Priority for
                                                                              Systick Interrupt */
