...
#define GET_UINT32_LE(b, i)                                                                        \
    ((uint32_t)(b)[(i)]) | ((uint32_t)(b)[(i) + 1] << 8) | ((uint32_t)(b)[(i) + 2] << 16)          \
        | ((uint32_t)(b)[(i) + 3] << 24)
#endif /* GET_UINT32_LE */
...
#else /* LORAWAN_KMS == 1 */
static Key_t KeyList[NUM_OF_KEYS] = {{APP_KEY, KMS_APP_KEY_OBJECT_HANDLE},
 ...
 * \retval                    - Status of the operation
 */
static SecureElementStatus_t ComputeCmac(uint8_t *micBxBuffer,
                                         uint8_t *buffer,
                                         uint32_t size,
                                         KeyIdentifier_t keyID,
                                         uint32_t *cmac);
                                         ...
                                         uint32_t *cmac)
{
    ...
    /* Configure session to Authentication message in AES CMAC with settings included into the
     * mechanism */
    if (rv == CKR_OK)
    {
...
need to replace this code as below with C_SignUpdate and C_SignFinal methods usage.
    ...
    if (rv == CKR_OK)
    {
        ...
        {
            rv = C_Sign(session,
                        (CK_BYTE_PTR)&input_align_combined_buf[0],
                        size + SE_KEY_SIZE,
                        &tag[0],
...
SecureElementStatus_t SecureElementSetKey(KeyIdentifier_t keyID, uint8_t *key)
{
    ...
    if (rv == CKR_OK)
    {
        rv = C_CreateObject(session,
                            key_attribute_template,
                            GET_NB_ATTRIBUTE(key_attribute_template),
                            &key_handle);
    }
...
SecureElementStatus_t SecureElementSetID(SecureElementNvmDevJoinAddrKey_t *KMSKeyBlob)
{
    ...
    if (rv == CKR_OK)
    {
        rv = C_CreateObject(session,
                            attribute_template,
                            GET_NB_ATTRIBUTE(attribute_template),
                            &key_handle);
    }
                                                 ...
                                                 KeyIdentifier_t keyID)
{
    ...
    if (rv == CKR_OK)
    {
        ...
        rv = C_Verify(session,
                      (CK_BYTE_PTR)input_align_combined_buf,
                      size,
                      (CK_BYTE_PTR)&expectedCmac,
                      4);
    }
                                                     ...
                                                     KeyIdentifier_t targetKeyID)
{
    ...
    if (rv == CKR_OK)
    {
        rv = C_DeriveKey(session,
                         &(mech),
                         rootkey_object_handle,
                         &DeriveKey_template,
                         GET_NB_ATTRIBUTE(DeriveKey_template),
                                                     ...
                                                     uint8_t *versionMinor)
{
    ...
    *versionMinor = ((decJoinAccept[11] & 0x80) == 0x80) ? 1 : 0;
    ...
    else if (*versionMinor == 1)
    {
         ...
         *   cmac = aes128_cmac(JSIntKey, JoinReqType | JoinEUI | DevNonce | MHDR | JoinNonce |
         * NetID | DevAddr | DLSettings | RxDelay | CFList | CFListType) Prepare the msg for
         * integrity check (adding JoinReqType, JoinEUI and DevNonce)
         */
        uint8_t localBuffer[LORAMAC_JOIN_ACCEPT_FRAME_MAX_SIZE + JOIN_ACCEPT_MIC_COMPUTATION_OFFSET]
            = {0};

        memcpy1(localBuffer, micHeader11, JOIN_ACCEPT_MIC_COMPUTATION_OFFSET);
        memcpy1(localBuffer + JOIN_ACCEPT_MIC_COMPUTATION_OFFSET - 1,
                decJoinAccept,
                encJoinAcceptSize);
