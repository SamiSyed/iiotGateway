...
#define CRYPTO_MIC_COMPUTATION_OFFSET                                          \
  (JOIN_REQ_TYPE_SIZE + LORAMAC_JOIN_EUI_FIELD_SIZE + DEV_NONCE_SIZE +         \
 ...
 */
static uint8_t
    input_align_combined_buf[PAYLOAD_MAX_SIZE + SE_KEY_SIZE] ALIGN(4);
 ...
 * \retval                    - Status of the operation
 */
static SecureElementStatus_t GetKeyIndexByID(KeyIdentifier_t keyID,
 ...
 * \retval                    - Status of the operation
 */
static SecureElementStatus_t GetSpecificLabelByID(KeyIdentifier_t keyID,
                                                  uint32_t *keyLabel);
#endif /* LORAWAN_KMS */
...
static SecureElementStatus_t ComputeCmac(uint8_t *micBxBuffer, uint8_t *buffer,
                                         uint32_t size, KeyIdentifier_t keyID,
                                         uint32_t *cmac);
...
static void PrintKey(KeyIdentifier_t keyID) {
  ...
  if (SECURE_ELEMENT_SUCCESS ==
      SecureElementGetKeyByID(keyID, (uint8_t *)extractable_key))
#endif /* LORAWAN_KMS */
  {
    for (uint8_t i = 0; i < NUM_OF_KEYS; i++) {
      if (KeyLabel[i].keyID == keyID) {
        ...
        MW_LOG(TS_OFF, VLEVEL_M,
               ...
               "02X:%02X:%02X:"
               "%02X\r\n",
               KeyLabel[i].keyStr, HEX16(keyItem->KeyValue));
#else
        MW_LOG(TS_OFF, VLEVEL_M,
               "###### %s "
               "%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%"
               "02X:%02X:%02X:"
               "%02X\r\n",
               KeyLabel[i].keyStr, HEX16(extractable_key));
#endif /* LORAWAN_KMS */
        return;
      }
    }
...
static SecureElementStatus_t ComputeCmac(uint8_t *micBxBuffer, uint8_t *buffer,
  ...
  /* Configure session to Authentication message in AES CMAC with settings
   * included into the mechanism */
  if (rv == CKR_OK) {
...
need to replace this code as below with C_SignUpdate and C_SignFinal methods usage.
  ...
  if (rv == CKR_OK) {
    /* work around : need to double-check if possible to use micBxBuffer as IV
     * for Sign */
    if (micBxBuffer != NULL) {
      memcpy1((uint8_t *)&input_align_combined_buf[0], (uint8_t *)micBxBuffer,
              SE_KEY_SIZE);
      memcpy1((uint8_t *)&input_align_combined_buf[SE_KEY_SIZE],
              (uint8_t *)buffer, size);
    } else {
  ...
  if (rv == CKR_OK) {
    if (micBxBuffer != NULL) {
      rv = C_Sign(session, (CK_BYTE_PTR)&input_align_combined_buf[0],
                  size + SE_KEY_SIZE, &tag[0], (CK_ULONG_PTR)&tag_length);
    } else {
      rv = C_Sign(session, (CK_BYTE_PTR)&input_align_combined_buf[0], size,
                  &tag[0], (CK_ULONG_PTR)&tag_length);
    }
  }
#else
  ...
  if ((buffer_addr % 4) == 0) /* buffer address is aligned */
  {
    ...
    while ((size != 0) && (rv == CKR_OK)) {
      ...
      memcpy1((uint8_t *)input_align_combined_buf, (uint8_t *)buffer,
              max_allocated_size);
      rv = C_SignUpdate(session, (CK_BYTE_PTR)input_align_combined_buf,
                        max_allocated_size);
...
SecureElementStatus_t SecureElementInit(SecureElementNvmData_t *nvm) {
  ...
  CK_ATTRIBUTE key_template = {CKA_LABEL, (CK_VOID_PTR)local_template_label,
                               sizeof(local_template_label)};
  ...
  for (uint8_t itr = 0; itr < NUM_OF_KEYS; itr++) {
    if (SECURE_ELEMENT_SUCCESS ==
        GetSpecificLabelByID(KeyList[itr].KeyID, &local_template_label[1])) {
      ulCount = 0;
      /* Search from Template pattern */
      if (rv == CKR_OK) {
        rv = C_FindObjectsInit(session, &key_template,
                               GET_NB_ATTRIBUTE(key_template));
      }
...
SecureElementInitMcuID(SecureElementGetUniqueId_t seGetUniqueId,
...
#if (LORAWAN_KMS == 0)
SecureElementStatus_t SecureElementGetKeyByID(KeyIdentifier_t keyID,
                                              Key_t **keyItem) {
#if (KEY_EXTRACTABLE == 1)
  for (uint8_t i = 0; i < NUM_OF_KEYS; i++) {
    if (SeNvm->KeyList[i].KeyID == keyID) {
...
SecureElementStatus_t SecureElementGetKeyByID(KeyIdentifier_t keyID,
  ...
  CK_ATTRIBUTE key_attribute_template = {
      CKA_VALUE, (CK_VOID_PTR)&derive_key_template_class, size};

  if (GetKeyIndexByID(keyID, &key_handle) ==
...
SecureElementStatus_t SecureElementDeleteDynamicKey(KeyIdentifier_t keyID,
                                                    uint32_t *key_label) {
#if (LORAWAN_KMS == 0)
  return SECURE_ELEMENT_ERROR;
#else  /* LORAWAN_KMS == 1 */
  ...
  CK_OBJECT_HANDLE hObject[NUM_OF_KEYS];
  CK_ULONG local_template_label[] = {GlobalTemplateLabel, 0UL};
  CK_ATTRIBUTE dynamic_key_template = {CKA_LABEL,
                                       (CK_VOID_PTR)local_template_label,
                                       sizeof(local_template_label)};
  uint32_t ulCount = 0;
  ...
  if (SECURE_ELEMENT_SUCCESS !=
      GetSpecificLabelByID(keyID, &local_template_label[1])) {
    return SECURE_ELEMENT_ERROR;
  }
  *key_label = local_template_label[1];
  ...
  /* Search from Template pattern */
  if (rv == CKR_OK) {
    rv = C_FindObjectsInit(session, &dynamic_key_template,
                           GET_NB_ATTRIBUTE(dynamic_key_template));
  }
...
SecureElementStatus_t SecureElementSetObjHandler(KeyIdentifier_t keyID,
                                                 uint32_t keyIndex) {
#if (LORAWAN_KMS == 0)
  return SECURE_ELEMENT_ERROR;
#else  /* LORAWAN_KMS == 1 */
...
SecureElementStatus_t SecureElementSetKey(KeyIdentifier_t keyID, uint8_t *key) {
  ...
  for (uint8_t i = 0; i < NUM_OF_KEYS; i++) {
    if (SeNvm->KeyList[i].KeyID == keyID) {
...
#endif /* LORAMAC_MAX_MC_CTX */
      {
        ...
        retval =
            SecureElementAesEncrypt(key, SE_KEY_SIZE, MC_KE_KEY, decryptedKey);
  ...
  if (SECURE_ELEMENT_SUCCESS !=
      SecureElementDeleteDynamicKey(keyID, &specific_label[1])) {
    return SECURE_ELEMENT_ERROR;
  }
  ...
  /* Get key to display */
  if (rv == CKR_OK) {
    rv = C_CreateObject(session, key_attribute_template,
                        GET_NB_ATTRIBUTE(key_attribute_template), &key_handle);
  }
...
SecureElementStatus_t
SecureElementSetID(SecureElementNvmDevJoinAddrKey_t *KMSKeyBlob) {
#if (LORAWAN_KMS == 0)
  return SECURE_ELEMENT_SUCCESS;
#else  /* LORAWAN_KMS == 1 */
  SecureElementStatus_t retval = SECURE_ELEMENT_ERROR;
  ...
  uint32_t buf_ui32[] = {
      GET_UINT32_BE(buf, 0),  GET_UINT32_BE(buf, 4),  GET_UINT32_BE(buf, 8),
      GET_UINT32_BE(buf, 12), GET_UINT32_BE(buf, 16), GET_UINT32_BE(buf, 20),
  };
  ...
  /* Get key to display */
  if (rv == CKR_OK) {
    rv = C_CreateObject(session, attribute_template,
                        GET_NB_ATTRIBUTE(attribute_template), &key_handle);
  }
...
SecureElementStatus_t SecureElementVerifyAesCmac(uint8_t *buffer, uint32_t size,
  ...
  /* Configure session to Verify the message in AES CMAC with settings included
   * into the mechanism
   */
  if (rv == CKR_OK) {
  ...
  if (rv == CKR_OK) {
    ...
    rv = C_Verify(session, (CK_BYTE_PTR)input_align_combined_buf, size,
                  (CK_BYTE_PTR)&expectedCmac, 4);
  }
...
SecureElementStatus_t SecureElementAesEncrypt(uint8_t *buffer, uint32_t size,
                                              KeyIdentifier_t keyID,
                                              uint8_t *encBuffer) {
  ...
  /* Configure session to encrypt message in AES ECB with settings included into
   * the mechanism */
  if (rv == CKR_OK) {
  ...
  if (rv == CKR_OK) {
    ...
    rv =
        C_EncryptFinal(session, &dummy_tag[0], (CK_ULONG_PTR)&dummy_tag_length);
  }
...
SecureElementDeriveAndStoreKey(uint8_t *input, KeyIdentifier_t rootKeyID,
  ...
  uint32_t specific_label[] = {GlobalTemplateLabel, 0UL};
  CK_ATTRIBUTE DeriveKey_template = {CKA_LABEL, (CK_VOID_PTR)specific_label,
                                     sizeof(specific_label)};

  /* Derive key */
  if (SECURE_ELEMENT_SUCCESS !=
      GetKeyIndexByID(rootKeyID, &rootkey_object_handle)) {
    return SECURE_ELEMENT_ERROR;
  }

  if (SECURE_ELEMENT_SUCCESS !=
      GetKeyIndexByID(targetKeyID, &derived_object_handle)) {
    return SECURE_ELEMENT_ERROR;
  }

  if (SECURE_ELEMENT_SUCCESS !=
      SecureElementDeleteDynamicKey(targetKeyID, &specific_label[1])) {
    return SECURE_ELEMENT_ERROR;
  }
  ...
  if (rv == CKR_OK) {
    rv = C_DeriveKey(session, &(mech), rootkey_object_handle,
                     &DeriveKey_template, GET_NB_ATTRIBUTE(DeriveKey_template),
                     &derived_object_handle);
  }
...
SecureElementStatus_t SecureElementProcessJoinAccept(
  ...
  *versionMinor = ((decJoinAccept[11] & 0x80) == 0x80) ? 1 : 0;
  ...
  uint32_t mic =
      GET_UINT32_LE(decJoinAccept, encJoinAcceptSize - LORAMAC_MIC_FIELD_SIZE);
  ...
  if (*versionMinor == 0) {
     ...
     * JoinNonce | NetID | DevAddr | DLSettings | RxDelay | CFList | CFListType)
...
SecureElementStatus_t SecureElementSetDevEui(uint8_t *devEui) {
  ...
  status =
      SecureElementGetKeyByID(DEV_JOIN_EUI_ADDR_KEY, (uint8_t *)&KMSKeyBlob);
  if ((status == SECURE_ELEMENT_SUCCESS) &&
      (memcmp(KMSKeyBlob.DevEui, devEui, SE_EUI_SIZE) != 0)) {
...
SecureElementStatus_t SecureElementSetJoinEui(uint8_t *joinEui) {
  ...
  status =
      SecureElementGetKeyByID(DEV_JOIN_EUI_ADDR_KEY, (uint8_t *)&KMSKeyBlob);
  if ((status == SECURE_ELEMENT_SUCCESS) &&
      (memcmp(KMSKeyBlob.JoinEui, joinEui, SE_EUI_SIZE) != 0)) {
...
SecureElementStatus_t SecureElementSetDevAddr(ActivationType_t mode,
                                              uint32_t devAddr) {
#if (LORAWAN_KMS == 0)
...
SecureElementStatus_t SecureElementGetDevAddr(ActivationType_t mode,
                                              uint32_t *devAddr) {
  if (devAddr == NULL) {
    return SECURE_ELEMENT_ERROR_NPE;
  }
