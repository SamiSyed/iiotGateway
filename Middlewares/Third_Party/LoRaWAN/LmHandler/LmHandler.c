 ...
 * \note    Compliance test protocol callbacks used when TS001-1.0.4 +
...
LmHandlerErrorStatus_t LmHandlerInit(LmHandlerCallbacks_t *handlerCallbacks,
  ...
  LmhpComplianceParams.OnTxPeriodicityChanged =
      LmHandlerCallbacks->OnTxPeriodicityChanged;
  LmhpComplianceParams.OnTxFrameCtrlChanged = LmHandlerOnTxFrameCtrlChanged;
  LmhpComplianceParams.OnPingSlotPeriodicityChanged =
      LmHandlerOnPingSlotPeriodicityChanged;
#endif /* LORAMAC_VERSION */

  /*The LoRa-Alliance Compliance protocol package should always be initialized
   * and activated.*/
  if (LmHandlerPackageRegister(PACKAGE_ID_COMPLIANCE, &LmhpComplianceParams) !=
      LORAMAC_HANDLER_SUCCESS) {
    return LORAMAC_HANDLER_ERROR;
  }

  if (LmhpPackagesRegistrationInit((Version_t *)&fwVersion) !=
      LORAMAC_HANDLER_SUCCESS) {
    return LORAMAC_HANDLER_ERROR;
  }
...
LmHandlerErrorStatus_t LmHandlerConfigure(LmHandlerParams_t *handlerParams) {
  ...
  memcpy1((void *)&LmHandlerParams, (const void *)handlerParams,
          sizeof(LmHandlerParams_t));
  ...
  if (0U == ((1 << (LmHandlerParams.ActiveRegion)) & (loraInfo->Region))) {
    MW_LOG(TS_ON, VLEVEL_ALWAYS,
           "error: Region is not defined in the MW: set lorawan_conf.h "
           "accordingly\r\n");
    while (1) {
    } /* error: Region is not defined in the MW */
  }

  if (LoRaMacInitialization(&LoRaMacPrimitives, &LoRaMacCallbacks,
                            LmHandlerParams.ActiveRegion) !=
      LORAMAC_STATUS_OK) {
    return LORAMAC_HANDLER_ERROR;
  }

#if (defined(CONTEXT_MANAGEMENT_ENABLED) && (CONTEXT_MANAGEMENT_ENABLED == 1))
  /* Try the restore context from the Backup RAM structure if data retention is
   * available */
  ...
  if (LoRaMacMibSetRequestConfirm(&mibReq) == LORAMAC_STATUS_OK) {
    ...
    if (LmHandlerCallbacks->OnRestoreContextRequest != NULL) {
      ...
      LmHandlerCallbacks->OnRestoreContextRequest(mibReq.Param.BackupContexts,
  ...
  if (SecureElementInitMcuID(LoRaMacCallbacks.GetUniqueId,
                             LoRaMacCallbacks.GetDevAddress) !=
...
void LmHandlerProcess(void) {
  /* Call at first the LoRaMAC process before to run all package process
   * features */
  ...
  if (IsUplinkTxPending == true) {
    ...
    if (LmHandlerSend(&appData, LmHandlerParams.IsTxConfirmed, false) ==
        LORAMAC_HANDLER_SUCCESS) {
      IsUplinkTxPending = false;
    }
  }
...
static void MlmeConfirm(MlmeConfirm_t *mlmeConfirm) {
  ...
  case MLME_JOIN: {
    ...
    mibReq.Type = MIB_DEV_ADDR;
    LoRaMacMibGetRequestConfirm(&mibReq);
    if (SecureElementSetDevAddr(JoinParams.Mode, mibReq.Param.DevAddr) ==
        SECURE_ELEMENT_SUCCESS) {
      CommissioningParams.DevAddr = mibReq.Param.DevAddr;
    }
...
static void MlmeIndication(MlmeIndication_t *mlmeIndication,
  ...
  if ((LmHandlerCallbacks->OnRxData != NULL) &&
      ...
      (mlmeIndication->MlmeIndication != MLME_BEACON_LOST)) {
...
LmHandlerErrorStatus_t LmHandlerPackageRegister(uint8_t id, void *params) {
  ...
  if (package != NULL) {
    ...
    LmHandlerPackages[id]->OnPackageProcessEvent =
    ...
    if (LmHandlerPackages[i] != NULL) {
      ...
      case PACKAGE_MCPS_INDICATION: {
        ...
        if ((LmHandlerPackages[i]->OnMcpsIndicationProcess != NULL) &&
            ((LmHandlerPackages[i]->Port ==
              ((McpsIndication_t *)params)->Port) ||
             ((i == PACKAGE_ID_COMPLIANCE) &&
              (LmHandlerPackages[PACKAGE_ID_COMPLIANCE]->IsRunning()))))
#elif (defined(LORAMAC_VERSION) &&                                             \
       ((LORAMAC_VERSION == 0x01000400) || (LORAMAC_VERSION == 0x01010100)))
        if (LmHandlerPackages[i]->OnMcpsIndicationProcess != NULL)
#endif /* LORAMAC_VERSION */
...
LmHandlerErrorStatus_t LmHandlerGetVersion(LmHandlerVersionType_t lmhType,
...
LmHandlerErrorStatus_t LmHandlerSetKey(KeyIdentifier_t keyID, uint8_t *key) {
  /* Not yet joined */
  if (LmHandlerJoinStatus() != LORAMAC_HANDLER_SET) {
    if (keyID == APP_KEY) /* Specific usage because APP_KEY is used to derive
                             other keys */
    {
      MibRequestConfirm_t mibReq;
...
LmHandlerErrorStatus_t LmHandlerNvmDataStore(void) {
  ...
  if (lmhStatus == LORAMAC_HANDLER_SUCCESS) {
    ...
    } else if ((status != NVM_DATA_OK) ||
               (LmHandlerCallbacks->OnStoreContextRequest == NULL)) {
      lmhStatus = LORAMAC_HANDLER_ERROR;
    } else {
      MibRequestConfirm_t mibReq;
      mibReq.Type = MIB_NVM_CTXS;
  ...
  if ((lmhStatus == LORAMAC_HANDLER_SUCCESS) &&
      (LmHandlerCallbacks->OnNvmDataChange != NULL)) {
