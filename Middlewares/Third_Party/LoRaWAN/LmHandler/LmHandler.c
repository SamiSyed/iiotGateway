...
static LmHandlerJoinParams_t JoinParams = {.CommissioningParams = &CommissioningParams,
...
LmHandlerErrorStatus_t LmHandlerConfigure(LmHandlerParams_t *handlerParams)
{
    ...
    if (LoRaMacInitialization(&LoRaMacPrimitives, &LoRaMacCallbacks, LmHandlerParams.ActiveRegion)
        != LORAMAC_STATUS_OK)
    {
        return LORAMAC_HANDLER_ERROR;
    }
    ...
    if (SecureElementInitMcuID(LoRaMacCallbacks.GetUniqueId, LoRaMacCallbacks.GetDevAddress)
                                     ...
                                     bool allowDelayedTx)
{
    ...
    mcpsReq.Type
        = (isTxConfirmed == LORAMAC_HANDLER_UNCONFIRMED_MSG) ? MCPS_UNCONFIRMED : MCPS_CONFIRMED;
...
static void MlmeIndication(MlmeIndication_t *mlmeIndication, LoRaMacRxStatus_t *rxStatus)
{
    ...
    if ((LmHandlerCallbacks->OnRxData != NULL) && (mlmeIndication->MlmeIndication != MLME_BEACON)
...
static void LmHandlerPackagesProcess(void)
{
    for (int8_t i = 0; i < PKG_MAX_NUMBER; i++)
    {
        if ((LmHandlerPackages[i] != NULL) && (LmHandlerPackages[i]->Process != NULL)
