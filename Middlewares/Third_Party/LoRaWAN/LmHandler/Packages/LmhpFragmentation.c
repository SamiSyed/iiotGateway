...
static void LmhpFragmentationOnMcpsIndication(McpsIndication_t *mcpsIndication)
{
    ...
    while (cmdIndex < mcpsIndication->BufferSize)
    {
        ...
        case FRAGMENTATION_FRAG_STATUS_REQ:
        {
                    ...
                    && (FragSessionData[fragIndex].FragDecoderStatus.FragNbLost > 0)))
            {
                ...
                /* Fetch the co-efficient value required to calculate delay of that respective
                 * session. */
                BlockAckDelay
                    = FragSessionData[fragIndex].FragGroupData.Control.Fields.BlockAckDelay;
        ...
        case FRAGMENTATION_FRAG_SESSION_SETUP_REQ:
        {
            ...
            fragSessionData.FragGroupData.Mic
                = (mcpsIndication->Buffer[cmdIndex++] << 0) & 0x000000FF;
            fragSessionData.FragGroupData.Mic
                += (mcpsIndication->Buffer[cmdIndex++] << 8) & 0x0000FF00;
            fragSessionData.FragGroupData.Mic
            ...
            if ((fragSessionData.FragGroupData.FragNb > FRAG_MAX_NB)
                ...
                || ((fragSessionData.FragGroupData.FragNb * fragSessionData.FragGroupData.FragSize)
                    > FRAG_DECODER_DWL_REGION_SIZE))
            {
            ...
            if ((status & 0x1F) == 0)
            {
                ...
                FragSessionData[fragSessionData.FragGroupData.FragSession.Fields.FragIndex]
                    = fragSessionData;
        ...
        case FRAGMENTATION_DATA_FRAGMENT:
        {
            ...
            {
                FragSessionData[fragIndex].FragDecoderProcessStatus
                ...
                if (FragSessionData[fragIndex].FragDecoderProcessStatus >= 0)
                {
                    ...
                    if (FragSessionData[fragIndex].FragGroupData.Control.Fields.AckReception == 1)
                    {
                        ...
                        /* Compute MIC */
                        LoRaMacProcessMicForDatablock(
                            (uint8_t *)UnfragmentedBufferAddr,
                            (FragSessionData[fragIndex].FragGroupData.FragNb
                             * FragSessionData[fragIndex].FragGroupData.FragSize)
                                - FragSessionData[fragIndex].FragGroupData.Padding,
                        ...
                        LmhpFragmentationState.DataBuffer[dataBufferIndex++]
                            = FRAGMENTATION_FRAG_DATA_BLOCK_RECEIVED_REQ;
                        LmhpFragmentationState.DataBuffer[dataBufferIndex++] = status;
                        BlockAckDelay
                            = FragSessionData[fragIndex].FragGroupData.Control.Fields.BlockAckDelay;
                        isAnswerDelayed = true;
    ...
    /* After processing the commands, if the end-node has to reply back then a flag is checked if
     * the */
    ...
    if (dataBufferIndex != 0)
    {
        ...
        {
            /* Delay value is calculated using BlockAckDelay which is communicated by server during
             * the FragSessionSetupReq Pseudo Random Delay = rand(0:1) * 2^(BlockAckDelay + 4)
             * Seconds. Delay = Pseudo Random Delay * 1000 milli seconds. Eg: BlockAckDelay = 7
