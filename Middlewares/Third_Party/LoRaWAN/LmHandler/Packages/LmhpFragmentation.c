...
LmhpFragmentationOnMcpsIndication(McpsIndication_t *mcpsIndication) {
  ...
  while (cmdIndex < mcpsIndication->BufferSize) {
    ...
    case FRAGMENTATION_FRAG_SESSION_SETUP_REQ: {
      ...
      if ((status & 0x0F) == 0) {
#elif (FRAGMENTATION_VERSION == 2)
      if (SessionCntPrev[fragSessionData.FragGroupData.FragSession.Fields
                             .FragIndex] >=
          fragSessionData.FragGroupData.SessionCnt) {
    ...
    case FRAGMENTATION_FRAG_DATA_BLOCK_RECEIVED_ANS: {
      ...
      if ((FragSessionData[fragIndex]
               .FragGroupData.FragSession.Fields.FragIndex == fragIndex) &&
          (LmhpFragmentationState.FragDataBlockAnsRequired == true)) {
    ...
    case FRAGMENTATION_DATA_FRAGMENT: {
      ...
      if (mcpsIndication->Multicast == 1) {
        ...
        uint8_t groupId =
            LoRaMacMcChannelGetGroupId(mcpsIndication->DevAddress);
        if ((groupId == 0xFF) ||
            ((FragSessionData[fragIndex]
                  .FragGroupData.FragSession.Fields.McGroupBitMask &
              (1 << groupId)) == 0)) {
          /* Ignore message */
          cmdIndex = mcpsIndication->BufferSize;
          break;
      ...
      if (FragSessionData[fragIndex].FragDecoderProcessStatus ==
        ...
        if (FragSessionData[fragIndex].FragDecoderProcessStatus >= 0) {
          ...
          if (FragSessionData[fragIndex]
                  .FragGroupData.Control.Fields.AckReception == 1) {
            uint8_t status = 0x00;
            ...
            LoRaMacProcessMicForDatablock(
                ...
                (FragSessionData[fragIndex].FragGroupData.FragNb *
                 FragSessionData[fragIndex].FragGroupData.FragSize) -
                    FragSessionData[fragIndex].FragGroupData.Padding,
                FragSessionData[fragIndex].FragGroupData.SessionCnt, fragIndex,
  ...
  /* In some scenarios it is not desired that multiple end-notes send uplinks at
   * the same time to
   */
  ...
  if (dataBufferIndex != 0) {
    ...
    if (isAnswerDelayed == true) {
       ...
       * seconds. Eg: BlockAckDelay = 7 Pseudo Random Delay = rand(0:1) * 2^11
