...
typedef enum eSessionState { SESSION_STOPPED, SESSION_STARTED } SessionState_t;
...
static void LmhpRemoteMcastSetupProcess(void) {
  ...
  case REMOTE_MCAST_SETUP_SESSION_STATE_START:
    ...
    if (((McSessionData[id].RxParams.Class == CLASS_B) &&
      ...
      if (LmHandlerRequestClass(McSessionData[id].RxParams.Class) ==
    ...
    case REMOTE_MCAST_SETUP_MC_GROUP_SETUP_REQ: {
      ...
      if (id < LORAMAC_MAX_MC_CTX) {
        ...
        McChannelParams_t channel = {
            ...
            .RxParams.Params
                .ClassC = /* Field not used for multicast channel setup. Must be
  ...
  if (dataBufferIndex != 0) {
    ...
    if (id != 0xFF && id < LORAMAC_MAX_MC_CTX) {
      ...
      MW_LOG(TS_OFF, VLEVEL_M, "McAddr      : %08X\r\n",
             McSessionData[id].McGroupData.McAddr);
      MW_LOG(TS_OFF, VLEVEL_M, "McKey       : %02X",
             McSessionData[id].McGroupData.McKeyEncrypted[0]);
      for (int32_t i = 1; i < 16; i++) {
        MW_LOG(TS_OFF, VLEVEL_M, "-%02X",
               McSessionData[id].McGroupData.McKeyEncrypted[i]);
      }
