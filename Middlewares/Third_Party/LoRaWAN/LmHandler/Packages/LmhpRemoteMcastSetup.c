    ...
    case REMOTE_MCAST_SETUP_SESSION_STATE_START:
        ...
        else
        {
            ...
            if (LmHandlerRequestClass(McSessionData[id].RxParams.Class) == LORAMAC_HANDLER_SUCCESS)
            {
                ...
                {
                    TimerSetValue(&SessionStopTimer[id],
                                  (1 << McSessionData[id].SessionTimeout) * 1000 * 128);
                }
                else /* CLASS_C */
                {
                    TimerSetValue(&SessionStopTimer[id],
                                  (1 << McSessionData[id].SessionTimeout) * 1000);
                }
...
static void LmhpRemoteMcastSetupOnMcpsIndication(McpsIndication_t *mcpsIndication)
{
    ...
    while (cmdIndex < mcpsIndication->BufferSize)
    {
        ...
        case REMOTE_MCAST_SETUP_MC_GROUP_STATUS_REQ:
        {
            ...
            LmhpRemoteMcastSetupState.DataBuffer[dataBufferIndex++]
                = REMOTE_MCAST_SETUP_MC_GROUP_STATUS_ANS;
            ...
            for (id = 0; id < LORAMAC_MAX_MC_CTX; id++)
            {
                if (McSessionData[id].McGroupData.McGroupEnabled)
                {
                    ...
                    if ((reqGroupMask & (1 << id)) != 0)
                    {
                        ...
                        LmhpRemoteMcastSetupState.DataBuffer[dataBufferIndex++]
                            = (McSessionData[id].McGroupData.McAddr >> 0) & 0xFF;
                        LmhpRemoteMcastSetupState.DataBuffer[dataBufferIndex++]
                            = (McSessionData[id].McGroupData.McAddr >> 8) & 0xFF;
                        LmhpRemoteMcastSetupState.DataBuffer[dataBufferIndex++]
                            = (McSessionData[id].McGroupData.McAddr >> 16) & 0xFF;
                        LmhpRemoteMcastSetupState.DataBuffer[dataBufferIndex++]
                            = (McSessionData[id].McGroupData.McAddr >> 24) & 0xFF;
                    }
                }
            }
            /* set the status bit */
            LmhpRemoteMcastSetupState.DataBuffer[1]
                = (nbAvailableGroups & 0x07) << 4 | (AnsGroupMask & 0x0F);
        ...
        case REMOTE_MCAST_SETUP_MC_GROUP_SETUP_REQ:
        {
            ...
            if (id < LORAMAC_MAX_MC_CTX)
            {
                ...
                McChannelParams_t channel = {
                    ...
                    .IsEnabled = true,
                    .GroupID
                    = (AddressIdentifier_t)McSessionData[id].McGroupData.IdHeader.Fields.McGroupId,
                    ...
                    .RxParams.Params.ClassC = /* Field not used for multicast channel setup. Must be
            ...
            LmhpRemoteMcastSetupState.DataBuffer[dataBufferIndex++]
                = (idError << 2) | McSessionData[id].McGroupData.IdHeader.Fields.McGroupId;
            break;
        }
        ...
        case REMOTE_MCAST_SETUP_MC_GROUP_CLASS_B_SESSION_REQ:
        {
            ...
            if (id < LORAMAC_MAX_MC_CTX)
            {
                ...
                McSessionData[id].RxParams.Params.ClassB.Periodicity
                    = (mcpsIndication->Buffer[cmdIndex] >> 4) & 0x07;
                McSessionData[id].SessionTimeout = mcpsIndication->Buffer[cmdIndex++] & 0x0F;
            ...
            LmhpRemoteMcastSetupState.DataBuffer[dataBufferIndex++]
                = REMOTE_MCAST_SETUP_MC_GROUP_CLASS_B_SESSION_ANS;
            LmhpRemoteMcastSetupState.DataBuffer[dataBufferIndex++] = status;
            if (isTimerSet == true)
            {
                LmhpRemoteMcastSetupState.DataBuffer[dataBufferIndex++]
    ...
    {
        /* Answer commands */
        LmHandlerAppData_t appData = {.Buffer = LmhpRemoteMcastSetupState.DataBuffer,
        ...
        if (id != 0xFF && id < LORAMAC_MAX_MC_CTX)
        {
            ...
            MW_LOG(TS_OFF,
                   VLEVEL_M,
                   "McKey       : %02X",
                   McSessionData[id].McGroupData.McKeyEncrypted[0]);
