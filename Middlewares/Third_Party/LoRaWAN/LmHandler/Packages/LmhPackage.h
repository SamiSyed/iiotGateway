...
typedef struct LmhPackage_s {
   ...
   * \param [in] forceRejoin Flag to force the rejoin even if LoRaWAN context
   * can be restored
   */
   ...
   * \param [IN] isTxConfirmed Indicates if the uplink requires an
   * acknowledgement
   *
   * \retval status Returns \ref LORAMAC_HANDLER_SUCCESS if request has been
   *                processed else \ref LORAMAC_HANDLER_ERROR
   */
  LmHandlerErrorStatus_t (*OnSendRequest)(LmHandlerAppData_t *appData,
