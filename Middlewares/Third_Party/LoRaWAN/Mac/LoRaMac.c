 ...
 * LoRaWAN version \param[IN] maxFCntGap            - Maximum allowed frame
 * counter difference (only for 1.0.X necessary) \param[OUT]    fCntID - Frame
 * counter identifier \param[OUT]    currentDown           - Current downlink
 * counter value
 *
...
GetFCntDown(AddressIdentifier_t addrID, FType_t fType,
            ...
            uint16_t maxFCntGap, FCntIdentifier_t *fCntID,
 ...
 * \param [in]    macMsg                - Data message object, holding the
 * current 16 bit transmitted frame counter \param [in]    lrWanVersion -
 * LoRaWAN version \param [out] fCntID                - Frame counter identifier
 ...
 */
static LoRaMacStatus_t Send(LoRaMacHeader_t *macHdr, uint8_t fPort,
...
static LoRaMacStatus_t PrepareFrame(LoRaMacHeader_t *macHdr,
                                    LoRaMacFrameCtrl_t *fCtrl, uint8_t fPort,
                                    void *fBuffer, uint16_t fBufferSize);
 ...
 */
static LoRaMacStatus_t SetTxContinuousWave1(uint16_t timeout,
                                            uint32_t frequency, uint8_t power);
#elif (defined(LORAMAC_VERSION) &&                                             \
       ((LORAMAC_VERSION == 0x01000400) || (LORAMAC_VERSION == 0x01010100)))
/*!
 * \brief Sets the radio in continuous transmission mode
 ...
 * RF output power to be set. \retval status          Status of the operation.
 */
static LoRaMacStatus_t SetTxContinuousWave(uint16_t timeout, uint32_t frequency,
                                           uint8_t power);
#endif /* LORAMAC_VERSION */
#if (defined(LORAMAC_VERSION) && (LORAMAC_VERSION == 0x01010100))
/*!
 ...
 *          \ref LORAMAC_STATUS_PARAMETER_INVALID,
 */
static LoRaMacStatus_t DetermineFrameType(LoRaMacMessageData_t *macMsg,
                                          FType_t *fType);
...
/*!
 * \brief Calls the MacProcessNotify callback to indicate that a LoRaMacProcess
 * call is pending
 */
...
static void ProcessRadioTxDone(void) {
  ...
  if ((Nvm.MacGroup2.DeviceClass == CLASS_C) ||
    ...
    TimerSetValue(&MacCtx.AckTimeoutTimer,
  ...
  if (MacCtx.NodeAckRequested == true) {
    ...
    phyParam = RegionGetPhyParam(Nvm.MacGroup2.Region, &getPhy);
    TimerSetValue(&MacCtx.RetransmitTimeoutTimer,
                  MacCtx.RxWindow2Delay + phyParam.Value);
...
static void ProcessRadioRxDone(void) {
  ...
  case FRAME_TYPE_JOIN_ACCEPT: {
    ...
    // Abort in case if the device is already joined and no rejoin request is
    // ongoing.
#if (defined(LORAMAC_VERSION) && (LORAMAC_VERSION == 0x01010100))
    if ((Nvm.MacGroup2.NetworkActivation != ACTIVATION_TYPE_NONE) &&
    ...
    macCryptoStatus =
        LoRaMacCryptoHandleJoinAccept(JOIN_REQ, joinEui, &macMsgJoinAccept);

#if (defined(LORAMAC_VERSION) && (LORAMAC_VERSION == 0x01010100))
    if (LORAMAC_CRYPTO_SUCCESS != macCryptoStatus) {
      macCryptoStatus = LoRaMacCryptoHandleJoinAccept(REJOIN_REQ_0, joinEui,
                                                      &macMsgJoinAccept);
      joinType = MLME_REJOIN_0;
    }

    if (LORAMAC_CRYPTO_SUCCESS != macCryptoStatus) {
      macCryptoStatus = LoRaMacCryptoHandleJoinAccept(REJOIN_REQ_1, joinEui,
                                                      &macMsgJoinAccept);
      joinType = MLME_REJOIN_1;
    }

    if (LORAMAC_CRYPTO_SUCCESS != macCryptoStatus) {
      macCryptoStatus = LoRaMacCryptoHandleJoinAccept(REJOIN_REQ_2, joinEui,
                                                      &macMsgJoinAccept);
    ...
    verifyRxDr.DatarateParams.Datarate =
        macMsgJoinAccept.DLSettings.Bits.RX2DataRate;
    verifyRxDr.DatarateParams.DownlinkDwellTime =
  ...
  case FRAME_TYPE_DATA_UNCONFIRMED_DOWN:
    ...
    macCryptoStatus =
        GetFCntDown(addrID, fType, &macMsgData, Nvm.MacGroup2.Version,
                    phyParam.Value, &fCntID, &downLinkCounter);
     ...
     ((LORAMAC_VERSION == 0x01000400) || (LORAMAC_VERSION == 0x01010100)))
        ...
        if ((MacCtx.RxStatus.RxSlot != RX_SLOT_WIN_1) &&
          ...
          MacCtx.McpsIndication.ResponseTimeout =
              Nvm.MacGroup2.MacParams.RxBCTimeout;
    ...
    RemoveMacCommands(MacCtx.RxStatus.RxSlot, macMsgData.FHDR.FCtrl,
...
static void OnRxWindow1TimerEvent(void *context) {
  ...
  MacCtx.RxWindow1Config.DownlinkDwellTime =
      Nvm.MacGroup2.MacParams.DownlinkDwellTime;
  MacCtx.RxWindow1Config.RepeaterSupport =
...
static void OnRxWindow2TimerEvent(void *context) {
  ...
  MacCtx.RxWindow2Config.DownlinkDwellTime =
      Nvm.MacGroup2.MacParams.DownlinkDwellTime;
  MacCtx.RxWindow2Config.RepeaterSupport =
      Nvm.MacGroup2.MacParams.RepeaterSupport;
...
GetFCntDown(AddressIdentifier_t addrID, FType_t fType,
  ...
  return LoRaMacCryptoGetFCntDown(*fCntID, maxFCntGap, macMsg->FHDR.FCnt,
                                  currentDown);
}
#elif (defined(LORAMAC_VERSION) &&                                             \
       ((LORAMAC_VERSION == 0x01000400) || (LORAMAC_VERSION == 0x01010100)))
      ...
      for (int8_t i = 0; i < LORAMAC_MAX_MC_CTX; i++) {
        if ((Nvm.MacGroup2.MulticastChannelList[i].ChannelParams.IsEnabled ==
            ...
            (Nvm.MacGroup2.MulticastChannelList[i]
                 .ChannelParams.RxParams.Class == CLASS_C)) {
...
#if (defined(LORAMAC_VERSION) &&                                               \
     ((LORAMAC_VERSION == 0x01000400) || (LORAMAC_VERSION == 0x01010100)))
        // In case of other receive windows the Device Time Answer is not
        // received.
        MacCtx.McpsIndication.DeviceTimeAnsReceived = false;
#endif /* LORAMAC_VERSION */
      }
...
static LoRaMacStatus_t Send(LoRaMacHeader_t *macHdr, uint8_t fPort,
  ...
  fCtrl.Bits.AdrAckReq =
      LoRaMacAdrCalcNext(&adrNext, &Nvm.MacGroup1.ChannelsDatarate,
...
static LoRaMacStatus_t SerializeTxFrame(void) {
  ...
  case LORAMAC_MSG_TYPE_RE_JOIN_1:
    serializeStatus =
        LoRaMacSerializerReJoinType1(&MacCtx.TxMsg.Message.ReJoin1);
    if (LORAMAC_SERIALIZER_SUCCESS != serializeStatus) {
      return LORAMAC_STATUS_CRYPTO_ERROR;
    }
  ...
  case LORAMAC_MSG_TYPE_RE_JOIN_0_2:
    serializeStatus =
        LoRaMacSerializerReJoinType0or2(&MacCtx.TxMsg.Message.ReJoin0or2);
    if (LORAMAC_SERIALIZER_SUCCESS != serializeStatus) {
      return LORAMAC_STATUS_CRYPTO_ERROR;
    }
...
static void ResetMacParameters(bool isRejoin) {
  ...
  Nvm.MacGroup2.MacParams.AdrAckLimit =
      Nvm.MacGroup2.MacParamsDefaults.AdrAckLimit;
  Nvm.MacGroup2.MacParams.AdrAckDelay =
      Nvm.MacGroup2.MacParamsDefaults.AdrAckDelay;
...
static bool CheckRetransUnconfirmedUplink(void) {
  ...
  if (MacCtx.ChannelsNbTransCounter >=
    ...
    if (Nvm.MacGroup2.DeviceClass == CLASS_A) {
      return true;
    } else { // For Class B & C stop only if the frame was received in RX1
             // window
      if (MacCtx.RxStatus.RxSlot == RX_SLOT_WIN_1) {
        return true;
      }
...
static bool StopRetransmission(void) {
  ...
  if (Nvm.MacGroup2.Version.Fields.Minor >= 1) {
    ...
    if (LoRaMacCommandsGetCmd(MOTE_MAC_REKEY_IND, &macCmd) ==
       ...
       */
      if (Nvm.MacGroup1.RekeyIndUplinksCounter ==
          Nvm.MacGroup2.MacParams.AdrAckLimit) {
        Nvm.MacGroup2.NetworkActivation = ACTIVATION_TYPE_NONE;
...
LoRaMacStatus_t LoRaMacMibGetRequestConfirm(MibRequestConfirm_t *mibGet) {
  ...
  case MIB_DEV_ADDR: {
    SecureElementGetDevAddr(Nvm.MacGroup2.NetworkActivation,
...
LoRaMacStatus_t LoRaMacMibSetRequestConfirm(MibRequestConfirm_t *mibSet) {
  ...
  case MIB_DEV_ADDR: {
    if (SecureElementSetDevAddr(Nvm.MacGroup2.NetworkActivation,
                                mibSet->Param.DevAddr) !=
        SECURE_ELEMENT_SUCCESS) {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
    } else {
  ...
  case MIB_NWK_S_KEY: {
    if (mibSet->Param.NwkSKey != NULL) {
      if (LORAMAC_CRYPTO_SUCCESS !=
          LoRaMacCryptoSetKey(NWK_S_KEY, mibSet->Param.NwkSKey)) {
        return LORAMAC_STATUS_CRYPTO_ERROR;
      }
    } else {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
  ...
  case MIB_REPEATER_SUPPORT: {
    Nvm.MacGroup2.MacParams.RepeaterSupport =
        mibSet->Param.EnableRepeaterSupport;
    break;
  }
  case MIB_RX2_CHANNEL: {
  ...
  *status &= 0x0F; // groupID OK
  ...
  if (rxParams->Class == CLASS_B) {
    ...
    LoRaMacClassBSetMulticastPeriodicity(
        &Nvm.MacGroup2.MulticastChannelList[groupID]);
  }
  return LORAMAC_STATUS_OK;
}
...
LoRaMacStatus_t LoRaMacProcessMicForDatablock(uint8_t *buffer, uint32_t size,
  ...
  macCryptoStatus = LoRaMacCryptoComputeDataBlock(buffer, size, sessionCnt,
                                                  fragIndex, descriptor, mic);
  if (LORAMAC_CRYPTO_SUCCESS != macCryptoStatus) {
    return LORAMAC_STATUS_CRYPTO_ERROR;
  }
...
LoRaMacStatus_t LoRaMacMlmeRequest(MlmeReq_t *mlmeRequest) {
  ...
  case MLME_DEVICE_TIME: {
    ...
    status = LORAMAC_STATUS_OK;
    MacCommand_t *newCmd;
    /* ST_CODE Begin: Add MAC command condition to prevent some duplicated
     * request */
    if (LoRaMacCommandsGetCmd(MOTE_MAC_DEVICE_TIME_REQ, &newCmd) ==
        LORAMAC_COMMANDS_SUCCESS) {
      status = LORAMAC_STATUS_OK;
    }
    /* ST_CODE End */
    else if (LoRaMacCommandsAddCmd(MOTE_MAC_DEVICE_TIME_REQ, macCmdPayload,
                                   0) != LORAMAC_COMMANDS_SUCCESS) {
      status = LORAMAC_STATUS_MAC_COMMAD_ERROR;
    }
    break;
  }
...
LoRaMacStatus_t LoRaMacMcpsRequest(McpsReq_t *mcpsRequest,
  ...
  case MCPS_CONFIRMED: {
    ...
    MacCtx.AckTimeoutRetries =
        MIN(mcpsRequest->Req.Confirmed.NbTrials, MAX_ACK_RETRIES);
