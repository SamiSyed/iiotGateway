...
#ifndef HYBRID_DEFAULT_MASK0        /*enabled channels from channel 15 down to \
                                       channel 0*/
#define HYBRID_DEFAULT_MASK0 0x00FF /*channel 7 down to channel 0  enabled*/
#endif
#ifndef HYBRID_DEFAULT_MASK1 /*enabled channels from channel 31 down to        \
                                channel 16*/
#define HYBRID_DEFAULT_MASK1 0x0000
#endif
#ifndef HYBRID_DEFAULT_MASK2 /*enabled channels from channel 47 down to        \
                                channel 32*/
#define HYBRID_DEFAULT_MASK2 0x0000
#endif
#ifndef HYBRID_DEFAULT_MASK3 /*enabled channels from channel 63 down to        \
                                channel 48*/
#define HYBRID_DEFAULT_MASK3 0x0000
#endif
#ifndef HYBRID_DEFAULT_MASK4 /*enabled channels from channel 79 down to        \
                                channel 64*/
#define HYBRID_DEFAULT_MASK4 0x0000
#endif
#ifndef HYBRID_DEFAULT_MASK5 /*enabled channels from channel 95 down to        \
                                channel 80*/
...
typedef struct sRegionCN470ChannelPlanCtx {
  ...
  uint32_t (*GetDownlinkFrequency)(uint8_t channel, uint8_t joinChannelIndex,
                                   bool isPingSlot);
  /*!
   * \brief Performs the update of the channelsMask based on the input
   * parameters.
   *
   * \param [in] joinChannelIndex The join channel index.
   *
...
// Static functions
static void ApplyChannelPlanConfig(RegionCN470ChannelPlan_t channelPlan,
                                   RegionCN470ChannelPlanCtx_t *ctx) {
  switch (channelPlan) {
...
static bool VerifyRfFreq(uint32_t freq) {
  ...
  // Rx frequencies
  if ((freq < CN470_FIRST_RX1_CHANNEL) || (freq > CN470_LAST_RX1_CHANNEL) ||
      (((freq - (uint32_t)CN470_FIRST_RX1_CHANNEL) %
        (uint32_t)CN470_STEPWIDTH_RX1_CHANNEL) != 0)) {
    return false;
  }
...
static TimerTime_t GetTimeOnAir(int8_t datarate, uint16_t pktLen) {
  ...
  return Radio.TimeOnAir(MODEM_LORA, bandwidth, phyDr, 1, 8, false, pktLen,
                         true);
}
#endif /* REGION_CN470 */
...
PhyParam_t RegionCN470GetPhyParam(GetPhyParams_t *getPhy) {
  ...
  case PHY_DEF_RX2_FREQUENCY: {
    ...
    if (RegionNvmGroup2->ChannelPlan != CHANNEL_PLAN_UNKNOWN) {
      phyParam.Value = ChannelPlanCtx.GetRx2Frequency(
  ...
  case PHY_BEACON_CHANNEL_FREQ: {
    ...
    phyParam.Value = RegionBaseUSCalcDownlinkFrequency(
        getPhy->Channel, CN470_BEACON_CHANNEL_FREQ,
    ...
    if (RegionNvmGroup2->ChannelPlan != CHANNEL_PLAN_UNKNOWN) {
      phyParam.Value = ChannelPlanCtx.GetDownlinkFrequency(
          getPhy->Channel, RegionNvmGroup2->CommonJoinChannelIndex, false);
    }
#endif /* REGION_VERSION */
    break;
  }
  ...
  case PHY_PING_SLOT_CHANNEL_FREQ: {
    phyParam.Value = RegionBaseUSCalcDownlinkFrequency(
        getPhy->Channel, CN470_PING_SLOT_CHANNEL_FREQ,
        CN470_BEACON_CHANNEL_STEPWIDTH);
    break;
  }
  ...
  case PHY_BEACON_CHANNEL_OFFSET: {
    // Implementation depending on the join channel
    if (RegionNvmGroup2->ChannelPlan != CHANNEL_PLAN_UNKNOWN) {
      phyParam.Value = ChannelPlanCtx.GetBeaconChannelOffset(
          RegionNvmGroup2->CommonJoinChannelIndex);
    }
    break;
  }
  case PHY_PING_SLOT_CHANNEL_FREQ: {
    ...
    if (RegionNvmGroup2->ChannelPlan != CHANNEL_PLAN_UNKNOWN) {
      phyParam.Value = ChannelPlanCtx.GetDownlinkFrequency(
          getPhy->Channel, RegionNvmGroup2->CommonJoinChannelIndex, true);
    }
    break;
  }
  case PHY_PING_SLOT_CHANNEL_DR: {
  ...
  case PHY_BW_FROM_DR: {
    phyParam.Value =
        RegionCommonGetBandwidth(getPhy->Datarate, BandwidthsCN470);
    break;
  }
  default: {
    break;
...
void RegionCN470InitDefaults(InitDefaultsParams_t *params) {
#if defined(REGION_CN470)
  Band_t bands[CN470_MAX_NB_BANDS] = {CN470_BAND0};
  ...
  case INIT_TYPE_DEFAULTS: {
    ...
    memcpy1((uint8_t *)RegionNvmGroup1->Bands, (uint8_t *)bands,
            sizeof(Band_t) * CN470_MAX_NB_BANDS);
    ...
    RegionCommonChanMaskCopy(RegionNvmGroup1->ChannelsMaskRemaining,
                             RegionNvmGroup2->ChannelsMask, CHANNELS_MASK_SIZE);
#endif /* REGION_VERSION */
    break;
  }
  ...
  case INIT_TYPE_ACTIVATE_DEFAULT_CHANNELS: {
    ...
    for (uint8_t i = 0; i < CHANNELS_MASK_SIZE;
         i++) { // Copy-And the channels mask
      RegionNvmGroup1->ChannelsMaskRemaining[i] &=
          RegionNvmGroup2->ChannelsMask[i];
    }
#endif /* REGION_VERSION */
    break;
  }
...
bool RegionCN470Verify(VerifyParams_t *verify, PhyAttribute_t phyAttribute) {
  ...
  case PHY_TX_POWER: {
    ...
    return RegionCommonValueInRange(verify->TxPower, CN470_MAX_TX_POWER,
                                    CN470_MIN_TX_POWER);
  }
  case PHY_DUTY_CYCLE: {
...
void RegionCN470ApplyCFList(ApplyCFListParams_t *applyCFList) {
  ...
  RegionNvmGroup2->ChannelPlan =
      IdentifyChannelPlan(RegionNvmGroup2->CommonJoinChannelIndex);
  ...
  // Last byte CFListType must be 0x01 to indicate the CFList contains a series
  // of ChMask fields
...
#if (defined(REGION_VERSION) && (REGION_VERSION == 0x01010003))
  for (uint8_t chMaskItr = 0, cntPayload = 0; chMaskItr <= 5;
...
#elif (defined(REGION_VERSION) && (REGION_VERSION == 0x02010001))
  for (uint8_t chMaskItr = 0, cntPayload = 0;
       chMaskItr < ChannelPlanCtx.JoinAcceptListSize;
       chMaskItr++, cntPayload += 2) {
    RegionNvmGroup2->ChannelsMask[chMaskItr] =
        (uint16_t)(0x00FF & applyCFList->Payload[cntPayload]);
    RegionNvmGroup2->ChannelsMask[chMaskItr] |=
...
void RegionCN470ComputeRxWindowParameters(int8_t datarate, uint8_t minRxSymbols,
  ...
  rxConfigParams->Bandwidth =
      RegionCommonGetBandwidth(rxConfigParams->Datarate, BandwidthsCN470);

  tSymbolInUs = RegionCommonComputeSymbolTimeLoRa(
      DataratesCN470[rxConfigParams->Datarate],
...
bool RegionCN470RxConfig(RxConfigParams_t *rxConfig, int8_t *datarate) {
  ...
  if (rxConfig->RxSlot == RX_SLOT_WIN_1) {
    ...
    frequency = CN470_FIRST_RX1_CHANNEL +
                (rxConfig->Channel % 48) * CN470_STEPWIDTH_RX1_CHANNEL;
  }
#elif (defined(REGION_VERSION) && (REGION_VERSION == 0x02010001))
  ...
  if (rxConfig->NetworkActivation != ACTIVATION_TYPE_NONE) {
    ...
    if (rxConfig->RxSlot == RX_SLOT_WIN_1) {
      ...
      frequency = ChannelPlanCtx.GetRx2Frequency(
          RegionNvmGroup2->CommonJoinChannelIndex,
          RegionNvmGroup2->IsOtaaDevice);
    }
  } else {
  ...
  // Radio configuration
  Radio.SetRxConfig(MODEM_LORA, rxConfig->Bandwidth, phyDr, 1, 0, 8,
...
bool RegionCN470TxConfig(TxConfigParams_t *txConfig, int8_t *txPower,
  ...
  int8_t txPowerLimited = RegionCommonLimitTxPower(
      ...
      RegionNvmGroup1->Bands[RegionNvmGroup2->Channels[txConfig->Channel].Band]
          .TxMaxPower);
  uint32_t bandwidth =
      RegionCommonGetBandwidth(txConfig->Datarate, BandwidthsCN470);
  int8_t phyTxPower = 0;
  ...
  Radio.SetTxConfig(MODEM_LORA, phyTxPower, 0, bandwidth, phyDr, 1, 8, false,
                    true, 0, 0, false, 4000);
  ...
  int8_t txPowerLimited =
      RegionCommonLimitTxPower(txConfig->TxPower, RegionBands[band].TxMaxPower);
  uint32_t bandwidth =
      RegionCommonGetBandwidth(txConfig->Datarate, BandwidthsCN470);
  int8_t phyTxPower = 0;
...
uint8_t RegionCN470LinkAdrReq(LinkAdrReqParams_t *linkAdrReq, int8_t *drOut,
  ...
  RegionCommonChanMaskCopy(channelsMask, RegionNvmGroup2->ChannelsMask,
                           CHANNELS_MASK_SIZE);
    ...
    if (linkAdrParams.ChMaskCtrl == 6) {
      ...
      for (uint8_t i = 0; i < 16; i++) {
        if (((linkAdrParams.ChMask & (1 << i)) != 0) &&
            (RegionNvmGroup2->Channels[linkAdrParams.ChMaskCtrl * 16 + i]
                 .Frequency == 0)) { // Trying to enable an undefined channel
          status &= 0xFE;            // Channel mask KO
        }
      }
  ...
  while (bytesProcessed < linkAdrReq->PayloadSize) {
    ...
    status = ChannelPlanCtx.LinkAdrChMaskUpdate(
        channelsMask, linkAdrParams.ChMaskCtrl, linkAdrParams.ChMask,
        RegionNvmGroup2->Channels);
  }

  // Make sure at least one channel is active
  if (RegionCommonCountChannels(channelsMask, 0,
                                ChannelPlanCtx.ChannelsMaskSize) == 0) {
    status &= 0xFE; // Channel mask KO
  }
#endif /* REGION_VERSION */
...
#elif (defined(REGION_VERSION) && (REGION_VERSION == 0x02010001))
    // Copy Mask
    RegionCommonChanMaskCopy(RegionNvmGroup2->ChannelsMask, channelsMask,
                             CHANNELS_MASK_SIZE);

    RegionNvmGroup1->ChannelsMaskRemaining[0] &=
        RegionNvmGroup2->ChannelsMask[0];
    RegionNvmGroup1->ChannelsMaskRemaining[1] &=
        RegionNvmGroup2->ChannelsMask[1];
    RegionNvmGroup1->ChannelsMaskRemaining[2] &=
        RegionNvmGroup2->ChannelsMask[2];
    RegionNvmGroup1->ChannelsMaskRemaining[3] &=
        RegionNvmGroup2->ChannelsMask[3];
    RegionNvmGroup1->ChannelsMaskRemaining[4] =
        RegionNvmGroup2->ChannelsMask[4];
    RegionNvmGroup1->ChannelsMaskRemaining[5] =
        RegionNvmGroup2->ChannelsMask[5];
#endif /* REGION_VERSION */
  }
...
uint8_t RegionCN470RxParamSetupReq(RxParamSetupReqParams_t *rxParamSetupReq) {
  ...
  // Verify datarate
  if (RegionCommonValueInRange(rxParamSetupReq->Datarate, CN470_RX_MIN_DATARATE,
                               CN470_RX_MAX_DATARATE) == false) {
...
LoRaMacStatus_t RegionCN470NextChannel(NextChanParams_t *nextChanParams,
  ...
  if (RegionCommonCountChannels(RegionNvmGroup2->ChannelsMask, 0,
                                CHANNELS_MASK_SIZE) ==
  ...
  // Count 125kHz channels
  if (RegionCommonCountChannels(RegionNvmGroup1->ChannelsMaskRemaining, 0,
                                ChannelPlanCtx.ChannelsMaskSize) ==
      0) { // Reactivate default channels
    RegionNvmGroup2->ChannelsMask[0] = 0xFFFF;
    RegionNvmGroup2->ChannelsMask[1] = 0xFFFF;
    RegionNvmGroup2->ChannelsMask[2] = 0xFFFF;
  ...
  // In this case the device shall not follow the individual channel plans for
  // the different type, but instead shall follow the common join channel plan.
  ...
  if (status == LORAMAC_STATUS_OK) {
    ...
    *channel = enabledChannels[randr(0, nbEnabledChannels - 1)];
    ...
    RegionCommonChanDisable(RegionNvmGroup1->ChannelsMaskRemaining, *channel,
                            ChannelPlanCtx.ChannelsMaskSize);
#endif /* REGION_VERSION */
  }
  return status;
