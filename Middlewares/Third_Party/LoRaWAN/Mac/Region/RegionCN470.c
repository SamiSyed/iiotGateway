...
typedef struct sRegionCN470ChannelPlanCtx
{
     ...
     */
    uint8_t (*LinkAdrChMaskUpdate)(uint16_t *channelsMask,
                                   uint8_t chMaskCntl,
                                   uint16_t chanMask,
...
static bool VerifyRfFreq(uint32_t freq)
{
    ...
    if ((freq < CN470_FIRST_RX1_CHANNEL) || (freq > CN470_LAST_RX1_CHANNEL)
        || (((freq - (uint32_t)CN470_FIRST_RX1_CHANNEL) % (uint32_t)CN470_STEPWIDTH_RX1_CHANNEL)
            != 0))
    {
        return false;
    }
...
PhyParam_t RegionCN470GetPhyParam(GetPhyParams_t *getPhy)
{
    ...
    case PHY_DEF_RX2_FREQUENCY:
    {
        ...
        {
            phyParam.Value = ChannelPlanCtx.GetRx2Frequency(RegionNvmGroup2->CommonJoinChannelIndex,
    ...
    case PHY_BEACON_CHANNEL_FREQ:
    {
        ...
        {
            phyParam.Value
                = ChannelPlanCtx.GetDownlinkFrequency(getPhy->Channel,
    ...
    case PHY_BEACON_CHANNEL_OFFSET:
    {
        ...
        {
            phyParam.Value
                = ChannelPlanCtx.GetBeaconChannelOffset(RegionNvmGroup2->CommonJoinChannelIndex);
        }
        break;
    }
    case PHY_PING_SLOT_CHANNEL_FREQ:
...
void RegionCN470InitDefaults(InitDefaultsParams_t *params)
{
    ...
    case INIT_TYPE_DEFAULTS:
    {
        ...
        // Default bands
        memcpy1((uint8_t *)RegionNvmGroup1->Bands,
                (uint8_t *)bands,
...
#elif (defined(REGION_VERSION) && (REGION_VERSION == 0x02010001))
    for (uint8_t chMaskItr = 0, cntPayload = 0; chMaskItr < ChannelPlanCtx.JoinAcceptListSize;
                                          ...
                                          RxConfigParams_t *rxConfigParams)
{
    ...
    tSymbolInUs = RegionCommonComputeSymbolTimeLoRa(DataratesCN470[rxConfigParams->Datarate],
                                                    BandwidthsCN470[rxConfigParams->Datarate]);

    RegionCommonComputeRxWindowParameters(tSymbolInUs,
                                          minRxSymbols,
                                          rxError,
                                          Radio.GetWakeupTime(),
...
bool RegionCN470RxConfig(RxConfigParams_t *rxConfig, int8_t *datarate)
{
    ...
    if (rxConfig->RxSlot == RX_SLOT_WIN_1)
    {
        ...
        frequency
            = CN470_FIRST_RX1_CHANNEL + (rxConfig->Channel % 48) * CN470_STEPWIDTH_RX1_CHANNEL;
    }
#elif (defined(REGION_VERSION) && (REGION_VERSION == 0x02010001))
    ...
    if (rxConfig->NetworkActivation != ACTIVATION_TYPE_NONE)
    {
        ...
        else if (rxConfig->RxSlot == RX_SLOT_WIN_2)
        {
            ...
            frequency = ChannelPlanCtx.GetRx2Frequency(RegionNvmGroup2->CommonJoinChannelIndex,
                                                       RegionNvmGroup2->IsOtaaDevice);
        }
    }
    else
...
bool RegionCN470TxConfig(TxConfigParams_t *txConfig, int8_t *txPower, TimerTime_t *txTimeOnAir)
{
    ...
    int8_t txPowerLimited = RegionCommonLimitTxPower(
        txConfig->TxPower,
        RegionNvmGroup1->Bands[RegionNvmGroup2->Channels[txConfig->Channel].Band].TxMaxPower);
    ...
    Radio.SetTxConfig(MODEM_LORA,
                      ...
                      false,
                      4000);
    RegionCommonTxConfigPrint(RegionNvmGroup2->Channels[txConfig->Channel].Frequency,
                              txConfig->Datarate);
                              ...
                              uint8_t *nbBytesParsed)
{
    ...
    while (bytesProcessed < linkAdrReq->PayloadSize)
    {
        ...
        nextIndex
            = RegionCommonParseLinkAdrReq(&(linkAdrReq->Payload[bytesProcessed]), &linkAdrParams);
        ...
        else
        {
            for (uint8_t i = 0; i < 16; i++)
            {
                if (((linkAdrParams.ChMask & (1 << i)) != 0)
                    && (RegionNvmGroup2->Channels[linkAdrParams.ChMaskCtrl * 16 + i].Frequency
                        == 0))
    ...
    while (bytesProcessed < linkAdrReq->PayloadSize)
    {
        ...
        status = ChannelPlanCtx.LinkAdrChMaskUpdate(channelsMask,
                                                    linkAdrParams.ChMaskCtrl,
                                                    linkAdrParams.ChMask,
    ...
    status = RegionCommonLinkAdrReqVerifyParams(&linkAdrVerifyParams,
                                       ...
                                       TimerTime_t *aggregatedTimeOff)
{
    ...
    // Count 125kHz channels
    if (RegionCommonCountChannels(RegionNvmGroup1->ChannelsMaskRemaining,
                                  0,
    ...
    if (status == LORAMAC_STATUS_OK)
    {
        ...
        *channel = enabledChannels[randr(0, nbEnabledChannels - 1)];
        ...
        RegionCommonChanDisable(RegionNvmGroup1->ChannelsMaskRemaining,
                                *channel,
                                ChannelPlanCtx.ChannelsMaskSize);
#endif /* REGION_VERSION */
    }
...
void RegionCN470SetContinuousWave(ContinuousWaveParams_t *continuousWave)
{
    ...
    int8_t txPowerLimited = RegionCommonLimitTxPower(
        ...
        RegionNvmGroup1->Bands[RegionNvmGroup2->Channels[continuousWave->Channel].Band].TxMaxPower);
    int8_t phyTxPower = 0;
    uint32_t frequency = RegionNvmGroup2->Channels[continuousWave->Channel].Frequency;

    // Calculate physical TX power
    phyTxPower = RegionCommonComputeTxPower(txPowerLimited,
                                            continuousWave->MaxEirp,
