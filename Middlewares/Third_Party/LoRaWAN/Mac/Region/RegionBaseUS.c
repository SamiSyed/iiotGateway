 ...
 *
 * \param [out] findAvailableChannelsIndex List containing the indexes of all
 * available 125 kHz channels.
 *
...
FindAvailable125kHzChannels(uint16_t currentChannelMaskLeft,
  ...
  *availableChannels = 0;
  for (uint8_t i = 0; i < 8; i++) {
    ...
    if ((currentChannelMaskLeft & (1 << i)) != 0) {
      ...
      // Increment counter of available channels if the current channel is
      // available
      (*availableChannels)++;
    }
  }
...
RegionBaseUSComputeNext125kHzJoinChannel(uint16_t *channelsMaskRemaining,
  ...
  // Null pointer check
  if (channelsMaskRemaining == NULL || groupsCurrentIndex == NULL ||
      newChannelIndex == NULL) {
    return LORAMAC_STATUS_PARAMETER_INVALID;
  }
  ...
  do {
    // Current ChannelMaskRemaining, two groups per channel mask. For example
    // Group 0 and 1 (8 bit) are ChannelMaskRemaining 0 (16 bit), etc.
    currentChannelMaskLeftIndex = (uint8_t)startIndex / 2;

    // For even numbers we need the 8 LSBs and for uneven the 8 MSBs
    if ((startIndex % 2) == 0) {
      currentChannelMaskLeft =
          (channelsMaskRemaining[currentChannelMaskLeftIndex] & 0x00FF);
    } else {
      currentChannelMaskLeft =
          ((channelsMaskRemaining[currentChannelMaskLeftIndex] >> 8) & 0x00FF);
    }

    if (FindAvailable125kHzChannels(
            currentChannelMaskLeft, findAvailableChannelsIndex,
            &availableChannels) == LORAMAC_STATUS_PARAMETER_INVALID) {
      return LORAMAC_STATUS_PARAMETER_INVALID;
    }
    ...
    if (availableChannels > 0) {
      ...
      *newChannelIndex =
          (startIndex * 8) +
          findAvailableChannelsIndex[randr(0, (availableChannels - 1))];
    }
