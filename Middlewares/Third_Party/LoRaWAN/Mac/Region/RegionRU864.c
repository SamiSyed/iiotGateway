...
{
#if defined(REGION_RU864)
    Band_t bands[RU864_MAX_NB_BANDS] = {RU864_BAND0};
    ...
    case INIT_TYPE_DEFAULTS:
    {
        ...
        memcpy1((uint8_t *)RegionNvmGroup1->Bands,
                (uint8_t *)bands,
                sizeof(Band_t) * RU864_MAX_NB_BANDS);
#elif (defined(REGION_VERSION) && (REGION_VERSION == 0x02010001))
        RegionNvmGroup2 = (RegionNvmDataGroup2_t *)params->NvmGroup2;
...
bool RegionRU864Verify(VerifyParams_t *verify, PhyAttribute_t phyAttribute)
{
    ...
    case PHY_TX_DR:
    {
        return RegionCommonValueInRange(verify->DatarateParams.Datarate,
                                        RU864_TX_MIN_DATARATE,
                                        RU864_TX_MAX_DATARATE);
    }
    case PHY_DEF_TX_DR:
    {
    ...
    {
        return RegionCommonValueInRange(verify->DatarateParams.Datarate,
                                        RU864_RX_MIN_DATARATE,
                                        RU864_RX_MAX_DATARATE);
    }
    case PHY_DEF_TX_POWER:
    case PHY_TX_POWER:
    {
...
void RegionRU864ApplyCFList(ApplyCFListParams_t *applyCFList)
{
    ...
    for (uint8_t i = 0, chanIdx = RU864_NUMB_DEFAULT_CHANNELS; chanIdx < RU864_MAX_NB_CHANNELS;
         i += 3, chanIdx++)
    {
...
void RegionRU864ComputeRxWindowParameters(int8_t datarate,
                                          ...
                                          RxConfigParams_t *rxConfigParams)
{
    ...
    { // LoRa
        tSymbolInUs = RegionCommonComputeSymbolTimeLoRa(DataratesRU864[rxConfigParams->Datarate],
                                                        BandwidthsRU864[rxConfigParams->Datarate]);
    }
...
uint8_t RegionRU864LinkAdrReq(LinkAdrReqParams_t *linkAdrReq,
                              int8_t *drOut,
                              int8_t *txPowOut,
                              uint8_t *nbRepOut,
    ...
    while (bytesProcessed < linkAdrReq->PayloadSize)
    {
        ...
        else
        {
            for (uint8_t i = 0; i < RU864_MAX_NB_CHANNELS; i++)
            {
                ...
                {
                    if (((chMask & (1 << i)) != 0) && (RegionNvmGroup2->Channels[i].Frequency == 0))
...
LoRaMacStatus_t RegionRU864NextChannel(NextChanParams_t *nextChanParams,
                                       uint8_t *channel,
                                       TimerTime_t *time,
