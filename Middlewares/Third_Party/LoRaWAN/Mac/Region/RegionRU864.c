...
PhyParam_t RegionRU864GetPhyParam(GetPhyParams_t *getPhy) {
  ...
  case PHY_BW_FROM_DR: {
    phyParam.Value =
        RegionCommonGetBandwidth(getPhy->Datarate, BandwidthsRU864);
    break;
  }
  default: {
    break;
...
bool RegionRU864Verify(VerifyParams_t *verify, PhyAttribute_t phyAttribute) {
  ...
  case PHY_TX_POWER: {
    ...
    return RegionCommonValueInRange(verify->TxPower, RU864_MAX_TX_POWER,
                                    RU864_MIN_TX_POWER);
  }
  case PHY_DUTY_CYCLE: {
...
void RegionRU864ApplyCFList(ApplyCFListParams_t *applyCFList) {
  ...
  for (uint8_t i = 0, chanIdx = RU864_NUMB_DEFAULT_CHANNELS;
       chanIdx < RU864_MAX_NB_CHANNELS; i += 3, chanIdx++) {
...
void RegionRU864ComputeRxWindowParameters(int8_t datarate, uint8_t minRxSymbols,
  ...
  rxConfigParams->Bandwidth =
      RegionCommonGetBandwidth(rxConfigParams->Datarate, BandwidthsRU864);

  if (rxConfigParams->Datarate == DR_7) { // FSK
    tSymbolInUs = RegionCommonComputeSymbolTimeFsk(
        DataratesRU864[rxConfigParams->Datarate]);
  } else { // LoRa
...
bool RegionRU864TxConfig(TxConfigParams_t *txConfig, int8_t *txPower,
...
#endif /* REGION_VERSION */
  uint32_t bandwidth =
      RegionCommonGetBandwidth(txConfig->Datarate, BandwidthsRU864);
  int8_t phyTxPower = 0;
...
uint8_t RegionRU864LinkAdrReq(LinkAdrReqParams_t *linkAdrReq, int8_t *drOut,
...
uint8_t RegionRU864RxParamSetupReq(RxParamSetupReqParams_t *rxParamSetupReq) {
  ...
  // Verify datarate
  if (RegionCommonValueInRange(rxParamSetupReq->Datarate, RU864_RX_MIN_DATARATE,
                               RU864_RX_MAX_DATARATE) == false) {
...
uint8_t RegionRU864ApplyDrOffset(uint8_t downlinkDwellTime, int8_t dr,
                                 int8_t drOffset) {
#if defined(REGION_RU864)
