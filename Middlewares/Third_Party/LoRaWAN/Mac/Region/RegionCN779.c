...
PhyParam_t RegionCN779GetPhyParam(GetPhyParams_t *getPhy) {
  ...
  case PHY_BW_FROM_DR: {
    phyParam.Value =
        RegionCommonGetBandwidth(getPhy->Datarate, BandwidthsCN779);
    break;
  }
  default: {
    break;
...
bool RegionCN779Verify(VerifyParams_t *verify, PhyAttribute_t phyAttribute) {
  ...
  case PHY_TX_POWER: {
    ...
    return RegionCommonValueInRange(verify->TxPower, CN779_MAX_TX_POWER,
                                    CN779_MIN_TX_POWER);
  }
  case PHY_DUTY_CYCLE: {
...
void RegionCN779ApplyCFList(ApplyCFListParams_t *applyCFList) {
  ...
  for (uint8_t i = 0, chanIdx = CN779_NUMB_DEFAULT_CHANNELS;
       chanIdx < CN779_MAX_NB_CHANNELS; i += 3, chanIdx++) {
...
void RegionCN779ComputeRxWindowParameters(int8_t datarate, uint8_t minRxSymbols,
  ...
  rxConfigParams->Bandwidth =
      RegionCommonGetBandwidth(rxConfigParams->Datarate, BandwidthsCN779);

  if (rxConfigParams->Datarate == DR_7) { // FSK
    tSymbolInUs = RegionCommonComputeSymbolTimeFsk(
        DataratesCN779[rxConfigParams->Datarate]);
  } else { // LoRa
...
bool RegionCN779TxConfig(TxConfigParams_t *txConfig, int8_t *txPower,
...
#endif /* REGION_VERSION */
  uint32_t bandwidth =
      RegionCommonGetBandwidth(txConfig->Datarate, BandwidthsCN779);
  int8_t phyTxPower = 0;
...
uint8_t RegionCN779LinkAdrReq(LinkAdrReqParams_t *linkAdrReq, int8_t *drOut,
...
uint8_t RegionCN779RxParamSetupReq(RxParamSetupReqParams_t *rxParamSetupReq) {
  ...
  // Verify datarate
  if (RegionCommonValueInRange(rxParamSetupReq->Datarate, CN779_RX_MIN_DATARATE,
                               CN779_RX_MAX_DATARATE) == false) {
...
uint8_t RegionCN779ApplyDrOffset(uint8_t downlinkDwellTime, int8_t dr,
                                 int8_t drOffset) {
#if defined(REGION_CN779)
