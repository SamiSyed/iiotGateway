...
{
#if defined(REGION_CN779)
    Band_t bands[CN779_MAX_NB_BANDS] = {CN779_BAND0};
    ...
    case INIT_TYPE_DEFAULTS:
    {
        ...
        memcpy1((uint8_t *)RegionNvmGroup1->Bands,
                (uint8_t *)bands,
                sizeof(Band_t) * CN779_MAX_NB_BANDS);
#elif (defined(REGION_VERSION) && (REGION_VERSION == 0x02010001))
        RegionNvmGroup2 = (RegionNvmDataGroup2_t *)params->NvmGroup2;
...
void RegionCN779ApplyCFList(ApplyCFListParams_t *applyCFList)
{
    ...
    for (uint8_t i = 0, chanIdx = CN779_NUMB_DEFAULT_CHANNELS; chanIdx < CN779_MAX_NB_CHANNELS;
         i += 3, chanIdx++)
    {
...
void RegionCN779ComputeRxWindowParameters(int8_t datarate,
                                          ...
                                          RxConfigParams_t *rxConfigParams)
{
    ...
    { // LoRa
        tSymbolInUs = RegionCommonComputeSymbolTimeLoRa(DataratesCN779[rxConfigParams->Datarate],
...
uint8_t RegionCN779LinkAdrReq(LinkAdrReqParams_t *linkAdrReq,
                              int8_t *drOut,
                              int8_t *txPowOut,
                              uint8_t *nbRepOut,
...
uint8_t RegionCN779RxParamSetupReq(RxParamSetupReqParams_t *rxParamSetupReq)
{
    ...
    if (RegionCommonValueInRange(rxParamSetupReq->DrOffset,
                                 CN779_MIN_RX1_DR_OFFSET,
                                 CN779_MAX_RX1_DR_OFFSET)
        == false)
    {
...
LoRaMacStatus_t RegionCN779NextChannel(NextChanParams_t *nextChanParams,
                                       uint8_t *channel,
                                       TimerTime_t *time,
