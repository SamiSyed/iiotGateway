 ...
 *
 * \brief     Specific Region implementation of CN470 Channel plan type A,
 * 20MHz.
 *
...
uint8_t RegionCN470A20LinkAdrChMaskUpdate(uint16_t *channelsMask,
  ...
  if ((chMaskCntl == 4) || (chMaskCntl == 5)) {
    ...
    for (uint8_t i = 0; i < 16; i++) {
      if (((chanMask & (1 << i)) != 0) &&
          (channels[chMaskCntl * 16 + i].Frequency ==
...
bool RegionCN470A20VerifyRfFreq(uint32_t freq) {
  ...
  if (RegionBaseUSVerifyFrequencyGroup(
          freq, CN470_A20_FIRST_RX_CHANNEL, CN470_A20_LAST_RX_CHANNEL,
...
void RegionCN470A20InitializeChannels(ChannelParams_t *channels) {
  ...
  for (uint8_t i = 0; i < 32; i++) {
    channels[i].Frequency =
        CN470_A20_FIRST_TX1_CHANNEL + i * CN470_A20_STEPWIDTH_RX_CHANNEL;
    channels[i].DrRange.Value =
        (CN470_TX_MAX_DATARATE << 4) | CN470_TX_MIN_DATARATE;
    channels[i].Band = 0;
  }
  // Upstream group 2
  for (uint8_t i = 32; i < 64; i++) {
    channels[i].Frequency =
        CN470_A20_FIRST_TX2_CHANNEL + (i - 32) * CN470_A20_STEPWIDTH_RX_CHANNEL;
    channels[i].DrRange.Value =
        (CN470_TX_MAX_DATARATE << 4) | CN470_TX_MIN_DATARATE;
    channels[i].Band = 0;
  }
...
uint32_t RegionCN470A20GetRx1Frequency(uint8_t channel) {
  ...
  return (baseFrequency +
          ((channel - offset) * CN470_A20_STEPWIDTH_RX_CHANNEL));
}
