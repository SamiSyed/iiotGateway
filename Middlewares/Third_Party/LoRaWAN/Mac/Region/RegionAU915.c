...
PhyParam_t RegionAU915GetPhyParam(GetPhyParams_t *getPhy) {
  ...
  case PHY_NEXT_LOWER_TX_DR: {
    RegionCommonGetNextLowerTxDrParams_t nextLowerTxDrParams = {
        ...
        .MinDr = (int8_t)((getPhy->UplinkDwellTime == 0)
  ...
  case PHY_MAX_PAYLOAD_REPEATER: {
    if (getPhy->UplinkDwellTime == 0) {
      phyParam.Value =
          MaxPayloadOfDatarateRepeaterDwell0AU915[getPhy->Datarate];
    } else {
      phyParam.Value =
          MaxPayloadOfDatarateRepeaterDwell1AU915[getPhy->Datarate];
    }
    break;
  }
  case PHY_DUTY_CYCLE: {
  ...
  case PHY_BEACON_CHANNEL_FREQ: {
    phyParam.Value = RegionBaseUSCalcDownlinkFrequency(
        getPhy->Channel, AU915_BEACON_CHANNEL_FREQ,
  ...
  case PHY_PING_SLOT_CHANNEL_FREQ: {
    phyParam.Value = RegionBaseUSCalcDownlinkFrequency(
        getPhy->Channel, AU915_PING_SLOT_CHANNEL_FREQ,
        AU915_BEACON_CHANNEL_STEPWIDTH);
    break;
  }
  ...
  case PHY_BW_FROM_DR: {
    phyParam.Value =
        RegionCommonGetBandwidth(getPhy->Datarate, BandwidthsAU915);
    break;
  }
  default: {
    break;
...
void RegionAU915InitDefaults(InitDefaultsParams_t *params) {
  ...
  case INIT_TYPE_DEFAULTS: {
    ...
    for (uint8_t i = AU915_MAX_NB_CHANNELS - 8; i < AU915_MAX_NB_CHANNELS;
         i++) {
      // 500 kHz channels
...
bool RegionAU915Verify(VerifyParams_t *verify, PhyAttribute_t phyAttribute) {
  ...
  case PHY_TX_POWER: {
    ...
    return RegionCommonValueInRange(verify->TxPower, AU915_MAX_TX_POWER,
                                    AU915_MIN_TX_POWER);
  }
  case PHY_DUTY_CYCLE: {
...
#endif /* REGION_VERSION */
  uint32_t bandwidth =
      RegionCommonGetBandwidth(txConfig->Datarate, BandwidthsAU915);
  int8_t phyTxPower = 0;
...
uint8_t RegionAU915RxParamSetupReq(RxParamSetupReqParams_t *rxParamSetupReq) {
  ...
  // Verify datarate
  if (RegionCommonValueInRange(rxParamSetupReq->Datarate, AU915_RX_MIN_DATARATE,
                               AU915_RX_MAX_DATARATE) == false) {
  ...
  if (RegionCommonValueInRange(rxParamSetupReq->DrOffset,
                               AU915_MIN_RX1_DR_OFFSET,
                               AU915_MAX_RX1_DR_OFFSET) == false) {
