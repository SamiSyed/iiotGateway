                                          ...
                                          ChannelParams_t *channels)
{
    ...
    return RegionCN470A20LinkAdrChMaskUpdate(channelsMask, chMaskCntl, chanMask, channels);
}
...
bool RegionCN470B20VerifyRfFreq(uint32_t freq)
{
    ...
    // Downstream group 1
    group1Status = RegionBaseUSVerifyFrequencyGroup(freq,
                                                    CN470_B20_FIRST_RX1_CHANNEL,
                                                    CN470_B20_LAST_RX1_CHANNEL,
                                                    CN470_B20_STEPWIDTH_RX1_CHANNEL);
    // Downstream group 2
    group2Status = RegionBaseUSVerifyFrequencyGroup(freq,
...
void RegionCN470B20InitializeChannels(ChannelParams_t *channels)
{
    ...
    for (uint8_t i = 32; i < 64; i++)
    {
        channels[i].Frequency
            = CN470_B20_FIRST_TX2_CHANNEL + (i - 32) * CN470_B20_STEPWIDTH_TX2_CHANNEL;
        channels[i].DrRange.Value = (CN470_TX_MAX_DATARATE << 4) | CN470_TX_MIN_DATARATE;
        channels[i].Band = 0;
    }
}
