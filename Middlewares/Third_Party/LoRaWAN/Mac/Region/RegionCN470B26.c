...
uint8_t RegionCN470B26LinkAdrChMaskUpdate(uint16_t *channelsMask,
  ...
  return RegionCN470A26LinkAdrChMaskUpdate(channelsMask, chMaskCntl, chanMask,
                                           channels);
}
...
bool RegionCN470B26VerifyRfFreq(uint32_t freq) {
  ...
  if (RegionBaseUSVerifyFrequencyGroup(
          freq, CN470_B26_FIRST_RX_CHANNEL, CN470_B26_LAST_RX_CHANNEL,
...
void RegionCN470B26InitializeChannels(ChannelParams_t *channels) {
  ...
  for (uint8_t i = 0; i < 48; i++) {
    channels[i].Frequency =
        CN470_B26_FIRST_TX_CHANNEL + i * CN470_B26_STEPWIDTH_RX_CHANNEL;
...
uint32_t RegionCN470B26GetRx1Frequency(uint8_t channel) {
  return (CN470_B26_FIRST_RX_CHANNEL +
          ((channel % 24) * CN470_B26_STEPWIDTH_RX_CHANNEL));
}
