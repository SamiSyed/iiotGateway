...
PhyParam_t RegionKR920GetPhyParam(GetPhyParams_t *getPhy) {
  ...
  case PHY_BW_FROM_DR: {
    phyParam.Value =
        RegionCommonGetBandwidth(getPhy->Datarate, BandwidthsKR920);
    break;
  }
  default: {
    break;
...
void RegionKR920InitDefaults(InitDefaultsParams_t *params) {
#if defined(REGION_KR920)
  Band_t bands[KR920_MAX_NB_BANDS] = {KR920_BAND0};
  ...
  case INIT_TYPE_DEFAULTS: {
    ...
    memcpy1((uint8_t *)RegionNvmGroup1->Bands, (uint8_t *)bands,
            sizeof(Band_t) * KR920_MAX_NB_BANDS);
#elif (defined(REGION_VERSION) && (REGION_VERSION == 0x02010001))
    RegionNvmGroup2 = (RegionNvmDataGroup2_t *)params->NvmGroup2;
...
bool RegionKR920Verify(VerifyParams_t *verify, PhyAttribute_t phyAttribute) {
  ...
  case PHY_TX_POWER: {
    ...
    return RegionCommonValueInRange(verify->TxPower, KR920_MAX_TX_POWER,
                                    KR920_MIN_TX_POWER);
  }
  case PHY_DUTY_CYCLE: {
...
void RegionKR920ApplyCFList(ApplyCFListParams_t *applyCFList) {
  ...
  for (uint8_t i = 0, chanIdx = KR920_NUMB_DEFAULT_CHANNELS;
       chanIdx < KR920_MAX_NB_CHANNELS; i += 3, chanIdx++) {
...
void RegionKR920ComputeRxWindowParameters(int8_t datarate, uint8_t minRxSymbols,
  ...
  rxConfigParams->Bandwidth =
      RegionCommonGetBandwidth(rxConfigParams->Datarate, BandwidthsKR920);

  tSymbolInUs = RegionCommonComputeSymbolTimeLoRa(
      DataratesKR920[rxConfigParams->Datarate],
...
#endif /* REGION_VERSION */
  uint32_t bandwidth =
      RegionCommonGetBandwidth(txConfig->Datarate, BandwidthsKR920);
  float maxEIRP =
      GetMaxEIRP(RegionNvmGroup2->Channels[txConfig->Channel].Frequency);
  int8_t phyTxPower = 0;

  // Take the minimum between the maxEIRP and txConfig->MaxEirp.
  // The value of txConfig->MaxEirp could have changed during runtime, e.g. due
  // to a MAC command.
  maxEIRP = MIN(txConfig->MaxEirp, maxEIRP);

  // Calculate physical TX power
  phyTxPower = RegionCommonComputeTxPower(txPowerLimited, maxEIRP,
...
uint8_t RegionKR920RxParamSetupReq(RxParamSetupReqParams_t *rxParamSetupReq) {
  ...
  // Verify datarate
  if (RegionCommonValueInRange(rxParamSetupReq->Datarate, KR920_RX_MIN_DATARATE,
                               KR920_RX_MAX_DATARATE) == false) {
...
LoRaMacStatus_t RegionKR920NextChannel(NextChanParams_t *nextChanParams,
  ...
  if (status == LORAMAC_STATUS_OK) {
    for (uint8_t i = 0, j = randr(0, nbEnabledChannels - 1);
      ...
      if (Radio.IsChannelFree(RegionNvmGroup2->Channels[channelNext].Frequency,
                              KR920_LBT_RX_BANDWIDTH,
                              RegionNvmGroup2->RssiFreeThreshold,
                              RegionNvmGroup2->CarrierSenseTime) == true) {
        // Free channel found
        *channel = channelNext;
...
void RegionKR920SetContinuousWave(ContinuousWaveParams_t *continuousWave) {
  ...
  int8_t txPowerLimited = RegionCommonLimitTxPower(
      ...
      RegionNvmGroup1
          ...
          .TxMaxPower);
  float maxEIRP =
      GetMaxEIRP(RegionNvmGroup2->Channels[continuousWave->Channel].Frequency);
  int8_t phyTxPower = 0;
  uint32_t frequency =
      RegionNvmGroup2->Channels[continuousWave->Channel].Frequency;

  // Take the minimum between the maxEIRP and continuousWave->MaxEirp.
  // The value of continuousWave->MaxEirp could have changed during runtime,
