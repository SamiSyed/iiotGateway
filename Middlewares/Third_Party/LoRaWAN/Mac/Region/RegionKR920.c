...
void RegionKR920ApplyCFList(ApplyCFListParams_t *applyCFList)
{
    ...
    for (uint8_t i = 0, chanIdx = KR920_NUMB_DEFAULT_CHANNELS; chanIdx < KR920_MAX_NB_CHANNELS;
         i += 3, chanIdx++)
    {
                                          ...
                                          RxConfigParams_t *rxConfigParams)
{
    ...
    tSymbolInUs = RegionCommonComputeSymbolTimeLoRa(DataratesKR920[rxConfigParams->Datarate],
...
void RegionKR920SetContinuousWave(ContinuousWaveParams_t *continuousWave)
{
    ...
    // The value of continuousWave->MaxEirp could have changed during runtime, e.g. due to a MAC
    // command.
