...
static bool VerifyRfFreq(uint32_t freq)
{
    ...
    if ((freq < US915_FIRST_RX1_CHANNEL) || (freq > US915_LAST_RX1_CHANNEL)
        || (((freq - (uint32_t)US915_FIRST_RX1_CHANNEL) % (uint32_t)US915_STEPWIDTH_RX1_CHANNEL)
            != 0))
    {
        return false;
    }
...
void RegionUS915ComputeRxWindowParameters(int8_t datarate,
                                          ...
                                          RxConfigParams_t *rxConfigParams)
{
    ...
    tSymbolInUs = RegionCommonComputeSymbolTimeLoRa(DataratesUS915[rxConfigParams->Datarate],
...
uint8_t RegionUS915LinkAdrReq(LinkAdrReqParams_t *linkAdrReq,
                              int8_t *drOut,
                              int8_t *txPowOut,
                              uint8_t *nbRepOut,
    ...
    if (status == LORAMAC_STATUS_OK)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                uint8_t i = 0;
                while (((RegionNvmGroup1->ChannelsMaskRemaining[4] & CHANNELS_MASK_500KHZ_MASK)
                        & (1 << i))
                       == 0)
                {
                    i++;
