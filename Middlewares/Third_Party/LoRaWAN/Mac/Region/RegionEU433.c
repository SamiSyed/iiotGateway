...
PhyParam_t RegionEU433GetPhyParam(GetPhyParams_t *getPhy) {
  ...
  case PHY_BW_FROM_DR: {
    phyParam.Value =
        RegionCommonGetBandwidth(getPhy->Datarate, BandwidthsEU433);
    break;
  }
  default: {
    break;
...
void RegionEU433InitDefaults(InitDefaultsParams_t *params) {
#if defined(REGION_EU433)
  Band_t bands[EU433_MAX_NB_BANDS] = {EU433_BAND0};
  ...
  case INIT_TYPE_DEFAULTS: {
    ...
    memcpy1((uint8_t *)RegionNvmGroup1->Bands, (uint8_t *)bands,
            sizeof(Band_t) * EU433_MAX_NB_BANDS);
#elif (defined(REGION_VERSION) && (REGION_VERSION == 0x02010001))
    RegionNvmGroup2 = (RegionNvmDataGroup2_t *)params->NvmGroup2;
...
bool RegionEU433Verify(VerifyParams_t *verify, PhyAttribute_t phyAttribute) {
  ...
  case PHY_TX_POWER: {
    ...
    return RegionCommonValueInRange(verify->TxPower, EU433_MAX_TX_POWER,
                                    EU433_MIN_TX_POWER);
  }
  case PHY_DUTY_CYCLE: {
...
void RegionEU433ApplyCFList(ApplyCFListParams_t *applyCFList) {
  ...
  for (uint8_t i = 0, chanIdx = EU433_NUMB_DEFAULT_CHANNELS;
       chanIdx < EU433_MAX_NB_CHANNELS; i += 3, chanIdx++) {
...
void RegionEU433ComputeRxWindowParameters(int8_t datarate, uint8_t minRxSymbols,
                                          uint32_t rxError,
  ...
  rxConfigParams->Bandwidth =
      RegionCommonGetBandwidth(rxConfigParams->Datarate, BandwidthsEU433);

  if (rxConfigParams->Datarate == DR_7) { // FSK
    tSymbolInUs = RegionCommonComputeSymbolTimeFsk(
        DataratesEU433[rxConfigParams->Datarate]);
  } else { // LoRa
...
bool RegionEU433TxConfig(TxConfigParams_t *txConfig, int8_t *txPower,
...
#endif /* REGION_VERSION */
  uint32_t bandwidth =
      RegionCommonGetBandwidth(txConfig->Datarate, BandwidthsEU433);
  int8_t phyTxPower = 0;
...
uint8_t RegionEU433LinkAdrReq(LinkAdrReqParams_t *linkAdrReq, int8_t *drOut,
...
uint8_t RegionEU433RxParamSetupReq(RxParamSetupReqParams_t *rxParamSetupReq) {
  ...
  // Verify datarate
  if (RegionCommonValueInRange(rxParamSetupReq->Datarate, EU433_RX_MIN_DATARATE,
                               EU433_RX_MAX_DATARATE) == false) {
...
uint8_t RegionEU433ApplyDrOffset(uint8_t downlinkDwellTime, int8_t dr,
                                 int8_t drOffset) {
#if defined(REGION_EU433)
