...
PhyParam_t RegionAS923GetPhyParam(GetPhyParams_t *getPhy)
{
    ...
    case PHY_NEXT_LOWER_TX_DR:
    {
        RegionCommonGetNextLowerTxDrParams_t nextLowerTxDrParams = {
            ...
            .MinDr = (int8_t)((getPhy->UplinkDwellTime == 0) ? AS923_TX_MIN_DATARATE
...
void RegionAS923ApplyCFList(ApplyCFListParams_t *applyCFList)
{
    ...
    for (uint8_t i = 0, chanIdx = AS923_NUMB_DEFAULT_CHANNELS; chanIdx < AS923_MAX_NB_CHANNELS;
         i += 3, chanIdx++)
    {
                                          ...
                                          RxConfigParams_t *rxConfigParams)
{
    ...
    { // LoRa
        tSymbolInUs = RegionCommonComputeSymbolTimeLoRa(DataratesAS923[rxConfigParams->Datarate],
    ...
    if (status == LORAMAC_STATUS_OK)
    {
        ...
        for (uint8_t i = 0, j = randr(0, nbEnabledChannels - 1); i < AS923_MAX_NB_CHANNELS; i++)
        {
            ...
            if (Radio.IsChannelFree(RegionNvmGroup2->Channels[channelNext].Frequency,
                                    ...
                                    RegionNvmGroup2->CarrierSenseTime)
                == true)
            {
                // Free channel found
                *channel = channelNext;
