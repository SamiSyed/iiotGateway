...
PhyParam_t RegionAS923GetPhyParam(GetPhyParams_t *getPhy) {
  ...
  case PHY_MAX_PAYLOAD_REPEATER: {
    if (getPhy->UplinkDwellTime == 0) {
      phyParam.Value =
          MaxPayloadOfDatarateRepeaterDwell0AS923[getPhy->Datarate];
    } else {
      phyParam.Value = MaxPayloadOfDatarateDwell1AS923[getPhy->Datarate];
    }
    break;
  ...
  case PHY_BW_FROM_DR: {
    phyParam.Value =
        RegionCommonGetBandwidth(getPhy->Datarate, BandwidthsAS923);
    break;
  }
  default: {
    break;
...
void RegionAS923InitDefaults(InitDefaultsParams_t *params) {
#if defined(REGION_AS923)
  Band_t bands[AS923_MAX_NB_BANDS] = {AS923_BAND0};
  ...
  case INIT_TYPE_DEFAULTS: {
    ...
    memcpy1((uint8_t *)RegionNvmGroup1->Bands, (uint8_t *)bands,
            sizeof(Band_t) * AS923_MAX_NB_BANDS);
#elif (defined(REGION_VERSION) && (REGION_VERSION == 0x02010001))
    RegionNvmGroup2 = (RegionNvmDataGroup2_t *)params->NvmGroup2;
...
bool RegionAS923Verify(VerifyParams_t *verify, PhyAttribute_t phyAttribute) {
  ...
  case PHY_TX_POWER: {
    ...
    return RegionCommonValueInRange(verify->TxPower, AS923_MAX_TX_POWER,
                                    AS923_MIN_TX_POWER);
  }
  case PHY_DUTY_CYCLE: {
...
void RegionAS923ApplyCFList(ApplyCFListParams_t *applyCFList) {
  ...
  for (uint8_t i = 0, chanIdx = AS923_NUMB_DEFAULT_CHANNELS;
       chanIdx < AS923_MAX_NB_CHANNELS; i += 3, chanIdx++) {
...
void RegionAS923ComputeRxWindowParameters(int8_t datarate, uint8_t minRxSymbols,
                                          uint32_t rxError,
  ...
  rxConfigParams->Bandwidth =
      RegionCommonGetBandwidth(rxConfigParams->Datarate, BandwidthsAS923);

  if (rxConfigParams->Datarate == DR_7) { // FSK
    tSymbolInUs = RegionCommonComputeSymbolTimeFsk(
        DataratesAS923[rxConfigParams->Datarate]);
  } else { // LoRa
...
bool RegionAS923TxConfig(TxConfigParams_t *txConfig, int8_t *txPower,
...
#endif /* REGION_VERSION */
  uint32_t bandwidth =
      RegionCommonGetBandwidth(txConfig->Datarate, BandwidthsAS923);
  int8_t phyTxPower = 0;
...
uint8_t RegionAS923LinkAdrReq(LinkAdrReqParams_t *linkAdrReq, int8_t *drOut,
...
uint8_t RegionAS923RxParamSetupReq(RxParamSetupReqParams_t *rxParamSetupReq) {
  ...
  // Verify datarate
  if (RegionCommonValueInRange(rxParamSetupReq->Datarate, AS923_RX_MIN_DATARATE,
                               AS923_RX_MAX_DATARATE) == false) {
...
uint8_t RegionAS923ApplyDrOffset(uint8_t downlinkDwellTime, int8_t dr,
                                 int8_t drOffset) {
#if defined(REGION_AS923)
