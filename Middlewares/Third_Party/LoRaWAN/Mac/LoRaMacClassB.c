 ...
 */
static TimerTime_t TimerTempCompensation(TimerTime_t period,
                                         int16_t temperature);
  ...
  if ((Ctx.BeaconCtx.Ctrl.BeaconAcquired == 1) ||
    ...
    RegionComputeRxWindowParameters(
        *Ctx.LoRaMacClassBParams.LoRaMacRegion,
        ...
        Ctx.LoRaMacClassBParams.LoRaMacParams->SystemMaxRxError, rxConfig);
  }
...
static void GetTemperature(LoRaMacClassBCallback_t *callbacks,
                           BeaconContext_t *beaconCtx) {
...
static void LoRaMacClassBProcessBeacon(void) {
        ...
        if (Ctx.BeaconCtx.BeaconTimingDelay > 0) {
          ...
          if (SysTimeToMs(Ctx.BeaconCtx.NextBeaconRx) > now) {
            ...
            beaconEventTime = TimerTempCompensation(
                SysTimeToMs(Ctx.BeaconCtx.NextBeaconRx) - now,
  ...
  case BEACON_STATE_REACQUISITION: {
    ...
    if ((beaconTimestamp - SysTimeToMs(Ctx.BeaconCtx.LastBeaconRx)) >
      ...
      // Handle beacon miss
      beaconEventTime = UpdateBeaconState(
...
static void LoRaMacClassBProcessMulticastSlot(void) {
  ...
  case PINGSLOT_STATE_IDLE: {
    ...
    multicastSlotRxConfig.Datarate =
        Ctx.PingSlotCtx.NextMulticastChannel->ChannelParams.RxParams.Params
            .ClassB.Datarate;
...
bool LoRaMacClassBRxBeacon(uint8_t *payload, uint16_t size) {
  ...
  if ((Ctx.BeaconState == BEACON_STATE_RX) ||
    ...
    if (size == phyParam.BeaconFormat.BeaconSize) {
      ...
      beaconCrc0 =
          ((uint16_t)payload[phyParam.BeaconFormat.Rfu1Size + 4]) & 0x00FF;
      beaconCrc0 |=
          ((uint16_t)payload[phyParam.BeaconFormat.Rfu1Size + 4 + 1] << 8) &
          0xFF00;
      ...
      if (crc0 == beaconCrc0) {
        ...
        Ctx.BeaconCtx.BeaconTime.Seconds |=
            ((uint32_t)(payload[phyParam.BeaconFormat.Rfu1Size + 1] << 8)) &
            0x0000FF00;
        Ctx.BeaconCtx.BeaconTime.Seconds |=
            ((uint32_t)(payload[phyParam.BeaconFormat.Rfu1Size + 2] << 16)) &
            0x00FF0000;
        Ctx.BeaconCtx.BeaconTime.Seconds |=
            ((uint32_t)(payload[phyParam.BeaconFormat.Rfu1Size + 3] << 24)) &
            0xFF000000;
        Ctx.BeaconCtx.BeaconTime.SubSeconds = 0;
      ...
      beaconCrc1 = ((uint16_t)payload[phyParam.BeaconFormat.Rfu1Size + 4 + 2 +
                                      7 + phyParam.BeaconFormat.Rfu2Size]) &
                   0x00FF;
      beaconCrc1 |= ((uint16_t)payload[phyParam.BeaconFormat.Rfu1Size + 4 + 2 +
                                       7 + phyParam.BeaconFormat.Rfu2Size + 1]
                     << 8) &
                    0xFF00;
      ...
      if (crc1 == beaconCrc1) {
        ...
        memcpy1(
            ...
            &payload[phyParam.BeaconFormat.Rfu1Size + 4 + 2 + 1], 6);
      }
#elif (defined(LORAMAC_VERSION) &&                                             \
       ((LORAMAC_VERSION == 0x01000400) || (LORAMAC_VERSION == 0x01010100)))
      // A beacon frame is defined as:
      ...
      if (beaconProcessed == true) {
        ...
        TimerTime_t time = Radio.TimeOnAir(
            MODEM_LORA, bandwidth, spreadingFactor, 1, 10, true, size, false);
        SysTime_t timeOnAir;
...
void LoRaMacClassBSetMulticastPeriodicity(MulticastCtx_t *multicastChannel) {
  ...
  if (multicastChannel != NULL) {
    multicastChannel->PingNb = CalcPingNb(
        multicastChannel->ChannelParams.RxParams.Params.ClassB.Periodicity);
