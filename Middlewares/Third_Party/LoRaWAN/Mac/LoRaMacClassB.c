 ...
 */
static void ComputePingOffset(uint64_t beaconTime,
                              uint32_t address,
                              uint16_t pingPeriod,
                              uint16_t *pingOffset)
{
...
static void RxBeaconSetup(TimerTime_t rxTime, bool activateDefaultChannel, uint16_t symbolTimeout)
{
    ...
    else
    {
        ...
        frequency = CalcDownlinkChannelAndFrequency(0,
                                                    Ctx.BeaconCtx.BeaconTime.Seconds
                                                        + (CLASSB_BEACON_INTERVAL / 1000),
    ...
    RegionRxBeaconSetup(*Ctx.LoRaMacClassBParams.LoRaMacRegion,
                        &rxBeaconSetup,
 ...
 */
static bool CalcNextSlotTime(uint16_t slotOffset,
                             uint16_t pingPeriod,
                             uint16_t pingNb,
                             TimerTime_t *timeOffset)
{
    ...
    {
        currentPingSlot = ((currentTime - slotTime) / (pingPeriod * CLASSB_PING_SLOT_WINDOW)) + 1;
        slotTime += ((TimerTime_t)(currentPingSlot * pingPeriod) * CLASSB_PING_SLOT_WINDOW);
    }

    if (currentPingSlot < pingNb)
    {
        if (slotTime <= (SysTimeToMs(Ctx.BeaconCtx.NextBeaconRx) - CLASSB_BEACON_GUARD
...
static void LoRaMacClassBProcessBeacon(void)
{
            ...
            if (Ctx.BeaconCtx.Ctrl.BeaconDelaySet == 1)
            {
                ...
                if (Ctx.BeaconCtx.BeaconTimingDelay > 0)
                {
                    ...
                    if (SysTimeToMs(Ctx.BeaconCtx.NextBeaconRx) > now)
                    {
                        ...
                        beaconEventTime
                            = TimerTempCompensation(SysTimeToMs(Ctx.BeaconCtx.NextBeaconRx) - now,
    ...
    case BEACON_STATE_REACQUISITION:
    {
        ...
        if ((beaconTimestamp - SysTimeToMs(Ctx.BeaconCtx.LastBeaconRx))
    ...
    case BEACON_STATE_LOCKED:
    {
        ...
        beaconEventTime
            = UpdateBeaconState(LORAMAC_EVENT_INFO_STATUS_BEACON_LOCKED, 0, beaconTimestamp);
...
static void LoRaMacClassBProcessPingSlot(void)
{
    ...
    {
        if (CalcNextSlotTime(Ctx.PingSlotCtx.PingOffset,
                             ClassBNvm->PingSlotCtx.PingPeriod,
                             ClassBNvm->PingSlotCtx.PingNb,
                             &pingSlotTime)
            == true)
        {
    ...
    case PINGSLOT_STATE_IDLE:
    {
        ...
        {
            // Restore floor plan
            frequency = CalcDownlinkChannelAndFrequency(*Ctx.LoRaMacClassBParams.LoRaMacDevAddr,
                                                        Ctx.BeaconCtx.BeaconTime.Seconds,
                                                        CLASSB_BEACON_INTERVAL,
                                                        false);
        }
        ...
        if (Ctx.MulticastSlotState != PINGSLOT_STATE_RX)
        {
            ...
            pingSlotRxConfig.RepeaterSupport
                = Ctx.LoRaMacClassBParams.LoRaMacParams->RepeaterSupport;
            ...
            RegionRxConfig(*Ctx.LoRaMacClassBParams.LoRaMacRegion,
                           &pingSlotRxConfig,
       ...
       && ((LORAMAC_VERSION == 0x01000400) || (LORAMAC_VERSION == 0x01010100)))
        if (Ctx.PingSlotCtx.NextMulticastChannel != NULL)
        {
            ...
            slotHasPriority
                = CheckSlotPriority(*Ctx.LoRaMacClassBParams.LoRaMacDevAddr,
                                    ClassBNvm->PingSlotCtx.FPendingSet,
                                    0,
                                    Ctx.PingSlotCtx.NextMulticastChannel->ChannelParams.Address,
                                    Ctx.PingSlotCtx.NextMulticastChannel->FPendingSet,
                                    1);
        }
...
static void LoRaMacClassBProcessMulticastSlot(void)
{
    ...
    case PINGSLOT_STATE_IDLE:
    {
        ...
        // Apply frequency
        frequency
            = Ctx.PingSlotCtx.NextMulticastChannel->ChannelParams.RxParams.Params.ClassB.Frequency;
        ...
        multicastSlotRxConfig.DownlinkDwellTime
            = Ctx.LoRaMacClassBParams.LoRaMacParams->DownlinkDwellTime;
        multicastSlotRxConfig.RepeaterSupport
            = Ctx.LoRaMacClassBParams.LoRaMacParams->RepeaterSupport;
       ...
       && ((LORAMAC_VERSION == 0x01000400) || (LORAMAC_VERSION == 0x01010100)))
        // Verify, if the unicast has priority.
        slotHasPriority
            = CheckSlotPriority(Ctx.PingSlotCtx.NextMulticastChannel->ChannelParams.Address,
                                Ctx.PingSlotCtx.NextMulticastChannel->FPendingSet,
                                1,
                                *Ctx.LoRaMacClassBParams.LoRaMacDevAddr,
                                ClassBNvm->PingSlotCtx.FPendingSet,
        ...
        if ((Ctx.PingSlotState != PINGSLOT_STATE_RX) || (slotHasPriority == true))
        {
            ...
            multicastSlotRxConfig.Datarate = Ctx.PingSlotCtx.NextMulticastChannel->ChannelParams
                                                 .RxParams.Params.ClassB.Datarate;
            multicastSlotRxConfig.DownlinkDwellTime
                = Ctx.LoRaMacClassBParams.LoRaMacParams->DownlinkDwellTime;
            multicastSlotRxConfig.Frequency = frequency;
            multicastSlotRxConfig.RxContinuous = false;
...
bool LoRaMacClassBRxBeacon(uint8_t *payload, uint16_t size)
{
    ...
    if ((Ctx.BeaconState == BEACON_STATE_RX) || (Ctx.BeaconCtx.Ctrl.AcquisitionPending == 1))
    {
        if (size == phyParam.BeaconFormat.BeaconSize)
        {
            ...
            //
            // Field RFU1 and RFU2 have variable sizes. It depends on the region specific
            // implementation
            ...
            if (crc0 == beaconCrc0)
            {
                ...
                Ctx.BeaconCtx.BeaconTime.Seconds
                    = ((uint32_t)payload[phyParam.BeaconFormat.Rfu1Size]) & 0x000000FF;
                Ctx.BeaconCtx.BeaconTime.Seconds
                    |= ((uint32_t)(payload[phyParam.BeaconFormat.Rfu1Size + 1] << 8)) & 0x0000FF00;
                Ctx.BeaconCtx.BeaconTime.Seconds
                    |= ((uint32_t)(payload[phyParam.BeaconFormat.Rfu1Size + 2] << 16)) & 0x00FF0000;
                Ctx.BeaconCtx.BeaconTime.Seconds
                    |= ((uint32_t)(payload[phyParam.BeaconFormat.Rfu1Size + 3] << 24)) & 0xFF000000;
                Ctx.BeaconCtx.BeaconTime.SubSeconds = 0;
            ...
            beaconCrc1 = ((uint16_t)payload[phyParam.BeaconFormat.Rfu1Size + 4 + 2 + 7
                                            + phyParam.BeaconFormat.Rfu2Size])
                         & 0x00FF;
            beaconCrc1 |= ((uint16_t)payload[phyParam.BeaconFormat.Rfu1Size + 4 + 2 + 7
                                             + phyParam.BeaconFormat.Rfu2Size + 1]
                           << 8)
                          & 0xFF00;
            crc1 = BeaconCrc(&payload[phyParam.BeaconFormat.Rfu1Size + 4 + 2],
            ...
            if (crc1 == beaconCrc1)
            {
                ...
                Ctx.LoRaMacClassBParams.MlmeIndication->BeaconInfo.GwSpecific.InfoDesc
                    = payload[phyParam.BeaconFormat.Rfu1Size + 4 + 2];
                memcpy1(Ctx.LoRaMacClassBParams.MlmeIndication->BeaconInfo.GwSpecific.Info,
                        &payload[phyParam.BeaconFormat.Rfu1Size + 4 + 2 + 1],
                        6);
            }
#elif (defined(LORAMAC_VERSION)                                                                    \
       && ((LORAMAC_VERSION == 0x01000400) || (LORAMAC_VERSION == 0x01010100)))
            ...
            beaconCrc0
                |= ((uint16_t)payload[phyParam.BeaconFormat.Rfu1Size + 1 + 4 + 1] << 8) & 0xFF00;
            ...
            if (crc0 == beaconCrc0)
            {
                ...
                // Fetch the precise time value in milliseconds that will be used for Rx ping slot
                // delay.
                Ctx.BeaconCtx.BeaconTimePrecision.SubSeconds
                    = BeaconPrecTimeValue[Ctx.LoRaMacClassBParams.MlmeIndication->BeaconInfo.Param];

                // Read Time field from the frame
                Ctx.BeaconCtx.BeaconTime.Seconds
                    = ((uint32_t)payload[phyParam.BeaconFormat.Rfu1Size + 1]) & 0x000000FF;
                Ctx.BeaconCtx.BeaconTime.Seconds
                    |= ((uint32_t)(payload[phyParam.BeaconFormat.Rfu1Size + 2] << 8)) & 0x0000FF00;
                Ctx.BeaconCtx.BeaconTime.Seconds
                    |= ((uint32_t)(payload[phyParam.BeaconFormat.Rfu1Size + 3] << 16)) & 0x00FF0000;
                Ctx.BeaconCtx.BeaconTime.Seconds
                    |= ((uint32_t)(payload[phyParam.BeaconFormat.Rfu1Size + 4] << 24)) & 0xFF000000;
                Ctx.BeaconCtx.BeaconTime.SubSeconds = 0;
                Ctx.LoRaMacClassBParams.MlmeIndication->BeaconInfo.Time = Ctx.BeaconCtx.BeaconTime;
                beaconProcessed = true;
            }
            ...
            // Read CRC2 field from the frame
            beaconCrc1 = ((uint16_t)payload[phyParam.BeaconFormat.Rfu1Size + 1 + 4 + 2 + 7
                                            + phyParam.BeaconFormat.Rfu2Size])
                         & 0x00FF;
            beaconCrc1 |= ((uint16_t)payload[phyParam.BeaconFormat.Rfu1Size + 1 + 4 + 2 + 7
                                             + phyParam.BeaconFormat.Rfu2Size + 1]
            ...
            if (crc1 == beaconCrc1)
            {
                ...
                memcpy1(Ctx.LoRaMacClassBParams.MlmeIndication->BeaconInfo.GwSpecific.Info,
                        &payload[phyParam.BeaconFormat.Rfu1Size + 1 + 4 + 2 + 1],
                        6);
            }
#endif /* LORAMAC_VERSION */
            ...
            if (beaconProcessed == true)
            {
                ...
                TimerTime_t time = Radio.TimeOnAir(MODEM_LORA,
                                                   bandwidth,
                                                   spreadingFactor,
                                                   1,
                                                   10,
        ...
        // downlink except the beacon frame itself. The reason for this is that no valid downlink
        // window is open. If it receives a frame which is
        // 1. not a beacon or
        // 2. a beacon with a crc fail
        // the MAC shall ignore the frame completely. Thus, the function must always return true,
        // even if no valid beacon has been received.
        beaconProcessed = true;
    }
...
{
#if (LORAMAC_CLASSB_ENABLED == 1)
    if ((Ctx.BeaconCtx.Ctrl.AcquisitionPending == 1) || (Ctx.BeaconState == BEACON_STATE_RX))
    {
        return true;
    }
    return false;
...
void LoRaMacClassBHaltBeaconing(void)
{
    ...
    if (Ctx.BeaconCtx.Ctrl.BeaconMode == 1)
    {
        if ((Ctx.BeaconState == BEACON_STATE_TIMEOUT) || (Ctx.BeaconState == BEACON_STATE_LOST))
        {
...
void LoRaMacClassBDeviceTimeAns(void)
{
    ...
    Ctx.BeaconCtx.LastBeaconRx
        = SysTimeSub(Ctx.BeaconCtx.NextBeaconRx,
...
TimerTime_t LoRaMacClassBIsUplinkCollision(TimerTime_t txTimeOnAir)
{
    ...
    beaconReserved = nextBeacon - CLASSB_BEACON_GUARD
                     ...
                     - Ctx.LoRaMacClassBParams.LoRaMacParams->ReceiveDelay2 - txTimeOnAir;
...
void LoRaMacClassBSetMulticastPeriodicity(MulticastCtx_t *multicastChannel)
{
    ...
    if (multicastChannel != NULL)
    {
        multicastChannel->PingNb
            = CalcPingNb(multicastChannel->ChannelParams.RxParams.Params.ClassB.Periodicity);
