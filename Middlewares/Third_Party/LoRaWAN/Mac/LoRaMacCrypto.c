 ...
 */
static LoRaMacCryptoStatus_t PayloadEncrypt(uint8_t *buffer,
                                            int16_t size,
 ...
 * \retval                      - Status of the operation
 */
static LoRaMacCryptoStatus_t FOptsEncrypt(uint16_t size,
                                          uint32_t address,
                                          uint8_t dir,
 ...
 * \retval                    - Status of the operation
 */
static LoRaMacCryptoStatus_t PrepareB0(uint16_t msgLen,
                                       KeyIdentifier_t keyID,
                                       bool isAck,
                                       uint8_t dir,
                                       uint32_t devAddr,
                                       ...
                                       uint8_t *b0)
{
    ...
    {
        // confFCnt contains the frame counter value modulo 2^16 of the "confirmed" uplink or
        // downlink frame that is being acknowledged
        uint16_t confFCnt = 0;
 ...
 * \retval                    - Status of the operation
 */
static LoRaMacCryptoStatus_t ComputeCmacB0(uint8_t *msg,
                                           uint16_t len,
                                           KeyIdentifier_t keyID,
                                           bool isAck,
                                           uint8_t dir,
 ...
 * \retval                    - Status of the operation
 */
static LoRaMacCryptoStatus_t VerifyCmacB0(uint8_t *msg,
                                          uint16_t len,
                                          KeyIdentifier_t keyID,
                                          bool isAck,
                                          uint8_t dir,
 ...
 * \retval                    - Status of the operation
 */
static LoRaMacCryptoStatus_t PrepareB1(uint16_t msgLen,
                                       bool isAck,
                                       uint8_t txDr,
                                       uint8_t txCh,
                                       uint32_t devAddr,
                                       ...
                                       uint8_t *b1)
{
    ...
    if (isAck == true)
    {
        // confFCnt contains the frame counter value modulo 2^16 of the "confirmed" uplink frame
 ...
 * \retval                    - Status of the operation
 */
static LoRaMacCryptoStatus_t ComputeCmacB1(uint8_t *msg,
                                           uint16_t len,
                                           KeyIdentifier_t keyID,
                                           bool isAck,
                                           uint8_t txDr,
 ...
 * \retval                    - Status of the operation
 */
static LoRaMacCryptoStatus_t PrepareB0ForDataBlock(uint16_t sessionCnt,
                                                   uint8_t fragIndex,
                                                   uint32_t descriptor,
                                                   uint32_t msgLen,
                                                   uint8_t *b0)
 ...
 * \retval                    - Status of the operation
 */
static LoRaMacCryptoStatus_t ComputeCmacB0ForDataBlock(uint8_t *msg,
                                                       uint32_t len,
                                                       KeyIdentifier_t keyID,
                                                       uint16_t sessionCnt,
                                                       uint8_t fragIndex,
...
LoRaMacCryptoStatus_t LoRaMacCryptoSetKey(KeyIdentifier_t keyID, uint8_t *key)
{
    ...
    if (keyID == APP_KEY)
    {
        ...
        if (LoRaMacCryptoDeriveLifeTimeKey(CryptoNvm->LrWanVersion.Fields.Minor, MC_ROOT_KEY)
...
LoRaMacCryptoStatus_t LoRaMacCryptoPrepareJoinRequest(LoRaMacMessageJoinRequest_t *macMsg)
{
    ...
    // Compute mic
    if (SecureElementComputeAesCmac(NULL,
                                    macMsg->Buffer,
...
LoRaMacCryptoStatus_t LoRaMacCryptoPrepareReJoinType1(LoRaMacMessageReJoinType1_t *macMsg)
{
    ...
    if (SecureElementComputeAesCmac(NULL,
                                    macMsg->Buffer,
                                    (LORAMAC_RE_JOIN_1_MSG_SIZE - LORAMAC_MIC_FIELD_SIZE),
                                    J_S_INT_KEY,
                                    &macMsg->MIC)
        != SECURE_ELEMENT_SUCCESS)
    {
                                                    ...
                                                    LoRaMacMessageJoinAccept_t *macMsg)
{
    ...
    {
        // If Join-accept is a reply to a rejoin, the RJcount(0 or 1) replaces DevNonce in the key
        // derivation process.
    ...
    if (SecureElementProcessJoinAccept(joinReqType,
                                       joinEUI,
                                       nonce,
                                       macMsg->Buffer,
                                       macMsg->BufSize,
                                                 ...
                                                 LoRaMacMessageData_t *macMsg)
{
    ...
    if (fCntUp > CryptoNvm->FCntList.FCntUp)
    {
        retval = PayloadEncrypt(macMsg->FRMPayload,
                                macMsg->FRMPayloadSize,
                                payloadDecryptionKeyID,
                                macMsg->FHDR.DevAddr,
                                UPLINK,
                                fCntUp);
        if (retval != LORAMAC_CRYPTO_SUCCESS)
        ...
        {
            // Encrypt FOpts
            retval = FOptsEncrypt(macMsg->FHDR.FCtrl.Bits.FOptsLen,
                                  macMsg->FHDR.DevAddr,
                                  UPLINK,
                                  FCNT_UP,
                                  fCntUp,
...
#endif /* LORAMAC_VERSION */
        // MIC = cmacF[0..3]
        // The IsAck parameter is every time false since the ConfFCnt field is not used in legacy
        // mode.
        retval = ComputeCmacB0(macMsg->Buffer,
                               (macMsg->BufSize - LORAMAC_MIC_FIELD_SIZE),
                               payloadDecryptionKeyID,
                                                   ...
                                                   LoRaMacMessageData_t *macMsg)
{
    ...
    if (CryptoNvm->LrWanVersion.Fields.Minor == 0)
    {
        // In legacy mode the IsAck parameter is forced to be false since the ConfFCnt field is not
        // used.
        isAck = false;
    }

    // Verify mic
    retval = VerifyCmacB0(macMsg->Buffer,
                          (macMsg->BufSize - LORAMAC_MIC_FIELD_SIZE),
                          micComputationKeyID,
                          isAck,
                          DOWNLINK,
                                                    ...
                                                    uint32_t *cmac)
{
    ...
    // Compute mic
    retval = ComputeCmacB0ForDataBlock(buffer,
                                       size,
                                       DATABLOCK_INT_KEY,
                                       sessionCnt,
                                       fragIndex,
                                                          ...
                                                          uint32_t mcAddr)
{
    ...
    if (SecureElementDeriveAndStoreKey(compBaseAppS, curItem->RootKey, curItem->AppSkey)
        != SECURE_ELEMENT_SUCCESS)
    {
        return LORAMAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
    }

    if (SecureElementDeriveAndStoreKey(compBaseNwkS, curItem->RootKey, curItem->NwkSkey)
        != SECURE_ELEMENT_SUCCESS)
    {
        return LORAMAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
    }
