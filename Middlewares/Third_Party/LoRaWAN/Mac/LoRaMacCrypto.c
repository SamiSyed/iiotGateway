 ...
 * ConfFCnt in B0 block ) \param [in] devAddr        - Device address \param
 * [in] dir            - Frame direction ( Uplink:0, Downlink:1 ) \param [in]
 * fCnt           - Frame counter \param [in,out] b0         - B0 block \retval
 ...
 * ConfFCnt in B0 block ) \param [in] devAddr        - Device address \param
 * [in] dir            - Frame direction ( Uplink:0, Downlink:1 ) \param [in]
 * fCnt           - Frame counter \param [out] cmac          - Computed cmac
 * \retval                    - Status of the operation
 */
 ...
 * ConfFCnt in B0 block ) \param [in] devAddr        - Device address \param
 * [in] dir            - Frame direction ( Uplink:0, Downlink:1 ) \param [in]
 * fCnt           - Frame counter \param [in] expectedCmac   - Expected cmac
 * \retval                    - Status of the operation
 */
 ...
 * operation
 */
static LoRaMacCryptoStatus_t PrepareB1(uint16_t msgLen, bool isAck,
...
PrepareB0ForDataBlock(uint16_t sessionCnt, uint8_t fragIndex,
                      uint32_t descriptor, uint32_t msgLen, uint8_t *b0) {
  if (b0 == 0) {
    return LORAMAC_CRYPTO_ERROR_NPE;
  }
 ...
 */
static LoRaMacCryptoStatus_t
ComputeCmacB0ForDataBlock(uint8_t *msg, uint32_t len, KeyIdentifier_t keyID,
...
LoRaMacCryptoStatus_t LoRaMacCryptoSetKey(KeyIdentifier_t keyID, uint8_t *key) {
  ...
  if (keyID == APP_KEY) {
    ...
    if (LoRaMacCryptoDeriveLifeTimeKey(CryptoNvm->LrWanVersion.Fields.Minor,
                                       MC_ROOT_KEY) != LORAMAC_CRYPTO_SUCCESS) {
      return LORAMAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
    }
    if (LoRaMacCryptoDeriveLifeTimeKey(0, MC_KE_KEY) !=
        LORAMAC_CRYPTO_SUCCESS) {
      return LORAMAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
    }
    if (LoRaMacCryptoDeriveLifeTimeKey(0, DATABLOCK_INT_KEY) !=
        LORAMAC_CRYPTO_SUCCESS) {
      return LORAMAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
    }
  }
...
LoRaMacCryptoStatus_t LoRaMacCryptoSecureMessage(uint32_t fCntUp, uint8_t txDr,
...
#endif /* LORAMAC_VERSION */
  {
    ...
    retval = ComputeCmacB0(macMsg->Buffer,
                           (macMsg->BufSize - LORAMAC_MIC_FIELD_SIZE),
                           payloadDecryptionKeyID, false, UPLINK,
                           macMsg->FHDR.DevAddr, fCntUp, &macMsg->MIC);
    if (retval != LORAMAC_CRYPTO_SUCCESS) {
      return retval;
    }
  }
...
LoRaMacCryptoComputeDataBlock(uint8_t *buffer, uint32_t size,
  ...
  // Compute mic
  retval = ComputeCmacB0ForDataBlock(buffer, size, DATABLOCK_INT_KEY,
                                     sessionCnt, fragIndex, descriptor, cmac);
  if (retval != LORAMAC_CRYPTO_SUCCESS) {
    return retval;
  }
