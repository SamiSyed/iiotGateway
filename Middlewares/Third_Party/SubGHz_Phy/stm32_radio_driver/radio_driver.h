 ...
 * @note   These RAM addresses are used to control accurately ramp-up, ramp-down and length of a
 * frame
 */
...
typedef enum
{
    RADIO_PACKET_FIXED_LENGTH = 0x00, //!< The packet is known on both sides, no header included in
                                      //!< the packet
...
typedef enum
{
    ...
    LORA_PACKET_FIXED_LENGTH = 0x01, //!< The packet is known on both sides, no header included in
                                     //!< the packet
...
typedef struct
{
    ...
    struct
    {
        ...
        struct
        {
            RadioLoRaSpreadingFactors_t SpreadingFactor; //!< Spreading Factor for the LoRa
                                                         //!< modulation
            RadioLoRaBandwidths_t Bandwidth;             //!< Bandwidth for the LoRa modulation
            RadioLoRaCodingRates_t CodingRate;           //!< Coding rate for the LoRa modulation
            uint8_t LowDatarateOptimize; //!< Indicates if the modem uses the low datarate
                                         //!< optimization
        } LoRa;
...
typedef struct
{
    ...
    struct
    {
        ...
        struct
        {
            ...
            RadioPreambleDetection_t PreambleMinDetect; //!< The preamble Rx length minimal for GFSK
                                                        //!< packet type
            ...
            RadioPacketLengthModes_t HeaderType; //!< If the header is explicit, it will be
        ...
        struct
        {
            uint16_t PreambleLength; //!< The preamble length is the number of LoRa symbols in the
                                     //!< preamble
            RadioLoRaPacketLengthsMode_t HeaderType; //!< If the header is explicit, it will be
                                                     //!< transmitted in the LoRa packet. If the
                                                     //!< header is implicit, it will not be
                                                     //!< transmitted
 ...
 *
 *         Please note that the random numbers produced by the generator do not have a uniform or
 ...
 */
void SUBGRF_SetDioIrqParams(uint16_t irqMask,
                            uint16_t dio1Mask,
                            uint16_t dio2Mask,
                            uint16_t dio3Mask);
 ...
 */
void SUBGRF_SetCadParams(RadioLoRaCadSymbols_t cadSymbolNum,
                         uint8_t cadDetPeak,
                         uint8_t cadDetMin,
...
/*!
 * \brief SUBGRF_GetCFO get the frequency offset between the remote transmitter and the radio
 * receiver
 *
