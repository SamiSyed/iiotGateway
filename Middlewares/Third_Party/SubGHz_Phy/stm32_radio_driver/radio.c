...
typedef struct SubgRf_s
{
    ...
    uint32_t RxDcPreambleDetectTimeout; /* 0:RxDutyCycle is off, otherwise on with  2*rxTime +
                                           sleepTime (See STM32WL Errata: RadioSetRxDutyCycle)*/
 ...
 */
static bool RadioIsChannelFree(uint32_t freq,
 ...
 */
static void RadioSetRxConfig(RadioModems_t modem,
                             uint32_t bandwidth,
                             uint32_t datarate,
                             uint8_t coderate,
 ...
 */
static void RadioSetTxConfig(RadioModems_t modem,
                             int8_t power,
                             uint32_t fdev,
                             uint32_t bandwidth,
                             uint32_t datarate,
 ...
 */
static uint32_t RadioTimeOnAir(RadioModems_t modem,
                               uint32_t bandwidth,
                               uint32_t datarate,
                               uint8_t coderate,
                               uint16_t preambleLen,
 ...
 */
const struct Radio_s Radio = {RadioInit,
                              RadioGetStatus,
                              RadioSetModem,
                              RadioSetChannel,
                              RadioIsChannelFree,
                             ...
                             bool rxContinuous)
{
    ...
    case MODEM_SIGFOX_RX:
        ...
        SubgRf.PacketParams.Params.Gfsk.PreambleLength = (preambleLen << 3); // convert byte into
                                                                             // bit
        ...
        /* Use of new bit synchronizer (to avoid CRC errors during PER for payloads with a small
         * amount of transitions). Reg 0x6ac, bits[4 :6] = 0x5 */
    ...
    case MODEM_FSK:
        ...
        SubgRf.PacketParams.Params.Gfsk.HeaderType
            = (fixLen == true) ? RADIO_PACKET_FIXED_LENGTH : RADIO_PACKET_VARIABLE_LENGTH;
        SubgRf.PacketParams.Params.Gfsk.PayloadLength = MaxPayloadLength;
        if (crcOn == true)
        {
    ...
    case MODEM_LORA:
        ...
        /* WORKAROUND - Set the step threshold value to 1 to avoid to miss low power signal after an
         * interferer jam the chip in LoRa modulaltion */
                               ...
                               bool crcOn)
{
    ...
    {
        numerator = 1000U
                    * RadioGetGfskTimeOnAirNumerator(datarate,
                                                     coderate,
                                                     preambleLen,
                                                     fixLen,
                                                     payloadLen,
    ...
    {
        numerator = 1000U
                    * RadioGetLoRaTimeOnAirNumerator(bandwidth,
                                                     datarate,
                                                     coderate,
                                                     preambleLen,
                                                     fixLen,
...
static radio_status_t RadioSend(uint8_t *buffer, uint8_t size)
{
    ...
    {
        uint32_t hop_sequence_count
            = lr_fhss_get_hop_sequence_count(&SubgRf.lr_fhss.lr_fhss_params.lr_fhss_params);
        ...
        if (RADIO_STATUS_OK
            != wl_lr_fhss_build_frame(&SubgRf.lr_fhss.lr_fhss_params,
...
static void RadioIrqProcess(void)
{
    ...
    case IRQ_RX_DONE:
        ...
        if ((RadioEvents != NULL) && (RadioEvents->RxDone != NULL))
        {
            ...
            case PACKET_TYPE_LORA:
                RadioEvents->RxDone(RadioBuffer,
                                    size,
    ...
    case IRQ_PREAMBLE_DETECTED:
        ...
        if (SubgRf.RxDcPreambleDetectTimeout != 0)
        {
            ...
            Radio.Write(SUBGHZ_RTCPRDR2, (SubgRf.RxDcPreambleDetectTimeout >> 16) & 0xFF); /*Update
                                                                                              Radio
                                                                                              RTC
                                                                                              Period
                                                                                              MSB*/
            Radio.Write(SUBGHZ_RTCPRDR1,
                        (SubgRf.RxDcPreambleDetectTimeout >> 8) & 0xFF); /*Update Radio RTC Period
                                                                            MidByte*/
            Radio.Write(SUBGHZ_RTCPRDR0, (SubgRf.RxDcPreambleDetectTimeout) & 0xFF); /*Update Radio
                                                                                        RTC Period
                                                                                        lsb*/
                                       ...
                                       uint32_t symbTimeout)
{
    ...
    case GENERIC_FSK:
        ...
        SUBGRF_SetStopRxTimerOnPreambleDetect((config->fsk.StopTimerOnPreambleDetect == 0) ? false
                                                                                           : true);
        ...
        SubgRf.PacketParams.PacketType = PACKET_TYPE_GFSK;
        SubgRf.PacketParams.Params.Gfsk.PreambleLength = (config->fsk.PreambleLen) << 3; // convert
                                                                                         // byte
                                                                                         // into bit
        SubgRf.PacketParams.Params.Gfsk.PreambleMinDetect
            = (RadioPreambleDetection_t)config->fsk.PreambleMinDetect;
        SubgRf.PacketParams.Params.Gfsk.SyncWordLength = (config->fsk.SyncWordLength) << 3; // convert
                                                                                            // byte
                                                                                            // into
                                                                                            // bit
        ...
        {
            /* Set max in the radio, in long packet mode will be tuned based dynamically on received
             * chunk */
    ...
    case GENERIC_LORA:
        ...
        SUBGRF_SetStopRxTimerOnPreambleDetect((config->lora.StopTimerOnPreambleDetect == 0) ? false
                                                                                            : true);
        SUBGRF_SetLoRaSymbNumTimeout(symbTimeout);
                                       ...
                                       uint32_t timeout)
{
    ...
    case GENERIC_MSK:
        ...
        SubgRf.PacketParams.Params.Gfsk.PreambleLength = (config->msk.PreambleLen) << 3; // convert
                                                                                         // byte
                                                                                         // into bit
        SubgRf.PacketParams.Params.Gfsk.PreambleMinDetect = RADIO_PREAMBLE_DETECTOR_08_BITS; // don't
                                                                                             // care
                                                                                             // in
                                                                                             // tx
        SubgRf.PacketParams.Params.Gfsk.SyncWordLength = (config->msk.SyncWordLength) << 3; // convert
                                                                                            // byte
                                                                                            // into
                                                                                            // bit
