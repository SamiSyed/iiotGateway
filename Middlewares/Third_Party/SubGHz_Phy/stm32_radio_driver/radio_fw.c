 ...
 * In RFWPacket mode, the packet is 'Fixed length' without Hw Crc check and
 * without Hw (de-)whitening
 */
typedef struct {
  ...
  uint16_t
      CrcPolynomial; /* Init Crc polynomial, to set before running RFW_CrcRun*/
  uint16_t CrcSeed;  /* Init Crc seed to set before running RFW_CrcRun*/
  RADIO_FSK_CrcTypes_t
      CrcType;        /* Init Crc types to set before running RFW_CrcRun*/
  uint16_t WhiteSeed; /* Init whitening seed, to set before running
                         Radio_FwWhiteRun*/
  uint16_t
      LongPacketMaxRxLength; /* Maximum expected amount of bytes in payload*/
  RadioModems_t Modem;
...
typedef struct {
  ...
  uint16_t CrcLfsrState; /* State of LFSR crc, set from CrcSeed at beginning of
                            each payload*/
  uint16_t WhiteLfsrState; /* State of LFSR whitening, set from WhiteSeed at
                              beginning of each payload use to save LFSR state
                              after rx payload length de-whitening*/
  uint16_t
      PayloadLength; /* In Rx, Payload length is first byte(s) of the payload
                        excluding CrcFieldSize and PayloadLengthFieldSize*/
  uint8_t LongPacketModeEnable; /* set to one when RFW_TransmitLongPacket or
                                   RFW_ReceiveLongPacket. 0 otherwise*/
  TimerEvent_t Timer;           /* Timer to get/Set Rx(Tx)Bytes*/
  uint16_t LongPacketRemainingBytes; /* Count remaining bytes to send receive
                                        (including crc)*/
...
#define LONGPACKET_CHUNK_LENGTH_BYTES                                          \
  ((int32_t)128) /* bytes (half Radio fifo) */
...
#define RFW_TRANSMIT_LONGPACKET_TX_CHUNK_PROCESS()                             \
  RFW_TransmitLongPacket_TxChunkProcess()
 ...
 * @param [in] CrcSeed               the Initial seed of the Crc types
 * (Ibm/CCITT)
 */
static void RFW_CrcInitState(RFwInit_t *Init, const uint16_t CrcPolynomial,
 ...
 */
static int32_t RFW_CrcRun(RadioFw_t *const RFWPacket, const uint8_t *Payload,
                          const uint32_t Size, uint8_t CrcResult[2]);
 ...
 *
 * @param [out] PayloadLength        the length of PayloadOnly excluding
 * CrcLengthField
...
/*!
 * @brief RFW_GetPayloadTimerEvent TimerEvent to get the payload data,
 * de-whitening, and crc verification
 *
 * @Note calls RFW_GetPayloadProcess directly inside IRQ (default) or in
 * background with sequencer or OS
 * @param [in] context        context of the timer
 */
static void RFW_GetPayloadTimerEvent(void *context);

/*!
 * @brief RFW_GetPayloadProcess process to get the payload data, de-whitening,
...
/*!
 * @brief RFW_TransmitLongPacket_NewTxChunkTimer: long packet Tx timer callback
 * to process intermediate chunk of TxData
 * @Note calls RFW_TransmitLongPacket_TxChunkProcess directly inside IRQ
 * (default) or in background with sequencer or any Rtos
 *
...
/*!
 * @brief RFW_TransmitLongPacket_TxChunkProcess: tx long packet process
 * intermediate chunk of TxData
 */
...
int32_t RFW_TransmitLongPacket(
  ...
  int32_t status = 0;
#if (RFW_LONGPACKET_ENABLE == 1)
  uint32_t total_size = payload_size + RFWPacket.Init.PayloadLengthFieldSize +
                        RFWPacket.Init.CrcFieldSize;

  RFW_MW_LOG(TS_ON, VLEVEL_M, "RevID=%04X\r\n", LL_DBGMCU_GetRevisionID());

  if ((TxLongPacketGetNextChunkCb == NULL) ||
      (payload_size >
       (1 << (8 * RFWPacket.Init.PayloadLengthFieldSize)) -
           1) || /*check that size fits inside the packetLengthField*/
      (RFWPacket.Init.Enable ==
       0) || /* Can only be used when after RadioSetTxGenericConfig*/
      (LL_DBGMCU_GetRevisionID() <
       0x1003)) /* Only available from stm32wl revision Y*/
  {
    status = -1;
  } else {
    ...
    case MODEM_MSK: {
      if (RFWPacket.Init.Enable == 1) {
        /*crc will be calculated on the fly along with packet chunk
         * transmission*/
        ...
        if (total_size > RADIO_BUF_SIZE) {
          ...
          if (total_size < RADIO_BUF_SIZE + RFWPacket.Init.CrcFieldSize) {
            ...
            chunk_size = RADIO_BUF_SIZE -
                         RFWPacket.Init.PayloadLengthFieldSize -
                         RFWPacket.Init.CrcFieldSize;
          } else {
            chunk_size = RADIO_BUF_SIZE - RFWPacket.Init.PayloadLengthFieldSize;
          }
          /*Set crc size for the crc calculation: no crc here because it is not
        ...
        RADIO_MEMCPY8(&ChunkBuffer[RFWPacket.Init.PayloadLengthFieldSize],
                      app_chunk_buffer_ptr, chunk_size);
        ...
        if (RFWPacket.Init.CrcEnable == 1) {
          ...
          RFW_CrcRun(&RFWPacket, ChunkBuffer,
                     RFWPacket.Init.PayloadLengthFieldSize + chunk_size,
                     crc_result);
          /* Append the crc result after the payload if total_size<=
           * RADIO_BUF_SIZE*/
          RADIO_MEMCPY8(
              &ChunkBuffer[RFWPacket.Init.PayloadLengthFieldSize + chunk_size],
              crc_result, crc_size);
        }
        /* Init whitening at beginning of the packet*/
        RFW_WhiteSetState(&RFWPacket);
        /* Run the whitening calculation on payload length, payload and crc if
         * crc fits inside 1st chunk*/
        RFW_WhiteRun(&RFWPacket, &ChunkBuffer[0],
                     RFWPacket.Init.PayloadLengthFieldSize + chunk_size +
                         crc_size);
        /* Configure the Transmitter to send all*/
        /* Init radio buffer */
        SUBGRF_WriteRegister(SUBGHZ_GRTXPLDLEN,
                             RFWPacket.Init.PayloadLengthFieldSize +
                                 chunk_size + crc_size);
        ...
        SUBGRF_SendPayload(
            ChunkBuffer,
            RFWPacket.Init.PayloadLengthFieldSize + chunk_size + crc_size, 0);
        if (total_size > RADIO_BUF_SIZE) {
          /*in case total size is greater than RADIO_BUF_SIZE, need to program a
           * timer to get next chunk*/
          /*RFWPacket.LongPacketRemainingBytes-=
           * RFWPacket.Init.PayloadLengthFieldSize+ chunk_size+ crc_size;*/
          /*Initialize Timer to get new chunk and update radio ptr*/
          chunk_timeout =
              (LONGPACKET_CHUNK_LENGTH_BYTES * 8 * 1000) / RFWPacket.BitRate;
          RFW_MW_LOG(TS_ON, VLEVEL_M, "Timeout=%d,\r\n", chunk_timeout);
          TimerInit(&RFWPacket.Timer,
                    RFW_TransmitLongPacket_NewTxChunkTimerEvent);
...
int32_t RFW_ReceiveLongPacket(
  ...
  if ((RxLongPacketStoreChunkCb == NULL) ||
      (RFWPacket.Init.Enable ==
       0)) /* Can only be used when after RadioSetRxGenericConfig*/
  {
    status = -1;
  } else {
    /*Records call back*/
...
int32_t RFW_Init(ConfigGeneric_t *config, RadioEvents_t *RadioEvents,
  ...
  /*record, used to reject packet in length decoded at sync time out greater
   * than LongPacketMaxRxLength*/
...
#if (RFW_ENABLE == 1)
  uint8_t crc_result[2];
  if (size + RFWPacket.Init.PayloadLengthFieldSize +
          RFWPacket.Init.CrcFieldSize >
      RADIO_BUF_SIZE) {
    ...
    if (RFWPacket.Init.CrcEnable == 1) {
      ...
      RFW_CrcSetState(&RFWPacket);
      /*Run the crc calculation on payload length and payload*/
      RFW_CrcRun(&RFWPacket, &ChunkBuffer[0],
                 size + RFWPacket.Init.PayloadLengthFieldSize, crc_result);
      /*append the crc result after the payload*/
      RADIO_MEMCPY8(&ChunkBuffer[size + RFWPacket.Init.PayloadLengthFieldSize],
                    crc_result, RFWPacket.Init.CrcFieldSize);
    }
    /*init whitening at beginning of the packet*/
    RFW_WhiteSetState(&RFWPacket);
    /*Run the whitening calculation on payload length, payload and crc*/
    RFW_WhiteRun(&RFWPacket, &ChunkBuffer[0],
                 size + RFWPacket.Init.PayloadLengthFieldSize +
                     RFWPacket.Init.CrcFieldSize);
    /*Configure the Transmitter to send all*/
    *outSize = (uint8_t)size + RFWPacket.Init.PayloadLengthFieldSize +
               RFWPacket.Init.CrcFieldSize;
    /*copy result*/
...
void RFW_DeInit_TxLongPacket(void) {
  ...
  /*long packet WA*/
  uint8_t reg = SUBGRF_ReadRegister(SUBGHZ_GPKTCTL1AR);
  SUBGRF_WriteRegister(SUBGHZ_GPKTCTL1AR,
                       reg & ~0x02); /* clear infinite_sequence bit */
...
void RFW_ReceivePayload(void) {
  ...
  if (RFW_GetPacketLength(&PayloadLength) == 0) {
    ...
    /*if decoded PayloadLength is longer than LongPacketMaxRxLength, reject
     * packet*/
    ...
    if (packet_length < LONGPACKET_CHUNK_LENGTH_BYTES) {
      ...
      timeout =
          DIVR(LONGPACKET_CHUNK_LENGTH_BYTES * 8 * 1000, RFWPacket.BitRate);
    }
...
static void RFW_TransmitLongPacket_TxChunkProcess(void) {
  ...
  uint8_t read_ptr = SUBGRF_ReadRegister(
      SUBGHZ_TXADRPTR); /*radio has transmitted up to read_ptr*/
  uint8_t write_ptr = SUBGRF_ReadRegister(
      SUBGHZ_GRTXPLDLEN); /*from read_ptr to write_ptr still to be transmitted*/
  ...
  RFW_MW_LOG(TS_ON, VLEVEL_M,
             "read_ptr=%d, write_ptr=%d, bytes_sent=%d, "
             "bytes_loaded=%d,remaining to send=%d\r\n",
             read_ptr, write_ptr, bytes_sent, bytes_loaded,
             RFWPacket.LongPacketRemainingBytes);
  if (RFWPacket.LongPacketRemainingBytes > 256) {
    /*get next chunk */
    /*make sure that at least full CrcFieldSize will be loaded for the last
     * chunk*/
    if (RFWPacket.LongPacketRemainingBytes >
        256 + RFWPacket.Init.CrcFieldSize) {
  ...
  RFWPacket.TxLongPacketGetNextChunkCb(&app_chunk_buffer_ptr,
                                       chunk_size - crc_size);
...
static void RFW_WhiteRun(RadioFw_t *RFWPacket, uint8_t *Payload,
                         uint32_t Size) {
  ...
  for (int32_t i = 0; i < Size; i++) {
    ...
    for (int32_t j = 0; j < 8; j++) {
      uint8_t msb =
          ((ibmwhite_state >> 5) & 0x1) ^ ((ibmwhite_state >> 0) & 0x1);
...
static int32_t RFW_GetPacketLength(uint16_t *PayloadLength) {
  ...
  if (RFWPacket.Init.CrcEnable == 1) {
    ...
    uint8_t crc_dummy[2];
    RFW_CrcRun(&RFWPacket, ChunkBuffer, RFWPacket.Init.PayloadLengthFieldSize,
               crc_dummy);
  }
  if (RFWPacket.Init.PayloadLengthFieldSize == 1) {
...
static void RFW_GetPayloadProcess(void) {
  ...
  if (RFWPacket.LongPacketRemainingBytes > size) {
    ...
    if (RFWPacket.LongPacketRemainingBytes < LONGPACKET_CHUNK_LENGTH_BYTES) {
      ...
      Timeout = DIVC((RFWPacket.LongPacketRemainingBytes) * 8 * 1000,
                     RFWPacket.BitRate) +
                2;
    } else if (RFWPacket.LongPacketRemainingBytes <
               (3 * LONGPACKET_CHUNK_LENGTH_BYTES) / 2) {
      /*this is to make sure that last chunk will always be greater than
       * LONGPACKET_CHUNK_LENGTH_BYTES/2 */
      Timeout = DIVR((RFWPacket.LongPacketRemainingBytes / 2) * 8 * 1000,
                     RFWPacket.BitRate);
    } else {
      /*size value is close to LONGPACKET_CHUNK_LENGTH_BYTES with +/- errors
       * compensated in closed loop here*/
      Timeout =
          DIVR((LONGPACKET_CHUNK_LENGTH_BYTES)*8 * 1000, RFWPacket.BitRate);
    }
    TimerSetValue(&RFWPacket.Timer, Timeout);
    ...
    if (RFWPacket.LongPacketRemainingBytes < RFWPacket.Init.CrcFieldSize) {
      /* force LongPacketRemainingBytes to CrcFieldSize: this should never
       * happen*/
      RFWPacket.LongPacketRemainingBytes = RFWPacket.Init.CrcFieldSize;
    }
    /*last chunk*/
    RFW_MW_LOG(
        TS_ON, VLEVEL_M, "LastChunk. offset= %d, size=%d, remaining=%d,\r\n",
...
static void RFW_GetPayload(uint8_t Offset, uint8_t Length) {
  ...
  if (RFWPacket.Init.CrcEnable == 1) {
    RFW_CrcRun(&RFWPacket, ChunkBuffer, Length - RFWPacket.Init.CrcFieldSize,
               crc_result);
  }
  if (RFWPacket.LongPacketModeEnable == 1) {
    /*report rx data chunk to application*/

    RFWPacket.RxLongPacketStoreChunkCb(ChunkBuffer,
                                       Length - RFWPacket.Init.CrcFieldSize);
  } else {
    if (RFWPacket.RxPayloadOffset + Length - RFWPacket.Init.CrcFieldSize <
        RADIO_BUF_SIZE) {
      RADIO_MEMCPY8(&RxBuffer[RFWPacket.RxPayloadOffset], ChunkBuffer,
                    Length - RFWPacket.Init.CrcFieldSize);
  ...
  /* CRC check*/
  RFW_MW_LOG(TS_ON, VLEVEL_M,
             "crc_result= 0x%02X%02X, crc_payload=0x%02X%02X\r\n",
             crc_result[0], crc_result[1], ChunkBuffer[Length - 2],
  ...
  if (((crc_result[0] == ChunkBuffer[Length - 2]) &&
    ...
    RFWPacket.Init.RadioEvents->RxDone(RxBuffer, RFWPacket.RxPayloadOffset,
                                       -(rssi_sync >> 1),
