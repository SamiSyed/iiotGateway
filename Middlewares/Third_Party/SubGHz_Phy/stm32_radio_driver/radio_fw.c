 ...
 * In RFWPacket mode, the packet is 'Fixed length' without Hw Crc check and without Hw
 * (de-)whitening
 */
typedef struct
{
...
typedef struct
{
    ...
    uint16_t WhiteLfsrState; /* State of LFSR whitening, set from WhiteSeed at beginning of each
                                payload use to save LFSR state after rx payload length
                                de-whitening*/
 ...
 */
static void RFW_CrcInitState(RFwInit_t *Init,
                             const uint16_t CrcPolynomial,
 ...
 */
static int32_t RFW_CrcRun(RadioFw_t *const RFWPacket,
...
/*!
 * @brief RFW_GetPayloadTimerEvent TimerEvent to get the payload data, de-whitening, and crc
 * verification
 *
 * @Note calls RFW_GetPayloadProcess directly inside IRQ (default) or in background with sequencer
 * or OS
...
/*!
 * @brief RFW_TransmitLongPacket_NewTxChunkTimer: long packet Tx timer callback to process
 * intermediate chunk of TxData
 * @Note calls RFW_TransmitLongPacket_TxChunkProcess directly inside IRQ (default) or in background
 * with sequencer or any Rtos
 *
...
#if (RFW_LONGPACKET_ENABLE == 1)
    uint32_t total_size
        = payload_size + RFWPacket.Init.PayloadLengthFieldSize + RFWPacket.Init.CrcFieldSize;

    RFW_MW_LOG(TS_ON, VLEVEL_M, "RevID=%04X\r\n", LL_DBGMCU_GetRevisionID());

    if ((TxLongPacketGetNextChunkCb == NULL)
        || (payload_size > (1 << (8 * RFWPacket.Init.PayloadLengthFieldSize)) - 1)
        ||                              /*check that size fits inside the packetLengthField*/
        ...
        case MODEM_MSK:
        {
            if (RFWPacket.Init.Enable == 1)
            {
                ...
                if (total_size > RADIO_BUF_SIZE)
                {
                    ...
                    /*Set crc size for the crc calculation: no crc here because it is not the end of
                     * the packet*/
                    crc_size = 0;
                }
                else
                ...
                if (RFWPacket.Init.CrcEnable == 1)
                {
                    ...
                    RFW_CrcRun(&RFWPacket,
                               ChunkBuffer,
                               RFWPacket.Init.PayloadLengthFieldSize + chunk_size,
                               crc_result);
                    /* Append the crc result after the payload if total_size<= RADIO_BUF_SIZE*/
                    RADIO_MEMCPY8(&ChunkBuffer[RFWPacket.Init.PayloadLengthFieldSize + chunk_size],
                                  crc_result,
                                  crc_size);
                }
                /* Init whitening at beginning of the packet*/
                RFW_WhiteSetState(&RFWPacket);
                /* Run the whitening calculation on payload length, payload and crc if crc fits
                 * inside 1st chunk*/
                RFW_WhiteRun(&RFWPacket,
                             &ChunkBuffer[0],
                ...
                /* Send*/
                SUBGRF_SendPayload(ChunkBuffer,
                                   RFWPacket.Init.PayloadLengthFieldSize + chunk_size + crc_size,
                                   0);
                if (total_size > RADIO_BUF_SIZE)
                {
                    /*in case total size is greater than RADIO_BUF_SIZE, need to program a timer to
                     * get next chunk*/
                    /*RFWPacket.LongPacketRemainingBytes-= RFWPacket.Init.PayloadLengthFieldSize+
                     * chunk_size+ crc_size;*/
                 ...
                 TimerEvent_t *TimeoutTimerEvent)
{
    ...
    /*record, used to reject packet in length decoded at sync time out greater than
     * LongPacketMaxRxLength*/
...
int32_t RFW_TransmitInit(uint8_t *inOutBuffer, uint8_t size, uint8_t *outSize)
{
    ...
    else
    {
        ...
        RFW_WhiteRun(&RFWPacket,
                     &ChunkBuffer[0],
                     size + RFWPacket.Init.PayloadLengthFieldSize + RFWPacket.Init.CrcFieldSize);
        /*Configure the Transmitter to send all*/
        *outSize
            = (uint8_t)size + RFWPacket.Init.PayloadLengthFieldSize + RFWPacket.Init.CrcFieldSize;
        /*copy result*/
...
static void RFW_TransmitLongPacket_TxChunkProcess(void)
{
    ...
    uint8_t read_ptr = SUBGRF_ReadRegister(SUBGHZ_TXADRPTR);    /*radio has transmitted up to
                                                                   read_ptr*/
    uint8_t write_ptr = SUBGRF_ReadRegister(SUBGHZ_GRTXPLDLEN); /*from read_ptr to write_ptr still
                                                                   to be transmitted*/
    ...
    RFW_MW_LOG(TS_ON,
               VLEVEL_M,
               "read_ptr=%d, write_ptr=%d, bytes_sent=%d, bytes_loaded=%d,remaining to send=%d\r\n",
               read_ptr,
               write_ptr,
               bytes_sent,
               bytes_loaded,
    ...
    RFW_MW_LOG(TS_ON,
               VLEVEL_M,
               "next chunk size=%d, new write ptr=%d\n\r",
               chunk_size + crc_size,
...
static void RFW_GetPayloadProcess(void)
{
    ...
    if (RFWPacket.LongPacketRemainingBytes > size)
    {
        ...
        RFW_MW_LOG(TS_ON,
                   VLEVEL_M,
                   "offset= %d, size=%d, remaining=%d,\r\n",
                   RFWPacket.RadioBufferOffset,
                   size,
                   RFWPacket.LongPacketRemainingBytes);
        ...
        {
            /*size value is close to LONGPACKET_CHUNK_LENGTH_BYTES with +/- errors compensated in
             * closed loop here*/
            Timeout = DIVR((LONGPACKET_CHUNK_LENGTH_BYTES)*8 * 1000, RFWPacket.BitRate);
        }
        TimerSetValue(&RFWPacket.Timer, Timeout);
    ...
    else
    {
        ...
        RFW_MW_LOG(TS_ON,
                   VLEVEL_M,
                   "LastChunk. offset= %d, size=%d, remaining=%d,\r\n",
                   RFWPacket.RadioBufferOffset,
                   size,
                   RFWPacket.LongPacketRemainingBytes);
...
static void RFW_GetPayload(uint8_t Offset, uint8_t Length)
{
    ...
    else
    {
        ...
        {
            RADIO_MEMCPY8(&RxBuffer[RFWPacket.RxPayloadOffset],
                          ChunkBuffer,
    ...
    RFW_MW_LOG(TS_ON,
               ...
               crc_result[0],
               crc_result[1],
               ChunkBuffer[Length - 2],
               ChunkBuffer[Length - 1]);
    if (((crc_result[0] == ChunkBuffer[Length - 2]) && (crc_result[1] == ChunkBuffer[Length - 1]))
        || (RFWPacket.Init.CrcEnable == 0))
    {
