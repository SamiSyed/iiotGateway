...
void SUBGRF_SetSleep(SleepParams_t sleepConfig)
{
    ...
    uint8_t value
        = (((uint8_t)sleepConfig.Fields.WarmStart << 2) | ((uint8_t)sleepConfig.Fields.Reset << 1)
...
void SUBGRF_SetRxBoosted(uint32_t timeout)
{
    ...
    SUBGRF_WriteRegister(REG_RX_GAIN, 0x97); // max LNA gain, increase current by ~2mA for around
                                             // ~3dB in sensitivity
...
void SUBGRF_SetTxParams(uint8_t paSelect, int8_t power, RadioRampTimes_t rampTime)
{
    ...
    {
        /* WORKAROUND - Better Resistance of the RFO High Power Tx to Antenna Mismatch, see STM32WL
         * Erratasheet*/
