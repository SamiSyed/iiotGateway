...
struct Radio_s {
   ...
   */
  bool (*IsChannelFree)(uint32_t freq, uint32_t rxBandwidth, int16_t rssiThresh,
  ...
  void (*TxPrbs)(void);
  /*!
   * \brief Sets the Transmitter in continuous un-modulated Carrier mode at
   * power dBm
   *
   ...
   * \brief Sets the reception parameters
   *
   * \param [in] modem        Radio modem to be used [GENERIC_FSK or
   * GENERIC_FSK] \param [in] config       configuration of receiver fsk field
   * to be used if modem =GENERIC_FSK lora field to be used if modem
   * =GENERIC_LORA \param [in] rxContinuous Sets the reception in continuous
   * mode [0: single mode, otherwise continuous mode] \param [in] symbTimeout
   * Sets the RxSingle timeout value FSK : timeout in number of bytes LoRa:
   * timeout in symbols \return 0 when no parameters error, -1 otherwise
   */
  int32_t (*RadioSetRxGenericConfig)(GenericModems_t modem,
   ...
   * or GENERIC_BPSK] \param [in] config       configuration of receiver fsk
   * field to be used if modem =GENERIC_FSK lora field to be used if modem
   * =GENERIC_LORA bpsk field to be used if modem =GENERIC_BPSK \param [in]
   * power        Sets the output power [dBm] \param [in] timeout      Reception
   * timeout [ms] \return 0 when no parameters error, -1 otherwise
   */
  int32_t (*RadioSetTxGenericConfig)(GenericModems_t modem,
   ...
   * size in bytes to feed \return 0 when no parameters error, -1 otherwise
   */
  int32_t (*TransmitLongPacket)(uint16_t payload_size, uint32_t timeout,
                                void (*TxLongPacketGetNextChunkCb)(
   ...
   * sensitivity \param [in] timeout             Reception timeout [ms] \param
   * [in] RxLongStorePacketChunkCb   callback to be implemented on user side to
   * record partial chunk in the application buffer: source buffer allocated in
   * the radio driver size: size in bytes to record \return 0 when no parameters
   * error, -1 otherwise
   */
  int32_t (*ReceiveLongPacket)(
  ...
  radio_status_t (*LrFhssGetTimeOnAirInMs)(
      const radio_lr_fhss_time_on_air_params_t *params,
