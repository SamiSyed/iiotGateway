...
typedef enum
{
    GENERIC_FSK = 0,
    GENERIC_LORA,
    GENERIC_BPSK, /*Tx only. In this mode, only payload is generated at the antenna (e.g. preamble
                     nor syncword is generated and must be placed into the payload*/
...
typedef enum
{
    ...
    RADIO_LORA_PACKET_FIXED_LENGTH = 0x01,    //!< The packet is known on both sides, no header
...
typedef enum
{
    RADIO_FSK_ADDRESSCOMP_FILT_OFF = 0x00, //!< No correlator turned on, i.e. do not search for
                                           //!< SyncWord
...
typedef enum
{
    RADIO_FSK_PACKET_FIXED_LENGTH = 0x00, //!< The packet is known on both sides, no header included
                                          //!< in the packet
    RADIO_FSK_PACKET_VARIABLE_LENGTH = 0x01, //!< 1 byte  packet length field inserted after the
                                             //!< sync word*/
    RADIO_FSK_PACKET_2BYTES_LENGTH = 0x02 //!< 2 bytes packet length field inserted after the sync
                                          //!< word, payload size greater than 255 bytes */
...
typedef struct
{
    ...
    RADIO_Ld_Opt_t LowDatarateOptimize; /*0 inactive, 1 active, otherwise auto (active for SF11 and
                                           SF12)*/
...
typedef struct
{
    ...
    RADIO_FSK_PacketLengthModes_t LengthMode; /* If the header is explicit, it will be transmitted
...
typedef struct
{
    ...
    RADIO_FSK_ModShapings_t ModulationShaping;
    RADIO_FSK_PacketLengthModes_t HeaderType; /* If the header is explicit, it will be transmitted
                                                 in the GFSK packet. If the header is implicit, it
                                                 will not be transmitted */
    RADIO_FSK_CrcTypes_t CrcLength;           /* Size of the CRC block in the GFSK packet */
