...
typedef enum {
  GENERIC_FSK = 0,
  GENERIC_LORA,
  GENERIC_BPSK, /*Tx only. In this mode, only payload is generated at the
                   antenna (e.g. preamble nor syncword is generated and must be
                   placed into the payload*/
...
typedef enum {
  ...
  RADIO_LORA_PACKET_FIXED_LENGTH = 0x01, //!< The packet is known on both sides,
...
typedef enum {
  ...
  RADIO_LORA_LOWDR_OPT_AUTO =
      0x02, /*Forced to 1 when SF11 or SF12, 0 otherwise*/
} RADIO_Ld_Opt_t;
...
typedef enum {
  ...
  RADIO_FSK_PREAMBLE_DETECTOR_08_BITS =
      0x04, //!< Preamble detection length 8 bits
  RADIO_FSK_PREAMBLE_DETECTOR_16_BITS =
      0x05, //!< Preamble detection length 16 bits
  RADIO_FSK_PREAMBLE_DETECTOR_24_BITS =
      0x06, //!< Preamble detection length 24 bits
  RADIO_FSK_PREAMBLE_DETECTOR_32_BITS =
 ...
 */
typedef enum {
  RADIO_FSK_ADDRESSCOMP_FILT_OFF =
 ...
 */
typedef enum {
  RADIO_FSK_PACKET_FIXED_LENGTH = 0x00, //!< The packet is known on both sides,
                                        //!< no header included in the packet
  RADIO_FSK_PACKET_VARIABLE_LENGTH =
      0x01, //!< 1 byte  packet length field inserted after the sync word*/
  RADIO_FSK_PACKET_2BYTES_LENGTH =
      0x02 //!< 2 bytes packet length field inserted after the sync word,
           //!< payload size greater than 255 bytes */
...
typedef struct {
  ...
  RADIO_Ld_Opt_t LowDatarateOptimize; /*0 inactive, 1 active, otherwise auto
                                         (active for SF11 and SF12)*/
...
typedef struct {
  ...
  uint16_t whiteSeed;        /* WhiteningSeed, whitening can also be disabled by
  ...
  RADIO_FSK_PacketLengthModes_t
      LengthMode; /* If the header is explicit, it will be transmitted in the
                     GFSK packet. If the header is implicit, it will not be
                     transmitted */
...
typedef struct {
  ...
  uint16_t CrcSeed;
  uint16_t whiteSeed; /* Whitening seed, whitening can be disabled by setting
                         this field to 0 */
  uint8_t SyncWordLength; /* in Byte */
  RADIO_FSK_ModShapings_t ModulationShaping;
  RADIO_FSK_PacketLengthModes_t
      HeaderType; /* If the header is explicit, it will be transmitted in the
                     GFSK packet. If the header is implicit, it will not be
                     transmitted */
  RADIO_FSK_CrcTypes_t CrcLength; /* Size of the CRC block in the GFSK packet */
