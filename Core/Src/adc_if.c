...
#define TEMPSENSOR_TYP_CAL1_V                                                                      \
    ((int32_t)760) /*!< Internal temperature sensor, parameter V30 (unit: mV). Refer to device     \
                      datasheet for min/typ/max values. */
#define TEMPSENSOR_TYP_AVGSLOPE                                                                    \
    ((int32_t)2500) /*!< Internal temperature sensor, parameter Avg_Slope (unit: uV/DegCelsius).   \
...
int16_t SYS_GetTemperatureLevel(void)
{
    ...
    if (((int32_t)*TEMPSENSOR_CAL2_ADDR - (int32_t)*TEMPSENSOR_CAL1_ADDR) != 0)
    {
        ...
        temperatureDegreeC
            = __LL_ADC_CALC_TEMPERATURE(batteryLevelmV, measuredLevel, LL_ADC_RESOLUTION_12B);
    }
    else
    {
