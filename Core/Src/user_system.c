...
static uint16_t filteredSensorValues[NUMBER_OF_SENSORS]; /* Global Sensor Values*/
...
SystemError initSensorFilter(void)
{
    ...
    {
        error = MovingAverage_Init(&sensorFilters[sId], &filterBuffer[sId][0], FILTER_BUFFER_SIZE);
        if (error != NO_ERROR)
        {
            break;
        }
...
void runAllFilter(void)
{
    for (uint8_t sId = 0; sId < (uint8_t)NUMBER_OF_SENSORS; sId++)
    {
        filteredSensorValues[sId] = runFilter(&sensorFilters[sId], newValueBuffer[sId]);
    }
}
...
SystemError gsmInit(void)
{
    ...
    status = sendATCommandRetry(AT_SAPBR, "3,1,\"Contype\",\"GPRS\"", AT_SAPBR, ADD_AT);
    if (!lastCommandOK)
    {
        return status;
    }
    ...
    status = sendATCommandRetry(AT_SMCONF, "\"URL\",broker.emqx.io:1883", AT_OK, ADD_AT);
...
char *prepareLoraMessage(uint8_t sID)
{
    ...
    snprintf(loraMessage_TX, sizeof(loraMessage_TX), "%s#%s", IOT_GATEWAY_KEY, sIDString);
