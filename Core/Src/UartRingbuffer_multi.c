...
void GetDataFromBuffer(char *startString,
                       char *endString,
                       char *buffertocopyfrom,
...
{
    if (uart == usb_uart)
        return (uint16_t)(UART_BUFFER_SIZE + _rx_buffer1->head - _rx_buffer1->tail)
               % UART_BUFFER_SIZE;
    else if (uart == gsm_uart)
        return (uint16_t)(UART_BUFFER_SIZE + _rx_buffer2->head - _rx_buffer2->tail)
               % UART_BUFFER_SIZE;
    return -1;
}
