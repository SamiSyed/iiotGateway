...
void Ringbuf_init(void)
{
  ...
  __HAL_UART_ENABLE_IT(usb_uart, UART_IT_ERR);
  __HAL_UART_ENABLE_IT(gsm_uart, UART_IT_ERR);

  /* Enable the UART Data Register not empty Interrupt */
  __HAL_UART_ENABLE_IT(usb_uart, UART_IT_RXNE);
  __HAL_UART_ENABLE_IT(gsm_uart, UART_IT_RXNE);
}
...
void Uart_write(int c, UART_HandleTypeDef *uart)
{
	if (c>=0)
	{
		...
		__HAL_UART_ENABLE_IT(usb_uart, UART_IT_TXE); // Enable UART transmission interrupt
			...
			__HAL_UART_ENABLE_IT(gsm_uart, UART_IT_TXE); // Enable UART transmission interrupt
			}
	}
}
...
{
	if (uart == usb_uart) return (uint16_t)(UART_BUFFER_SIZE + _rx_buffer1->head - _rx_buffer1->tail) % UART_BUFFER_SIZE;
	else if (uart == gsm_uart) return (uint16_t)(UART_BUFFER_SIZE + _rx_buffer2->head - _rx_buffer2->tail) % UART_BUFFER_SIZE;
	return -1;
}
...
int16_t Get_position (char *string, UART_HandleTypeDef *uart)
{
  ...
  if (uart == usb_uart)
  {
	 ...
	 {
	  if (Uart_read(usb_uart) == string[so_far])
	  		{
  ...
  else if (uart == gsm_uart)
  {
	 ...
	 {
	  if (Uart_read(gsm_uart) == string[so_far])
	  		{
	  		  counter=UART_BUFFER_SIZE-1;
	  		  so_far++;
	  		}
