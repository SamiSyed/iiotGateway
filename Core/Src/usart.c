...
#define MAIN_BUFFER_SIZE (RX_BUFFER_SIZE * 2)
...
/* USER CODE BEGIN 1 */
void initUart(void) { HAL_UARTEx_ReceiveToIdle_DMA(&huart1, &rxBuffer, 10); }

HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart) {
  if (huart->Instance == USART1) {
    memcpy(mainBuffer, rxBuffer, RX_BUFFER_SIZE);
    // printf("-------------------Inside call back\r\n");
    debug_RxCplt++;
