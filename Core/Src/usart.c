...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size) {
  if (huart->Instance == USART2) {
    ...
    //   uint16_t dataToCopy = MAIN_BUFFER_SIZE - oldPos;
    //   memcpy((uint8_t *)mainBuffer + oldPos, rxBuffer, dataToCopy);

    //   oldPos = 0;
    //   memcpy((uint8_t *)mainBuffer, (uint8_t *)rxBuffer + dataToCopy,
    //          (Size - dataToCopy));
    //   newPos = Size - dataToCopy;
    // } else {
    //   memcpy((uint8_t *)mainBuffer + oldPos, rxBuffer, Size);
    //   newPos = oldPos + Size;
    // }
...
bool replyContains(char *reply) {
  ...
  if (strncmp(rxBuffer, reply, strlen(reply)) != NULL) {
    return true;
  }
  return false;
}
