 ...
 * This copying will take place in the blocking mode, so you won't be able to perform any other
 * operations Returns 1 on success and -1 otherwise USAGE: while (!(Copy_Upto ("some string",
 * buffer, uart)));
 */
 ...
 * some particular string is detected Returns 1 on success and -1 otherwise USAGE: while
 * (!(Get_after ("some string", 6, buffer, uart)));
 */
